...
- "Submit Plan Pricing Matrix" now saves the whole matrix in one atomic request (`PUT /billing/plans/<id>/pricing-matrix`, `@super_admin_required`, new `plan_pricing_ops.replace_matrix()`), together with minimum billing months. The page previously saved row by row, so moving a bracket boundary failed with `Pricing ranges overlap` against the not-yet-updated neighbouring row and left the matrix half-saved. The whole submission is validated first (existing per-tier rules plus overlaps between submitted active rows, and tier IDs must belong to the plan); a rejected matrix changes nothing. Regions never configured (GH/TG placeholder rows) are skipped rather than blocking the save, since billing falls back to Global tiers. The page now keeps the active tab's edits, shows validation and API errors, and blocks double submission while saving.
