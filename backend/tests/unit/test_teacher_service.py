...
def test_create_teacher(app, teacher_service):
    ...
    Supplying 'employee_id' in the data bypasses the while-True uniqueness
    loop inside create_teacher.  Patching at app.services.teacher_service.*
    ensures the mocks are seen by the running service code (the service
    already imported Teacher/User at module load, so patching
    app.models.teacher.Teacher has no effect there).
    """
    teacher_data = {
    ...
    with app.app_context():
        with patch('app.services.teacher_service.User') as mock_user_class, \
            ...
            # No existing user with that email → service will create one
            mock_user_class.query.filter_by.return_value.first.return_value = None
            mock_user_class.return_value = mock_user

            # employee_id uniqueness check passes; no duplicate teacher profile
            mock_teacher_class.query.filter_by.return_value.first.return_value = None

            # Teacher(**payload) returns our mock instance
