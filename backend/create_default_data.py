...
def create_default_data():
    ...
    try:
        ...
        # Delete session tokens and other user-related data using raw SQL
        db.session.execute(db.text("DELETE FROM session_tokens WHERE TRUE;"))
        db.session.execute(db.text("DELETE FROM password_history WHERE TRUE;"))
        db.session.execute(db.text("DELETE FROM api_keys WHERE TRUE;"))
        db.session.execute(db.text("DELETE FROM password_reset_tokens WHERE TRUE;"))
        db.session.execute(db.text("DELETE FROM user_role_assignments_detailed WHERE TRUE;"))
        db.session.execute(db.text("DELETE FROM user_roles WHERE TRUE;"))
    ...
    if not tenant:
        tenant = Tenant(
            ...
            name='Default School',
            country_code='GH',
            schema_name='tenant_default_school'
        )
    ...
    from app.models.department import Department
    departments = [
        Department(tenant_id=tenant.id, name='Mathematics', code='MATH', description='Mathematics Department'),
        Department(tenant_id=tenant.id, name='English', code='ENG', description='English Department'),
        Department(tenant_id=tenant.id, name='Science', code='SCI', description='Science Department'),
        Department(tenant_id=tenant.id, name='Social Studies', code='SS', description='Social Studies Department'),
        Department(tenant_id=tenant.id, name='Technology', code='TECH', description='Technology Department'),
        Department(tenant_id=tenant.id, name='Athletics', code='ATH', description='Athletics Department'),
        Department(tenant_id=tenant.id, name='Arts', code='ART', description='Arts Department'),
    ]
    
    for dept in departments:
        db.session.add(dept)
    
    db.session.flush()  # To get department IDs
    
    # Create a mapping of department names to IDs
    dept_map = {dept.name: dept.id for dept in departments}
    
    # Create subjects with department_id instead of department
    subjects = [
        Subject(tenant_id=tenant.id, name='Mathematics', code='MATH101', description='Basic mathematics including algebra and geometry', department_id=dept_map['Mathematics'], credit_hours=4),
        Subject(tenant_id=tenant.id, name='English Literature', code='ENG101', description='Study of classic and modern literature', department_id=dept_map['English'], credit_hours=3),
        Subject(tenant_id=tenant.id, name='Physics', code='PHY101', description='Introduction to physics concepts', department_id=dept_map['Science'], credit_hours=4),
        Subject(tenant_id=tenant.id, name='Chemistry', code='CHEM101', description='Basic chemistry principles', department_id=dept_map['Science'], credit_hours=4),
        Subject(tenant_id=tenant.id, name='Biology', code='BIO101', description='Study of living organisms', department_id=dept_map['Science'], credit_hours=3),
        Subject(tenant_id=tenant.id, name='History', code='HIST101', description='World history overview', department_id=dept_map['Social Studies'], credit_hours=3),
        Subject(tenant_id=tenant.id, name='Geography', code='GEO101', description='Study of Earth and human geography', department_id=dept_map['Social Studies'], credit_hours=3),
        Subject(tenant_id=tenant.id, name='Computer Science', code='CS101', description='Introduction to programming', department_id=dept_map['Technology'], credit_hours=4),
        Subject(tenant_id=tenant.id, name='Physical Education', code='PE101', description='Sports and physical fitness', department_id=dept_map['Athletics'], credit_hours=2),
        Subject(tenant_id=tenant.id, name='Art', code='ART101', description='Visual arts and creativity', department_id=dept_map['Arts'], credit_hours=2),
        Subject(tenant_id=tenant.id, name='Music', code='MUS101', description='Music theory and practice', department_id=dept_map['Arts'], credit_hours=2),
    ]
    
    for subject in subjects:
        db.session.add(subject)
    
    db.session.flush()  # To get subject IDs
    
    # Create a mapping of subject IDs to subject objects for later use
    ...
    first_names_male = ['John', 'Michael', 'David', 'James', 'Robert', 'William', 'Richard', 'Joseph', 'Thomas', 'Christopher', 'Daniel', 'Paul', 'Mark', 'Donald', 'Steven', 'Kenneth', 'Andrew', 'Joshua', 'Kevin', 'Brian']
    ...
    for class_obj in classes:
        ...
        for subject_id in class_subject_ids:
            for exam_type in exam_types:
                ...
                # Get the subject object from the mapping
                subject_obj = subject_map.get(subject_id)
                if subject_obj is None:
                    continue  # Skip if subject not found
                
                exam = Exam(
                    title=f'{exam_type} - {subject_obj.name}',
                    description=f'{exam_type} examination for {subject_obj.name}',
                    exam_date=exam_date,
