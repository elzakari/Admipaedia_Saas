...
does not already exist. Also ensures billing_invoices and payment_gateways
have all the columns that the current SQLAlchemy models require.
"""
...
def upgrade():
    ...
    # ── Create billing_payments table if missing ──────────────────────────────
    ...
    if insp.has_table('payment_gateways'):
        existing = {c['name'] for c in insp.get_columns('payment_gateways')}

        if 'display_name' not in existing:
            op.add_column('payment_gateways', sa.Column('display_name', sa.String(128), nullable=True))
        if 'environment' not in existing:
            op.add_column('payment_gateways', sa.Column('environment', sa.String(16), nullable=False, server_default='sandbox'))
        if 'supported_channels' not in existing:
            op.add_column('payment_gateways', sa.Column('supported_channels', _json_type(), nullable=True))
        if 'updated_at' not in existing:
            op.add_column('payment_gateways', sa.Column('updated_at', sa.DateTime(timezone=True), server_default=now_default))


def downgrade():
    # Additive-only; no column drops on downgrade to prevent accidental data loss.
    conn = op.get_bind()
    insp = sa.inspect(conn)
