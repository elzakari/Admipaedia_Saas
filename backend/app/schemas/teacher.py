...
class TeacherSchema(Schema):
    ...
    employee_id = fields.String(
        required=False, validate=validate.Length(min=3, max=20), allow_none=True
    )
    ...
    gender = fields.String(
        validate=validate.OneOf(["male", "female", "other"]), allow_none=True
    )
    ...
    status = fields.String(
        validate=validate.OneOf(["active", "inactive", "on_leave"]), allow_none=True
    )
