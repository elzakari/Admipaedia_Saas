...
class ParentSchema(Schema):
        ...
        return [
            {
                ...
                "class_name": (
                    (getattr(s.class_, "display_name", None) or s.class_.name)
                    if s.class_
                    else None
                ),
            }
    ...
    def get_phone(self, obj):
        user = getattr(obj, "user", None)
        profile = getattr(user, "profile", None) if user else None
        return getattr(profile, "phone", None) or getattr(
            obj, "emergency_contact", None
        )
...
class ParentCreateSchema(Schema):
    ...
    status = fields.Str(
        validate=validate.OneOf(["active", "inactive"]), load_default="active"
    )
...
class ParentUpdateSchema(Schema):
    ...
    relationship = fields.Str(validate=validate.Length(max=50))
    status = fields.Str(
        validate=validate.OneOf(["active", "inactive"]), load_default=None
    )
