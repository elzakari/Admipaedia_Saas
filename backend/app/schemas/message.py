...
class MessageSchema(Schema):
    ...
    recipient_type = fields.String(
        ...
        validate=validate.OneOf(
            ["admin", "teacher", "student", "parent", "class", "user"]
        ),
    )
    ...
    def get_attachments_info(self, obj):
        ...
        db_attachments = Attachment.query.filter_by(
            entity_type="message", entity_id=str(obj.id)
        ).all()
        if db_attachments:
...
class MessageCreateSchema(Schema):
    ...
    def validate_recipient(self, data, **kwargs):
        if not data.get("recipient_ref") and (
            not data.get("recipient_id") or not data.get("recipient_type")
        ):
