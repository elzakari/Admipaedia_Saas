...
class BuyFormSchema(Schema):
    ...
    student_first_name = fields.String(
        required=True, validate=validate.Length(min=2, max=100)
    )
    student_last_name = fields.String(
        required=True, validate=validate.Length(min=2, max=100)
    )
...
class SaveDraftSchema(Schema):
    ...
    student_first_name = fields.String(
        required=False, validate=validate.Length(min=2, max=100)
    )
    student_last_name = fields.String(
        required=False, validate=validate.Length(min=2, max=100)
    )
...
class ReviewApplicationSchema(Schema):
    ...
    status = fields.String(
        required=True,
        validate=validate.OneOf(["under_review", "approved", "rejected", "returned"]),
    )
