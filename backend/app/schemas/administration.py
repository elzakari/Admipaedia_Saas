...
class BudgetSchema(Schema):
    ...
    budget_year = fields.Integer(
        required=True, validate=validate.Range(min=2020, max=2050)
    )
...
class TransactionSchema(Schema):
    ...
    payment_method = fields.String(
        validate=validate.OneOf(
            ["cash", "bank_transfer", "cheque", "mobile_money", "card"]
        ),
        load_default="cash",
    )
...
class FeeStructureSchema(Schema):
    ...
    tuition_fee = fields.Decimal(
        required=True, places=2, validate=validate.Range(min=0)
    )
    transport_fee = fields.Decimal(
        places=2, validate=validate.Range(min=0), load_default=0
    )
    meal_fee = fields.Decimal(places=2, validate=validate.Range(min=0), load_default=0)
    activity_fee = fields.Decimal(
        places=2, validate=validate.Range(min=0), load_default=0
    )
    library_fee = fields.Decimal(
        places=2, validate=validate.Range(min=0), load_default=0
    )
    technology_fee = fields.Decimal(
        places=2, validate=validate.Range(min=0), load_default=0
    )
    other_fees = fields.Decimal(
        places=2, validate=validate.Range(min=0), load_default=0
    )
...
class FeeRecordSchema(Schema):
    ...
    term = fields.String(
        validate=validate.OneOf(["term_1", "term_2", "term_3", "annual"]),
        load_default="term_1",
    )
    total_amount = fields.Decimal(
        required=True, places=2, validate=validate.Range(min=0)
    ...
    status = fields.String(
        validate=validate.OneOf(["pending", "partial", "paid", "overdue"]),
        load_default="pending",
    )
...
class FeePaymentSchema(Schema):
    ...
    mobile_money_number = fields.String(
    ...
    status = fields.String(
        validate=validate.OneOf(["pending", "verified", "cancelled"]),
        load_default="pending",
    )
    created_at = fields.DateTime(dump_only=True)
    updated_at = fields.DateTime(dump_only=True)
...
class InventoryItemSchema(Schema):
    ...
    status = fields.String(
        validate=validate.OneOf(["active", "inactive", "discontinued", "out_of_stock"]),
        load_default="active",
    )
    created_at = fields.DateTime(dump_only=True)
    updated_at = fields.DateTime(dump_only=True)
...
class MaintenanceRequestSchema(Schema):
    ...
    estimated_cost = fields.Decimal(
        places=2, validate=validate.Range(min=0), allow_none=True
    )
    ...
    contractor_contact = fields.String(
    ...
    facility = fields.Nested(
        "FacilitySchema",
        only=("id", "name", "facility_type", "location"),
        dump_only=True,
    )
    ...
    def validate_completed_date(self, value, **kwargs):
        if (
            hasattr(self, "reported_date")
            and self.reported_date
            and value < self.reported_date
        ):
...
class AssetSchema(Schema):
    ...
    condition = fields.String(
        validate=validate.OneOf(["excellent", "good", "fair", "poor", "damaged"]),
        load_default="good",
    )
    is_active = fields.Boolean(load_default=True)
    ...
    def validate_current_value(self, value, **kwargs):
        if (
            hasattr(self, "purchase_cost")
            and self.purchase_cost
            and value > self.purchase_cost
        ):
...
class FinancialSummarySchema(Schema):
    ...
    total_billed = fields.Decimal(places=2)
    total_collected_payments = fields.Decimal(places=2)
    total_income = fields.Decimal(places=2)
    total_expenses = fields.Decimal(places=2)
    net_balance = fields.Decimal(places=2)
    net_income = fields.Decimal(places=2)
...
class FacilitySchema(Schema):
    ...
    description = fields.String(validate=validate.Length(max=500), allow_none=True)
    is_active = fields.Method(
        "get_is_active", deserialize="load_is_active", load_default=True
    )
    maintenance_schedule = fields.String(
        validate=validate.OneOf(
    ...
    # Nested relationships
    maintenance_requests = fields.Nested(
        "MaintenanceRequestSchema", many=True, dump_only=True, exclude=["facility"]
    )
    assets = fields.Nested(
        "AssetSchema", many=True, dump_only=True, exclude=["facility"]
    )
...
class SystemSettingSchema(Schema):
    ...
    setting_type = fields.String(
        validate=validate.OneOf(["string", "int", "float", "boolean", "json"]),
        load_default="string",
    )
    updated_at = fields.DateTime(dump_only=True)
