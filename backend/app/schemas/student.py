...
def _extract_parent_name_parts_from_user(user):
    ...
    first_name = (
        getattr(profile, "first_name", None) or getattr(user, "first_name", None) or ""
    )
    last_name = (
        getattr(profile, "last_name", None) or getattr(user, "last_name", None) or ""
    )
...
def _resolve_student_parent_identity(obj):
    ...
    if parent:
        ...
        if not parent_phone and user:
            parent_phone = getattr(user, "phone", None) or getattr(
                user, "telephone", None
            )
    ...
    return {
        ...
        or "",
        "email": getattr(obj, "father_email", None)
        or getattr(obj, "mother_email", None),
...
class StudentSchema(Schema):
    ...
    name = fields.String(
        validate=validate.Length(max=200), allow_none=True, dump_only=True
    )
    surname = fields.String(
        validate=validate.Length(max=100), allow_none=True, dump_only=True
    )
    ...
    religious_denomination = fields.String(
    ...
    father_email = fields.Email(
        allow_none=True, load_default=None
    )  # Fix validation for empty strings
    father_profession = fields.String(
        validate=validate.Length(max=100), allow_none=True
    )
    ...
    profile_picture = fields.Method(
        "get_profile_picture", deserialize="load_profile_picture"
    )
    ...
    def get_grade_level(self, obj):
        if isinstance(obj, dict):
            ...
            if gl_name:
                return {"id": "unassigned", "name": gl_name, "code": None}
            return None
        ...
        if obj.grade_level:
            ...
            return {"id": "unassigned", "name": obj.class_.grade_level, "code": None}
        return None
...
class StudentListSchema(Schema):
    ...
    def get_attendance_percentage(self, obj):
        ...
        present_days = len(
            [a for a in obj.attendances if a.status in ["present", "late"]]
        )
