...
class ExamSchema(Schema):
    ...
    status = fields.String(
        validate=validate.OneOf(["scheduled", "ongoing", "completed", "cancelled"])
    )
    ...
    class_ = fields.Nested(
        "ClassSchema",
        only=("id", "name", "grade_level", "section", "academic_year"),
        dump_only=True,
    )
    subject = fields.Nested(
        "SubjectSchema", only=("id", "name", "code"), dump_only=True
    )
    ...
    def validate_passing_marks(self, value, **kwargs):
        ...
        if (
            "total_marks" in kwargs.get("data", {})
            and value > kwargs["data"]["total_marks"]
        ):
...
class ExamCreateSchema(Schema):
    ...
    created_by = fields.Integer(
        allow_none=True
    )  # Will be set to current user if not provided
    status = fields.String(
        validate=validate.OneOf(["scheduled", "ongoing", "completed", "cancelled"])
    )
...
class ExamListSchema(Schema):
    ...
    class_ = fields.Nested(
        "ClassSchema", only=("id", "name", "grade_level", "section"), dump_only=True
    )
    subject = fields.Nested(
        "SubjectSchema", only=("id", "name", "code"), dump_only=True
    )
