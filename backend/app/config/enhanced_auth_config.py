...
class EnhancedAuthConfig:
    ...
    MFA_CODE_VALIDITY_WINDOW = int(
        os.environ.get("MFA_CODE_VALIDITY_WINDOW", "30")
    )  # seconds
    ...
    # Device Management
    TRUSTED_DEVICE_DURATION = timedelta(
        days=int(os.environ.get("TRUSTED_DEVICE_DAYS", "90"))
    )
    MAX_TRUSTED_DEVICES = int(os.environ.get("MAX_TRUSTED_DEVICES", "10"))

    # Security Settings
    PASSWORD_RESET_TOKEN_EXPIRES = timedelta(
        hours=int(os.environ.get("PASSWORD_RESET_HOURS", "24"))
    )
    ...
    LOGIN_RATE_LIMIT = int(
        os.environ.get("LOGIN_RATE_LIMIT", "5")
    )  # attempts per window
    ...
    THREAT_DETECTION_ENABLED = (
        os.environ.get("THREAT_DETECTION_ENABLED", "true").lower() == "true"
    )
    IP_GEOLOCATION_ENABLED = (
        os.environ.get("IP_GEOLOCATION_ENABLED", "true").lower() == "true"
    )
    SECURITY_AUDIT_ENABLED = (
        os.environ.get("SECURITY_AUDIT_ENABLED", "true").lower() == "true"
    )
    ...
    def validate_config(cls):
        ...
        if cls.MFA_SMS_ENABLED and not all(
