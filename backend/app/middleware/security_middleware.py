...
import bleach
import structlog
from flask import (current_app, g, has_request_context, jsonify, make_response,
                   request, session)
...
class RateLimiter:
    ...
    def is_allowed(
        self, identifier: str, limit: int, window: int, burst_limit: int = None
    ) -> Tuple[bool, Dict]:
        """
        ...
        if len(request_times) >= effective_limit:
            ...
            return False, {"blocked": True, "retry_after": 60, "requests_remaining": 0}
...
class InputSanitizer:
    ...
    def sanitize_input(cls, data: any, field_type: str = "text") -> any:
        ...
        if isinstance(data, dict):
            return {
                key: cls.sanitize_input(value, field_type)
        ...
        if field_type == "html":
            for pattern in cls.SQL_INJECTION_PATTERNS:
                if re.search(pattern, data, re.IGNORECASE):
                    logger.warning(
                        "sql_injection_attempt", pattern=pattern, data=data[:100]
                    )
    ...
    @classmethod
    def sanitize_html(
        cls, html_content: str, allowed_tags=None, allowed_attributes=None
    ) -> str:
        ...
        def wrapper(*args, **kwargs):
            ...
            if hasattr(response, "headers"):
                ...
                response.headers["X-RateLimit-Remaining"] = str(
                    rate_info.get("requests_remaining", 0)
                )
                response.headers["X-RateLimit-Reset"] = str(
                    rate_info.get("reset_time", 0)
                )
                ...
                try:
                    ...
                    for key, value in request.json.items():
                        if key in SKIP_SANITIZE:
                            ...
                            field_type = (
                                field_types.get(key, "text") if field_types else "text"
                            )
                            sanitized_data[key] = InputSanitizer.sanitize_input(
                                value, field_type
                            )
                    ...
                    return (
                        jsonify({"success": False, "error": str(e), "message": str(e)}),
                        400,
                    )
