...
from flask import Blueprint, after_this_request, g, jsonify, request, send_file
...
def download_report_card_pdf():
    ...
    if not student_id or not academic_cycle_id:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Missing student_id or academic_cycle_id query arguments",
                }
            ),
            400,
        )
    ...
    try:
        ...
        pdf_stream = ReportCardGenerator.generate_report_card_pdf(
            student_id=int(student_id), academic_cycle_id=academic_cycle_id
        )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Failed to compile report card",
                    "error": str(e),
                }
            ),
            500,
...
def get_branch_ledger():
    ...
    if req_branch_id_str:
        try:
            ...
            return (
                jsonify({"success": False, "message": "Invalid branch_id format."}),
                400,
            )
    else:
    ...
    if not target_branch_id:
        return (
            jsonify({"success": False, "message": "Branch context not resolved."}),
            400,
        )
    ...
    try:
        metrics = FinancialLedgerService.get_branch_ledger_metrics(
            tenant_id, target_branch_id
        )
        return jsonify({"success": True, "data": serialize_decimals(metrics)})
    except ValueError as e:
        return jsonify({"success": False, "message": str(e)}), 404
    except Exception as e:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Failed to load ledger metrics.",
                    "error": str(e),
                }
            ),
            500,
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Failed to load global ledger metrics.",
                    "error": str(e),
                }
            ),
            500,
...
def get_timetable_slots():
    ...
    if not tenant_id or not branch_id:
        return (
            jsonify(
                {"success": False, "message": "Tenant and Branch context required."}
            ),
            400,
        )
...
def create_timetable_slot():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to create slot.", "error": str(e)}
            ),
            500,
        )
...
def update_timetable_slot(slot_id):
    ...
    data = request.json
    conflicts = TimetableEngine.check_conflicts(
        tenant_id, branch_id, data, current_slot_id=slot_id
    )
    if conflicts:
        return (
            jsonify(
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to update slot.", "error": str(e)}
            ),
            500,
        )
...
def delete_timetable_slot(slot_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to delete slot.", "error": str(e)}
            ),
            500,
        )
...
def paystack_webhook():
    ...
    from app.models.finance import PaymentAllocation, StudentFee
    ...
    if not signature:
        return (
            jsonify(
                {"success": False, "message": "Missing x-paystack-signature header"}
            ),
            401,
        )
    ...
    if not PaystackService.verify_webhook_signature(signature, raw_payload):
        return (
            jsonify({"success": False, "message": "Invalid cryptographic signature"}),
            401,
        )
    ...
    if event != "charge.success":
        return (
            jsonify(
                {"success": True, "message": f"Event {event} received and acknowledged"}
            ),
            200,
        )
    ...
    if not reference:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Missing reference in transaction payload",
                }
            ),
            400,
        )
    ...
    existing_payment = StudentPayment.query.filter_by(
    ...
    if (
        not tenant_id_str
        or not branch_id_str
        or not student_id_val
        or not student_fee_id_val
    ):
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Missing tenant_id, branch_id, student_id, or student_fee_id in metadata",
                }
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Invalid tenant_id or branch_id UUID format",
                }
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Invalid student_id or student_fee_id integer format",
                }
            ),
            400,
        )
    ...
    if not student_fee:
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Student fee record {student_fee_id} not found",
                }
            ),
            404,
        )
    ...
    if student_fee.student_id != student_id:
        return (
            jsonify(
                {"success": False, "message": "Student ID mismatch for target fee"}
            ),
            400,
        )
    ...
    try:
        ...
        student_fee.paid_amount = (
            Decimal(str(student_fee.paid_amount or "0.00")) + amount_paid
        )
...
def get_payment_config():
    ...
    return (
        jsonify(
            {"success": False, "message": "Paystack payment gateway not configured"}
        ),
        404,
    )
...
def get_admission_applications():
    ...
    query = AdmissionApplication.query.join(
        Parent, AdmissionApplication.parent_id == Parent.id
    ).filter(Parent.tenant_id == tenant_id)

    branch_id = getattr(g, "branch_id", None)
    if branch_id:
        query = query.join(
            Class, AdmissionApplication.target_class_id == Class.id
        ...
        result.append(
            {
                ...
                "status": app.status,
                "submission_date": (
                    app.submission_date.isoformat() if app.submission_date else None
                ),
...
def update_admission_status(application_id):
    ...
    app_record, student, raw_token, error = AdmissionService.change_application_status(
        application_id=application_id, new_status=new_status, tenant_id=tenant_id
    )
...
def get_attendance_analytics():
    ...
    stats = AttendanceAnalytics.get_monthly_branch_stats(
        branch_id=str(branch_id), year=year, month=month, class_id=class_id
    )
...
def log_student_attendance():
    ...
    try:
        ...
        parsed_date = datetime.strptime(
            str(date_str).split("T")[0].strip(), "%Y-%m-%d"
        ).date()
    except ValueError:
        return (
            jsonify(
    ...
    if not student or student.tenant_id != tenant_id:
        return (
            jsonify({"success": False, "message": "Student not found in this tenant."}),
            404,
        )
    ...
    if status == "absent":
        ...
        if not parent_phone:
            parent_phone = (
                student.father_contact or student.mother_contact or student.phone
            )
        if not parent_email:
        ...
        if parent_email:
            ...
            NotificationService.send_email(
                tenant_id, branch_id, parent_email, email_subject, email_content
            )
            notification_sent = True
...
@saas_report_bp.route("/saas/grade-levels", methods=["GET"])
@jwt_required()
@tenant_required
def get_saas_grade_levels():
    ...
    try:
        ...
        if not tenant_id:
            logger.warning(
                "get_saas_grade_levels: tenant_id context is missing. Falling back to defaults."
            )
        else:
            try:
                # Utilizing query_scoped() safely
                levels = (
                ...
                logger.error(
                    f"Database exception querying GradeLevel in saas route: {str(db_err)}"
                )
