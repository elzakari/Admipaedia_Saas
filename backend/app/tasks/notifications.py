...
from app.models.dashboard import CalendarEvent, Notification
...
def generate_attendance_notifications():
    ...
    try:
        ...
        absent_records = Attendance.query.filter(
            Attendance.date == yesterday, Attendance.status == "absent"
        ).all()
        ...
        logger.info(
            "Generated attendance notifications", count=len(parent_notifications)
        )
        return True
...
def generate_calendar_event_reminders():
    ...
    try:
        ...
        upcoming_events = CalendarEvent.query.filter(
            CalendarEvent.date >= today.date(), CalendarEvent.date <= tomorrow.date()
        ).all()
                ...
                for user in users:
                    ...
                    if not existing_notification:
                        ...
                        NotificationService.create_notification(
                            ...
                            notification_type="info",
                            user_id=user.id,
                            send_email=event.type
                            in ["exam", "meeting"],  # Send emails for important events
                            send_websocket=True,
                        )
...
def notify_event_changes(event_id, change_type):
    ...
    try:
        ...
        for role in target_roles:
            ...
            for user in users:
                NotificationService.create_notification(
                    ...
                    message=message,
                    notification_type=(
                        "warning" if change_type == "cancelled" else "info"
                    ),
                    user_id=user.id,
                    send_email=event.type
                    in ["exam", "meeting"],  # Send emails for important events
        ...
        logger.info(
            f"Generated event {change_type} notifications", count=notification_count
        )
        return True
    except Exception as e:
        logger.error(
            f"Failed to generate event {change_type} notifications", error=str(e)
        )
        return False
