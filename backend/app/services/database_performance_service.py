...
class DatabasePerformanceService:
    ...
    def _get_slow_queries_summary(self) -> List[Dict]:
        ...
        return sorted(
            recent_slow_queries, key=lambda x: x["execution_time"], reverse=True
        )[
            :10
    ...
    def _get_optimization_recommendations(self) -> List[str]:
        ...
        if len(self.query_log) > 0:
            avg_slow_time = sum(q["execution_time"] for q in self.query_log) / len(
                self.query_log
            )
    ...
    def optimize_query_plan(self, query_text: str) -> Dict[str, Any]:
        ...
        try:
            with db.engine.connect() as conn:
                ...
                return {"execution_plan": plan, "optimization_suggestions": suggestions}
        except Exception as e:
    ...
    def _analyze_execution_plan(self, plan: Dict) -> List[str]:
        ...
        if "Plan" in plan:
            ...
            if (
                node.get("Node Type") == "Nested Loop"
