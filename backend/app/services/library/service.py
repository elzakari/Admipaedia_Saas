...
from app.models.library import (Book, BookReservation, BookStatus,
                                BorrowRecord, BorrowStatus, FineRecord,
                                LibraryMember, LibrarySettings, MemberType)
...
class LibraryService:
    ...
    def _load_digital_resources():
        setting = LibrarySettings.query.filter_by(
    ...
    def list_digital_resources(search=None, category=None, resource_type=None):
        ...
        return sorted(
            filtered,
            key=lambda item: item.get("created_at") or item.get("uploadDate") or "",
            reverse=True,
        )
    ...
    def update_book(book_id, data):
        ...
        if "total_copies" in data:
            active_borrows = book.borrow_records.filter_by(
    ...
    def delete_book(book_id):
        ...
        active = BorrowRecord.query.filter_by(
            book_id=book.id, status=BorrowStatus.ACTIVE
        ).count()
        if active > 0:
    ...
    def get_borrow_records(
        page=1, per_page=20, status=None, search=None, member_type=None
    ):
        ...
        return query.order_by(BorrowRecord.created_at.desc()).paginate(
    ...
    def get_members(page=1, per_page=20, search=None, member_type=None, is_active=None):
        ...
        return query.order_by(LibraryMember.created_at.desc()).paginate(
            page=page, per_page=per_page, error_out=False
        )
    ...
    def return_book(borrow_record_id, returned_by_id, notes=None):
        ...
        if fine_amount > 0:
            ...
            record.member.total_fines = float(record.member.total_fines) + float(
                fine_amount
            )
        else:
            record.status = BorrowStatus.RETURNED
        ...
        reservation = (
            BookReservation.query.filter_by(book_id=record.book_id, status="active")
    ...
    def get_library_stats():
        ...
        total_copies = (
            db.session.query(func.coalesce(func.sum(Book.total_copies), 0)).scalar()
            or 0
        )
        available_copies = (
            db.session.query(func.coalesce(func.sum(Book.available_copies), 0)).scalar()
            or 0
        )
        ...
        returned_borrows = BorrowRecord.query.filter_by(
        ...
        return {
            ...
            "total_fines": float(
                db.session.query(func.coalesce(func.sum(FineRecord.amount), 0))
                .filter(FineRecord.status == "pending")
                .scalar()
                or 0
            ),
    ...
    def get_borrowing_activity(time_range="month"):
        ...
        records = (
            ...
            .order_by(func.date(BorrowRecord.borrow_date))
            .all()
        )
    ...
    def get_borrower_type_distribution():
        ...
        results = (
            db.session.query(LibraryMember.member_type, func.count(BorrowRecord.id))
            .join(BorrowRecord, LibraryMember.id == BorrowRecord.member_id)
            .group_by(LibraryMember.member_type)
            .all()
        )
    ...
    def get_overdue_trends():
        ...
        for row in results:
            ...
            if previous is None:
                ...
                trend = "stable"
            trend_rows.append(
                {"month": f"{year:04d}-{month:02d}", "count": count, "trend": trend}
            )
            previous = count
