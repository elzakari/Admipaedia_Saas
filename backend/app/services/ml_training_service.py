...
class MLTrainingService:
    ...
    @staticmethod
    def prepare_performance_features(
        student_id: int, lookback_days: int = 90
        ...
        try:
            ...
            features = {
                ...
                "score_std": (
                    np.std([g.percentage for g in grades]) if len(grades) > 1 else 0
                ),
                "grade_count": len(grades),
                "attendance_rate": (
                    len([a for a in attendance_records if a.status == "present"])
                    / len(attendance_records)
                    if attendance_records
                    else 0
                ),
                "days_since_last_grade": (
                    (end_date - max([g.created_at for g in grades])).days
                    if grades
                    else lookback_days
                ),
                "trend_slope": (
                    MLTrainingService._calculate_trend_slope(
                ...
                "consistency_score": (
                    MLTrainingService._calculate_consistency(
                        [g.percentage for g in grades]
                    )
                    if len(grades) > 1
                    else 0
                ...
                "recent_performance": (
                    np.mean([g.percentage for g in grades[-5:]])
                    if len(grades) >= 5
                    else (grades[-1].percentage if grades else 0)
                ),
            }
    ...
    def train_performance_prediction_model(
        ...
        try:
            ...
            if cls.PERFORMANCE_MODEL_PATH.exists() and not retrain:
                logger.info(
                    "Performance model already exists. Use retrain=True to force retraining."
                )
            ...
            for student in students:
                features_df = cls.prepare_performance_features(
                    student.id, lookback_days=120
                )
            ...
            logger.info(
                f"Performance prediction model trained successfully. RMSE: {rmse:.2f}"
            )
    ...
    def train_risk_assessment_model(cls, retrain: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if cls.RISK_MODEL_PATH.exists() and not retrain:
                logger.info(
                    "Risk model already exists. Use retrain=True to force retraining."
                )
                return {"status": "skipped", "message": "Model already exists"}
            ...
            # Cross-validation score
            cv_scores = cross_val_score(
                best_model, X, y_encoded, cv=5, scoring="accuracy"
            )
            ...
            logger.info(
                f"Risk assessment model trained successfully. Accuracy: {accuracy:.2f}"
            )
    ...
    def _calculate_risk_level(student_id: int) -> str:
        ...
        try:
            ...
            # Get grades
            grades = Grade.query.filter(
                Grade.student_id == student_id, Grade.created_at >= start_date
            ).all()
