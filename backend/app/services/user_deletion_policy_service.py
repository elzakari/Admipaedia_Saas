...
class UserDeletionPolicyService:
    ...
    def _user_role(user: Optional[User]) -> Optional[str]:
        ...
        try:
            if any(
                getattr(r, "name", None) == "super_admin" for r in (user.roles or [])
            ):
                return "super_admin"
            if any(
                getattr(r, "name", None) == "super_manager" for r in (user.roles or [])
            ):
    ...
    @staticmethod
    def _memberships_for_user(
        user_id: int,
    ) -> List[Tuple[TenantMembership, Optional[Tenant]]]:
        return (
    ...
    def _membership_summary(
        rows: List[Tuple[TenantMembership, Optional[Tenant]]],
    ) -> List[Dict[str, Any]]:
    ...
    def get_delete_status(
        user_id: int, actor_user_id: int, *, tenant_scope_id=None
    ) -> Dict[str, Any]:
        ...
        target_profile_tenant_ids = UserDeletionPolicyService._profile_tenant_ids(
            user_id
        )
        ...
        status: Dict[str, Any] = {
            ...
            "linked_tenant_ids": [
                str(tid) for tid in sorted(linked_tenant_ids, key=str)
            ],
        }
        ...
        if actor_role == "super_admin":
            ...
            tenant_status_map = {
                ...
                for tenant in Tenant.query.filter(
                    Tenant.id.in_(list(linked_tenant_ids))
                ).all()
            }
            active_tenants = [
                str(tid)
            ...
            if active_tenants:
                status["reasons"] = [
                    "Super Admin can only delete orphan users or users linked only to inactive or suspended schools"
                ]
        ...
        if "school_admin" in target_membership_roles or target_role in (
            "school_admin",
            "admin",
        ):
            status["reasons"] = [
