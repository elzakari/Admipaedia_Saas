...
class LibraryService:
    ...
    def create_library_member(
        user_id: int, member_type: str, **kwargs
    ) -> LibraryMember:
    ...
    @staticmethod
    def borrow_book(
        book_id: int, member_id: int, issued_by_id: int, due_date: Optional[date] = None
    ...
    @staticmethod
    def return_book(
        borrow_record_id: int, returned_by_id: int, return_date: Optional[date] = None
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(
                f"Book returned: {book.title} by {borrow_record.member.member_id}"
            )
    ...
    def get_member_borrow_history(
        member_id: int, page: int = 1, per_page: int = 20
    ) -> Any:
        """Get member's borrowing history"""
        return (
            BorrowRecord.query.filter_by(member_id=member_id)
    ...
    def get_library_statistics() -> Dict[str, Any]:
        ...
        try:
            ...
            available_copies = (
                db.session.query(func.sum(Book.available_copies)).scalar() or 0
            )
            ...
            total_fines = db.session.query(func.sum(FineRecord.amount)).filter_by(
                status="pending"
            ...
            return {
                ...
                "borrowing_rate": (
                    round((active_borrows / total_copies * 100), 2)
                    if total_copies > 0
                    else 0
                ),
                "overdue_rate": (
                    round((overdue_books / active_borrows * 100), 2)
                    if active_borrows > 0
                    else 0
                ),
            }
    ...
    def _generate_member_id(member_type: str) -> str:
        ...
        last_member = (
            LibraryMember.query.filter(LibraryMember.member_id.like(f"{prefix}%"))
            .order_by(desc(LibraryMember.member_id))
            .first()
        )
    ...
    @staticmethod
    def _create_fine_record(
        borrow_record: BorrowRecord, amount: Decimal, fine_type: str, issued_by_id: int
    ) -> FineRecord:
