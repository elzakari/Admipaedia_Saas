...
class CinetPayAdapter(PaymentGatewayAdapter):
    ...
    def initialize(
        ...
        *,
        ...
        if (
            ...
            or str(data.get("code")) not in ("201", "00", "0")
            and str(data.get("status")).lower() not in ("success", "successful")
        ):
            raise ValueError(data.get("message") or "CinetPay initialization failed")

        d = data.get("data") or data
        payment_url = (
            d.get("payment_url") or d.get("payment_url") or d.get("paymentUrl")
        )
    ...
    def verify(
        ...
        if not apikey or not site_id:
            return PaymentVerifyResult(
                status="failed", gateway_transaction_id=None, paid=False
            )
        ...
        if resp.status_code >= 400:
            return PaymentVerifyResult(
                status="failed", gateway_transaction_id=None, paid=False, raw=data
            )
        ...
        return PaymentVerifyResult(
            ...
            gateway_transaction_id=str(
                d.get("cpm_trans_id") or d.get("transaction_id") or ""
            )
            or None,
            paid=paid,
    ...
    def verify_webhook(
        ...
        token = (
            headers.get("x-token") or headers.get("X-Token") or headers.get("X-TOKEN")
        )
