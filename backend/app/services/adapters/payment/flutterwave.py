...
class FlutterwaveAdapter(PaymentGatewayAdapter):
    ...
    def initialize(
        ...
        *,
        ...
        resp = requests.post(
            url, headers=self._auth_headers(gateway), json=payload, timeout=15
        )
        data = resp.json() if resp.content else {}
        if resp.status_code >= 400 or str(data.get("status")).lower() not in (
            "success",
            "successful",
        ):
    ...
    def verify(
        ...
        resp = requests.get(
            url,
            headers=self._auth_headers(gateway),
            params={"tx_ref": payment.payment_reference},
            timeout=15,
        )
        data = resp.json() if resp.content else {}
