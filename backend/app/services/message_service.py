...
class MessageService:
        ...
        try:
            ...
            if not recipient_ref:
                ...
                if not recipient_id or not recipient_type:
                    raise ValueError(
                        "Either recipient_ref or recipient_id + recipient_type must be provided."
                    )
            ...
            resolved_recipients, r_type = MessageService.resolve_recipient_ref(
                recipient_ref, tenant_id=tenant_id
            )
            ...
            for r_id, r_role in resolved_recipients:
                ...
                if not rec_user:
                    raise ValueError(
                        f"Recipient user with ID {r_id} does not exist in users table."
                    )
                if not MessageService.validate_sender_recipient_relation(
                    data["sender_id"], r_id, tenant_id=tenant_id
                ):
                    raise ValueError(
                        f"Messaging not allowed between sender and recipient {r_id}."
                    )
            ...
            try:
                ...
                if created_messages:
                    ...
                    socketio.emit(
                        "message_sent",
                        {"success": True, "message": sender_msg_data},
                        room=f"user_{data['sender_id']}",
                        namespace="/messages",
                    )
                    socketio.emit(
                        "message_sent",
                ...
                logger.warning(
                    f"Failed to emit Socket.IO notification: {str(socket_err)}"
                )
    ...
    def create_bulk_message(data, tenant_id=None):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to emit Socket.IO notification for bulk messages: {str(socket_err)}"
                )
    ...
    def resolve_recipient_ref(recipient_ref, tenant_id=None):
        ...
        if ref_type == "class":
            if len(parts) < 3:
                raise ValueError(
                    f"Invalid class ref format. Expected class:<id>:<audience>, got: {recipient_ref}"
                )
            ...
            if ref_type == "parent":
                ...
                if not parent:
                    ...
                    if user and MessageService._get_user_type(user) == "parent":
                        if (
                            ...
                            and not MessageService._user_has_tenant_membership(
                                user.id, tenant_id
                            )
                        ):
                            raise ValueError(
                                "Parent does not belong to the active tenant."
                            )
                        return [(user.id, "parent")], "parent"
                    raise ValueError(
                        f"Parent profile with ID {numeric_id} does not exist."
                    )
                ...
                if not p_user_id or User.query.get(p_user_id) is None:
                    raise ValueError(
                        "Associated parent user account not found or deactivated."
                    )
                ...
                if not student:
                    ...
                    if user and MessageService._get_user_type(user) == "student":
                        if (
                            ...
                            raise ValueError(
                                "Student does not belong to the active tenant."
                            )
                        return [(user.id, "student")], "student"
                    raise ValueError(
                        f"Student profile with ID {numeric_id} does not exist."
                    )
                ...
                if not s_user_id or User.query.get(s_user_id) is None:
                    raise ValueError(
                        "Associated student user account not found or deactivated."
                    )
                ...
                if not teacher:
                    ...
                    if user and MessageService._get_user_type(user) == "teacher":
                        if (
                            ...
                            raise ValueError(
                                "Teacher does not belong to the active tenant."
                            )
                        return [(user.id, "teacher")], "teacher"
                    raise ValueError(
                        f"Teacher profile with ID {numeric_id} does not exist."
                    )
                ...
                if not t_user_id or User.query.get(t_user_id) is None:
                    raise ValueError(
                        "Associated teacher user account not found or deactivated."
                    )
    ...
    @staticmethod
    def validate_sender_recipient_relation(
        sender_id, recipient_user_id, tenant_id=None
    ):
        """
        ...
        return IdentityResolver.can_user_message_recipient(
            sender_id, f"user:{recipient_user_id}"
        )
    ...
    def _resolve_recipient(recipient_id, recipient_type):
        ...
        if not resolved_recipient_id:
            raise ValueError(
                f"Could not resolve recipient ID {original_recipient_id} with type {original_recipient_type} to a valid user."
            )

        resolved_user = User.query.get(resolved_recipient_id)
        if not resolved_user:
            raise ValueError(
                f"Resolved recipient ID {resolved_recipient_id} does not exist in users table."
            )
    ...
    @staticmethod
    def get_allowed_recipient_queries(
        current_user_id,
        current_role,
        tenant_id,
        type_filter,
        ...
        if current_role == "teacher":
            ...
            if type_filter == "class":
                classes_query = Class.query.filter(
                    Class.tenant_id == tenant_id, Class.id.in_(assigned_class_ids)
                )
                if search:
                ...
                if search:
                    search_pat = f"%{search}%"
                    parent_query = (
                        parent_query.join(User, User.id == Parent.user_id)
                ...
                admin_query = User.query.join(
                    TenantMembership, TenantMembership.user_id == User.id
                ).filter(
                    TenantMembership.tenant_id == tenant_id,
                    TenantMembership.role.in_(["admin", "school_admin", "super_admin"]),
                )
                if search:
                    search_pat = f"%{search}%"
                    admin_query = admin_query.outerjoin(
            ...
            if type_filter == "teacher":
                class_teachers = Class.query.filter(
