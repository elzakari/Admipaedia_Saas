...
class EnhancedStudentService(StudentService):
    ...
    def get_upload_directory(prefer_legacy: bool = False):
        ...
        if os.path.isabs(configured):
            ...
            upload_root = os.path.join(
                os.path.dirname(current_app.root_path), configured
            )
        if prefer_legacy:
    ...
    def allowed_file(filename):
        ...
        return (
            ...
            in EnhancedStudentService.ALLOWED_EXTENSIONS
        )
    ...
    def sync_user_avatar(student, avatar_url=None):
        ...
        normalized_avatar = (
            ...
            else EnhancedStudentService.build_profile_picture_url(
                getattr(student, "profile_picture", None)
            )
        )
    ...
    def create_student_with_user(student_data, user_data=None, tenant_id=None):
        ...
        try:
            ...
            student, error = StudentService(db.session).create_student(
            ...
            logger.info(
                "Student created with integrated user account",
        ...
        try:
            ...
            if file and EnhancedStudentService.allowed_file(file.filename):
                ...
                student.profile_picture = (
                    f"{EnhancedStudentService.UPLOAD_FOLDER}/{unique_filename}"
                )
                ...
                return (
                    EnhancedStudentService.build_profile_picture_url(
                        student.profile_picture
                    ),
                    None,
                )
    ...
    def bulk_import_students(
        file_path, create_users=False, update_existing=False, tenant_id=None
    ):
            ...
            for i in range(0, len(df), batch_size):
                ...
                for index, row in batch_df.iterrows():
                    try:
                        ...
                        existing_student_query = Student.query.filter_by(
                            admission_number=str(row["admission_number"])
                        )
                        if tenant_id is not None and hasattr(Student, "tenant_id"):
                            existing_student_query = existing_student_query.filter(
                        ...
                        if existing_student and update_existing:
                            ...
                            for column in df.columns:
                                if (
                                    ...
                                    and hasattr(existing_student, column)
                        ...
                        for field in optional_fields:
                            if field in df.columns and pd.notna(row[field]):
                                student_data[field] = (
                                    str(row[field])
                                    if field not in ["class_id", "parent_id"]
                                    else int(row[field])
                                )
                        ...
                        if create_users:
                            ...
                            user_data = {
                                "email": str(row["email"]),
                                "username": str(
                                    row.get("username", row["admission_number"])
                                ),
                                "password": str(row.get("password", "DefaultPass123!")),
                            }

                            student, error = (
                                EnhancedStudentService.create_student_with_user(
                                    student_data,
                                    user_data,
                                    tenant_id=tenant_id,
                                )
    ...
    def get_student_analytics(student_id, date_from=None, date_to=None):
        ...
        try:
            ...
            present_days = attendance_query.filter(
            ...
            attendance_rate = (
                ((present_days + late_days) / total_days * 100) if total_days > 0 else 0
            )
            ...
            performance_data = {
                "average_grade": (
                    float(perf_stats.avg_score)
                    if perf_stats and perf_stats.avg_score
                    else 0
                ),
            ...
            analytics = {
                ...
                "trends": {"weekly_attendance": trends},
            }
    ...
    @staticmethod
    def get_overall_analytics_summary(
        tenant_id=None, class_id=None, date_from=None, date_to=None
    ):
        ...
        try:
            ...
            present_attendance = attendance_query.filter(
            ...
            performance_query = db.session.query(func.avg(Grade.percentage)).join(
                Student
            )
            if tenant_id is not None:
            ...
            if tenant_id is not None:
                attendance_rollup = attendance_rollup.filter(
                    Student.tenant_id == tenant_id
                )
            if class_id:
                attendance_rollup = attendance_rollup.filter(
            ...
            for cls in classes:
                ...
                class_performance.append(
                    {"class_name": cls.name, "average_score": float(avg_percentage)}
                )
            ...
            trends_query = db.session.query(
                month_expr.label("month"), func.avg(Grade.percentage).label("avg_score")
            ).join(Student)
            if tenant_id is not None:
            ...
            academic_trends = (
                trends_query.group_by(month_expr).order_by(month_expr).all()
            )
    ...
    def link_student_to_parent(student_id, parent_id):
        ...
        try:
            ...
            logger.info(
                "Student linked to parent", student_id=student_id, parent_id=parent_id
            )
            return student, None
        ...
        try:
            ...
            report = {
                "student_info": {
                    ...
                    "admission_number": student.admission_number,
                    "user": (
                        {"username": student.user.username, "email": student.user.email}
                        if student.user
                        else None
                    ),
    ...
    def export_students_to_csv(
        class_id=None, grade_level=None, status=None, fields=None, tenant_id=None
    ):
        ...
        try:
            ...
            if grade_level:
                ...
                query = query.join(Student.class_).filter(
