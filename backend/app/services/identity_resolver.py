...
class IdentityResolver:
    ...
    def resolve_user_id(ref, expected_role=None):
            ...
            if user:
                if expected_role:
                    ...
                    if role != expected_role:
                        logger.warning(
                            "Role mismatch during direct ID resolution",
                            user_id=user_id,
        ...
        if resolved_id is not None:
            if expected_role:
                ...
                if role != expected_role:
                    logger.warning(
                        "Role mismatch after resolving reference",
                        ref=ref,
                        expected=expected_role,
                        actual=role,
                    )
    ...
    def resolve_class_parent_user_ids(class_id):
        ...
        parents = (
            Parent.query.join(Student, Student.parent_id == Parent.id)
            .filter(
    ...
    def can_user_message_recipient(sender_user_id, recipient_ref):
        ...
        try:
            resolved_recipients, ref_role = MessageService.resolve_recipient_ref(
                recipient_ref
            )
        except Exception as e:
            logger.warning(
                "Error resolving reference in can_user_message_recipient",
                error=str(e),
                ref=recipient_ref,
            )
            return False
        ...
        for recipient_user_id, recipient_role in resolved_recipients:
            ...
            tenant_context_active = (
                has_app_context() and getattr(g, "tenant_id", None) is not None
            )

            # Legacy/test bypass rules
            if (
                not tenant_context_active
                or sender_role == "user"
                or recipient_role == "user"
            ):
                continue
            ...
            sender_memberships = TenantMembership.query.filter_by(
                user_id=sender_user_id, status="active"
            ).all()
            recipient_memberships = TenantMembership.query.filter_by(
                user_id=recipient_user_id, status="active"
            ).all()
            ...
            if not common_tenants and sender_role not in (
                "super_admin",
                "super_manager",
            ):
                return False
            ...
            if sender_role == "teacher":
                if recipient_role == "teacher":
                    continue
                assigned_classes = IdentityResolver.resolve_teacher_class_ids(
                    sender_user_id
                )
                ...
                if recipient_role == "student":
                    ...
                    if (
                        not student_profile
                        or student_profile.class_id not in assigned_classes
                    ):
                        return False
                    ...
                    child_in_assigned = any(
                        c.class_id in assigned_classes for c in children if c.class_id
                    )
