...
def sign_invitation(invite: InvitationLink) -> tuple[int, str]:
    ...
    mac = hmac.new(
        _secret_bytes(), _msg_v1(invite, exp_ts).encode("utf-8"), hashlib.sha256
    ).digest()
...
def verify_invitation_signature(invite: InvitationLink, exp_ts: int, sig: str) -> bool:
    ...
    if not secrets.compare_digest(expected, sig):
        return False
    if invite.sig_hash and not secrets.compare_digest(
...
def enforce_create_rate_limit(
    ...
    allowed, info = rate_limiter.is_allowed(
        identifier, limit=limit, window=window, burst_limit=burst_limit
    )
    return allowed, info


def create_invitation_link(
    *, tenant_id, invitee_type: str, created_by_user_id: int, expires_in_days: int = 7
) -> tuple[InvitationLink, int, str]:
    ...
    _event(
        ...
        metadata={"invitee_type": invitee_type, "exp": exp_ts},
    )
