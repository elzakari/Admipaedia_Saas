...
class CompetencyService:
    ...
    def update_student_competency_profile(
        student_id: int, academic_year: str, updater_id: int
    ) -> StudentCompetencyProfile:
        ...
        profile.areas_for_improvement = CompetencyService._identify_improvement_areas(
            domain_scores
        )
    ...
    def _calculate_domain_scores(
        student_id: int, academic_year: str
        ...
        for domain in CompetencyDomain:
            ...
            competencies = (
                CoreCompetency.query.join(CompetencyIndicator)
                .filter(CompetencyIndicator.domain == domain)
                .distinct()
                .all()
            )
            ...
            if assessments:
                avg_score = sum(
                    assessment.level_achieved for assessment in assessments
                ) / len(assessments)
    ...
    def _recommend_activities(
        student_id: int, domain_scores: Dict[CompetencyDomain, float]
    ) -> List[Dict]:
        ...
        for domain, score in domain_scores.items():
            if score and score < 3.0:  # Below proficient
                activities = (
                    CompetencyLearningActivity.query.filter(
                        and_(
    ...
    @staticmethod
    def get_class_competency_overview(class_id: int, academic_year: str) -> Dict:
            ...
            for domain in CompetencyDomain:
                ...
                for profile in profiles:
                    if domain == CompetencyDomain.COMMUNICATION_COLLABORATION:
                        ...
                        domain == CompetencyDomain.CULTURAL_IDENTITY_GLOBAL_CITIZENSHIP
                    ):
                ...
                if scores:
                    class_overview["domain_averages"][domain_key] = sum(scores) / len(
                        scores
                    )
