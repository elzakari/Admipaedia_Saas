...
class AssignmentService:
    ...
    def get_attachment_map(entity_type, entity_ids):
        ...
        normalized_ids = [
            str(entity_id) for entity_id in entity_ids if entity_id is not None
        ]
    ...
    @staticmethod
    def _save_attachment_records(
        files, *, entity_type, entity_id, uploader_id=None, tenant_id=None
    ):
        saved_attachments = []
        for upload in files or []:
            ...
            file_path, error = FileUtils.upload_resource_file(
                upload, resource_id=f"{entity_type}_{entity_id}"
            )
            if error:
    ...
    def create_assignment(data, *, attachments=None, uploader_id=None, tenant_id=None):
        ...
        try:
            ...
            NotificationFanoutService.enqueue_class_fanout(
                ...
                or f"A new assignment for your subject has been posted. Due date: {assignment.due_date}",
