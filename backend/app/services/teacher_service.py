...
class TeacherService:
    ...
    def get_all_teachers(
        ...
        try:
            query = Teacher.query.options(
                joinedload(Teacher.user), joinedload(Teacher.subjects)
            )
            ...
            if specialization and specialization.strip():
                query = query.filter(
                    Teacher.specialization.ilike(f"%{specialization.strip()}%")
                )
            ...
            return query.order_by(Teacher.last_name).paginate(
                page=page, per_page=per_page, error_out=False
            )
        except Exception as e:
            logger.error(
                "Error in get_all_teachers",
                error=str(e),
    ...
    def create_teacher(data, tenant_id=None):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Failed to run TeacherProvisioningService during creation",
                    error=str(e),
                    user_id=new_teacher.user_id,
                )
    ...
    def update_teacher(teacher_id, teacher_data, tenant_id=None):
        ...
        try:
            ...
            if (
                "employee_id" in teacher_data
                and teacher_data["employee_id"] != teacher.employee_id
            ):
            ...
            try:
                ...
                logger.error(
                    "Failed to run TeacherProvisioningService during update",
                    error=str(e),
    ...
    def get_teacher_subjects(teacher_id):
        ...
        try:
            ...
            logger.error(
                "Error fetching teacher subjects", error=str(e), teacher_id=teacher_id
            )
            return None
        ...
        try:
            ...
            if status == "active":
                try:
                    ...
                    logger.error(
                        "Failed to run TeacherProvisioningService during status update",
                        error=str(e),
                        user_id=teacher.user_id,
                    )
            return teacher, None
            ...
            db.session.rollback()
            logger.error(
                "Error updating teacher status", error=str(e), teacher_id=teacher_id
            )
            return None, str(e)
    ...
    def get_teacher_attendance(
        teacher_id, page=1, per_page=20, start_date=None, end_date=None
    ):
        ...
        try:
            ...
            return (
                query.order_by(TeacherAttendance.date.desc()).paginate(
            ...
            logger.error(
                "Error fetching teacher attendance", error=str(e), teacher_id=teacher_id
            )
            return None, str(e)
    ...
    def mark_teacher_attendance(teacher_id, attendance_data):
        ...
        try:
            ...
            existing_attendance = TeacherAttendance.query.filter_by(
                teacher_id=teacher_id, date=attendance_data["date"]
            ).first()
            ...
            if existing_attendance:
                ...
                new_attendance = TeacherAttendance(
                    teacher_id=teacher_id, **attendance_data
                )
            ...
            db.session.rollback()
            logger.error(
                "Error marking teacher attendance", error=str(e), teacher_id=teacher_id
            )
            return None, str(e)
    ...
    def get_teacher_stats(teacher_id):
        ...
        try:
            ...
            total_students = (
                Student.query.filter(Student.class_id.in_(class_ids)).count()
                if class_ids
                else 0
            )
            ...
            logger.error(
                "Error fetching teacher stats", error=str(e), teacher_id=teacher_id
            )
            return None, str(e)
    ...
    def get_teacher_analytics(teacher_id):
        ...
        try:
            ...
            while current_date <= end_date:
                record = next(
                    (r for r in attendance_records if r.date == current_date), None
                )
            ...
            for c in classes:
                ...
                class_performance.append(
                    {"name": c.name, "performance": 85}  # Mock data
                )
            ...
            pending_assignments_stat = next(
                (s["value"] for s in stats if s["name"] == "Pending Assignments"), "0"
            )
            ...
            logger.error(
                "Error fetching teacher analytics", error=str(e), teacher_id=teacher_id
            )
            return None, str(e)
    ...
    def get_teacher_ai_insights(teacher_id):
        ...
        try:
            ...
            insights = {
                ...
                "trend_data": {"2024-01": 85, "2024-02": 87, "2024-03": 88},
                "performance_data": {
                    "factors": [
            ...
            logger.error(
                "Error generating teacher AI insights",
                error=str(e),
                teacher_id=teacher_id,
            )
            return None, str(e)
