...
class EnhancedExamService:
    ...
    def detect_exam_conflicts(
        ...
        try:
            ...
            time_conflict_query = Exam.query.filter(
                ...
                or_(
                    ...
                    and_(
                        ...
                        func.datetime(
                            Exam.exam_date,
                            "+" + func.cast(Exam.duration, db.String) + " minutes",
                        )
                        > exam_date,
                    ),
            ...
            same_day_start = exam_date.replace(
    ...
    def _suggest_alternative_dates(
        class_id: int, original_date: datetime, duration: int, days_range: int = 14
    ) -> List[datetime]:
    ...
    def get_exam_analytics(exam_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            percentages = [
                grade.percentage for grade in grades if grade.percentage is not None
            ]
            marks = [
            ...
            if percentages:
                statistics_data = {
                    ...
                    "mode": (
                        round(statistics.mode(percentages), 2)
                    ...
                    "pass_rate": round(
                        (len([p for p in percentages if p >= 50]) / len(percentages))
                        * 100,
                        2,
                    ),
                }
            ...
            # Grade distribution
            grade_distribution = EnhancedExamService._calculate_grade_distribution(
                percentages
            )
            ...
            return {
                ...
                "difficulty_analysis": EnhancedExamService._analyze_difficulty(
                    statistics_data
                ),
            }
    ...
    def _generate_performance_insights(
        stats: Dict, distribution: Dict, total_students: int
    ) -> List[str]:
        ...
        if stats["pass_rate"] >= 80:
            insights.append(
                "Excellent performance - High pass rate indicates good understanding"
            )
        ...
        if stats["std_dev"] > 20:
            insights.append(
                "High variation in performance - Consider differentiated instruction"
            )
        ...
        if distribution:
            a_grades = distribution.get("A+", {}).get("count", 0) + distribution.get(
                "A", {}
            ).get("count", 0)
            ...
            if a_grades > total_students * 0.3:
                insights.append(
                    "High number of A grades - Excellent preparation or easy exam"
                )

            if f_grades > total_students * 0.2:
                insights.append(
                    "High failure rate - Review teaching methods and exam difficulty"
                )
    ...
    def _analyze_difficulty(stats: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "reliability_indicator": (
                "High" if std_dev > 15 else "Moderate" if std_dev > 10 else "Low"
            ),
        }

    @staticmethod
    def get_class_exam_schedule(
        class_id: int, date_from: Optional[date] = None, date_to: Optional[date] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            workload_stats = {
                ...
                "max_exams_per_day": (
                    max([len(day_exams) for day_exams in schedule.values()])
                    if schedule
                    else 0
                ),
                "avg_exams_per_day": (
                    round(len(exams) / len(schedule), 1) if schedule else 0
                ),
            }
            ...
            return {
                ...
                "recommendations": EnhancedExamService._generate_schedule_recommendations(
                    workload_stats, conflicts
                ),
            }
    ...
    def _generate_schedule_recommendations(
        workload_stats: Dict, conflicts: List
    ) -> List[str]:
    ...
    @staticmethod
    def calculate_optimal_exam_duration(
        subject_id: int, total_marks: float, exam_type: str = "regular"
    ) -> Dict[str, Any]:
        """
