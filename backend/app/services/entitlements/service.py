...
class EntitlementService:
    ...
    def _build_features(schoolId, active: ActivePlan) -> Dict[str, bool]:
        ...
        overrides = SchoolFeatureOverride.query.filter_by(
            school_id=_as_uuid(schoolId)
        ).all()
        for o in overrides:
    ...
    def _build_limits(schoolId, active: ActivePlan) -> Dict[str, Any]:
        ...
        overrides = SchoolLimitOverride.query.filter_by(
            school_id=_as_uuid(schoolId)
        ).all()
        for o in overrides:
    ...
    def getSchoolActivePlan(schoolId) -> Tuple[Optional[ActivePlan], Optional[str]]:
        ...
        today = date.today()
        q = SchoolPlanSubscription.query.options(
            joinedload(SchoolPlanSubscription.plan)
        ).filter_by(school_id=sid, status="active")
        q = q.filter(SchoolPlanSubscription.starts_at <= today)
        q = q.filter(
            (SchoolPlanSubscription.ends_at.is_(None))
            | (SchoolPlanSubscription.ends_at >= today)
        )
    ...
    def enforceLimit(schoolId, limitKey: str, currentUsage: float):
        ...
        if currentUsage > limit_num:
            raise EntitlementError(
                f"This action exceeds your plan limit for {limitKey}."
            )
    ...
    def count_active_school_admins(schoolId) -> int:
        ...
        return TenantMembership.query.filter_by(
            tenant_id=sid, role="school_admin", status="active"
        ).count()
    ...
    def count_active_teachers(schoolId) -> int:
        sid = _as_uuid(schoolId)
        return TenantMembership.query.filter_by(
            tenant_id=sid, role="teacher", status="active"
        ).count()

    @staticmethod
    def count_active_registered_students_for_term(
        schoolId, academic_term_id: int
    ) -> int:
        sid = _as_uuid(schoolId)
        q = StudentTermRegistration.query.filter_by(
            tenant_id=sid, academic_term_id=int(academic_term_id)
        )
    ...
    def count_messages_this_month(schoolId) -> int:
        ...
        return Message.query.filter(
            and_(Message.tenant_id == sid, Message.created_at >= start)
        ).count()
