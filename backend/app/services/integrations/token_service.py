...
class ServiceTokenService:
    @staticmethod
    def _desired_allowance_for_tenant(
        tenant_id: str, service_type: str
    ...
    def provision_for_tenant(
        tenant_id: str, actor_user_id: Optional[int] = None
        ...
        for service_type in SERVICE_TYPES:
            allowance, unlimited, plan_id = (
                ServiceTokenService._desired_allowance_for_tenant(
                    str(tid_uuid), service_type
                )
            )
            token = TenantServiceToken.query.filter_by(
                tenant_id=tid_db, service_type=service_type
            ).first()
            ...
            if not token:
                ...
                token = TenantServiceToken(
                    ...
                    monthly_allowance=(
                        "unlimited" if unlimited else str(int(allowance or 0))
                    ),
                )
    ...
    @staticmethod
    def rotate_token(
        tenant_id: str, service_type: str, actor_user_id: Optional[int] = None
    ) -> str:
        ...
        if not token:
            ServiceTokenService.provision_for_tenant(
                str(tid_uuid), actor_user_id=actor_user_id
            )
            token = TenantServiceToken.query.filter_by(
                tenant_id=tid_db, service_type=service_type
            ).first()
    ...
    @staticmethod
    def _usage_row(
        tenant_id: str, service_type: str, year: int, month: int
    ) -> TenantServiceTokenUsage:
        tid_db = _as_db_tenant_id(tenant_id)
        row = (
    ...
    def get_status(tenant_id: str, service_type: str) -> TokenStatus:
        ...
        usage = TenantServiceTokenUsage.query.filter_by(
            tenant_id=tid_db, service_type=service_type, year=year, month=month
        ).first()
    ...
    @staticmethod
    def get_status_map(
        tenant_id: str, service_types: tuple[str, ...] = SERVICE_TYPES
    ) -> dict[str, TokenStatus]:
        tid_uuid = _as_uuid(tenant_id)
        tid_db = _as_db_tenant_id(tenant_id)
        year, month = current_period_ym()

        tokens = TenantServiceToken.query.filter(
            TenantServiceToken.tenant_id == tid_db,
            TenantServiceToken.service_type.in_(service_types),
        ).all()
        usage_rows = TenantServiceTokenUsage.query.filter(
        ...
        usage_map = {
            str(row.service_type): int(getattr(row, "used_count", 0) or 0)
    ...
    @staticmethod
    def validate_token(
        plain_token: str, service_type: str, tenant_id_hint: Optional[str] = None
    ) -> tuple[Optional[TenantServiceToken], Optional[str]]:
        if service_type not in SERVICE_TYPES:
        ...
        q = TenantServiceToken.query.filter_by(
            token_hash=h, service_type=service_type, status="active"
        )
        token = q.first()
        if not token:
            ServiceTokenService._event(
    ...
    ) -> tuple[Optional[TokenStatus], Optional[str], int]:
        if amount <= 0:
            amount = 1

        token, err = ServiceTokenService.validate_token(
            plain_token, service_type, tenant_id_hint=tenant_id_hint
        )
        if err or not token:
            db.session.commit()
