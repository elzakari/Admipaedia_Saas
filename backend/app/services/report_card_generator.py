...
class ReportCardGenerator:
    ...
    def generate_report_card_pdf(student_id: int, academic_cycle_id: str) -> io.BytesIO:
        ...
        if not student:
            raise ValueError(
                "Student not found or unauthorized access in active branch context."
            )

        clazz = Class.query_scoped().filter_by(id=student.class_id).first()
        if not clazz:
            raise ValueError(
                "Class not found or unauthorized access in active branch context."
            )
        ...
        cycle = AcademicCycle.query.filter_by(
            id=resolved_cycle_id, tenant_id=student.tenant_id
        ).first()
        if not cycle:
        ...
        if grade_level and grade_level.track_id:
            scale = PolymorphicGradingScale.query.filter_by(
                tenant_id=student.tenant_id, track_id=grade_level.track_id
            ).first()
        ...
        val_style = ParagraphStyle(
            "Val", parent=styles["Normal"], fontSize=9, fontName="Helvetica"
        )
        ...
        tb_style = ParagraphStyle(
            "TB", parent=styles["Normal"], fontSize=8.5, fontName="Helvetica"
        )
        ...
        info_data = [
            [
                ...
                Paragraph(f"{student.first_name} {student.last_name}", val_style),
                Paragraph("Admission Number:", label_style),
                Paragraph(student.admission_number, val_style),
            ],
            [
                Paragraph("Classroom:", label_style),
                Paragraph(clazz.name, val_style),
                Paragraph("Academic Cycle:", label_style),
                Paragraph(cycle.name, val_style),
            ],
        ]
        info_table = Table(
            info_data, colWidths=[1.5 * inch, 2.0 * inch, 1.5 * inch, 2.0 * inch]
        )
        info_table.setStyle(
            TableStyle(
                [
        ...
        for subject_id, scores in subject_scores.items():
            ...
            c_score = (
                sum(scores["class_scores"]) / len(scores["class_scores"])
                if scores["class_scores"]
                else 0.0
            )
            e_score = (
                sum(scores["exam_scores"]) / len(scores["exam_scores"])
                if scores["exam_scores"]
                else 0.0
            )

            # Weighted precision scoring via grading engine
            res = PolymorphicGradingEngine.calculate_final_score(
                clazz.id, c_score, e_score
            )

            status_text = "PASS" if res["passing"] else "FAIL"
            status_color = (
                colors.HexColor("#138a7c")
                if res["passing"]
            ...
            grades_table_data.append(
                [
                    Paragraph(
                        sub_name,
                        ParagraphStyle(
                            "SN", parent=tb_style, fontName="Helvetica-Bold"
                        ),
                    ),
                    Paragraph(
                        f"{c_score:.1f}",
                        ParagraphStyle("C", parent=tb_style, alignment=TA_CENTER),
                    ),
                    Paragraph(
                        f"{e_score:.1f}",
                        ParagraphStyle("E", parent=tb_style, alignment=TA_CENTER),
                    ),
                    Paragraph(
        ...
        for scheme in scale.schemes or []:
            ...
            legend_rows.append(
                [
                    Paragraph(
                        f"{s_min:.2f}% - {s_max:.2f}%",
                        ParagraphStyle("L", parent=tb_style, alignment=TA_CENTER),
                    ),
                    Paragraph(
                        g_mark,
                        ParagraphStyle(
        ...
        legend_table = Table(
            legend_rows, colWidths=[2.5 * inch, 2.0 * inch, 3.5 * inch]
        )
        ...
        sig_data = [
            [
                Paragraph(
                    "________________________<br/>Class Teacher",
                    ParagraphStyle("S", alignment=TA_CENTER, fontSize=8.5),
                ),
                Paragraph(
                    "________________________<br/>Principal / Headmaster",
                    ParagraphStyle("S", alignment=TA_CENTER, fontSize=8.5),
                ),
            ]
        ]
