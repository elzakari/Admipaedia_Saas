...
def send_via_resend_api(recipient, subject, html_content):
    ...
    try:
        ...
        logger.info(
            f"🚀 Email successfully sent to {recipient} via Resend API fallback. ID: {msg_id}"
        )
...
def send_transactional_email(recipient, subject, html_content):
    ...
    if provider == "aws_ses":
        try:
            ...
            logger.warning(
                f"⚠️ Primary AWS SES SMTP failed: {ses_error}. Activating Resend fallback..."
            )
        ...
        try:
            ...
            with smtplib.SMTP(
                ...
                server.send_message(msg)
            logger.info(
                f"🚀 Email successfully sent to {recipient} via {provider} SMTP."
            )
