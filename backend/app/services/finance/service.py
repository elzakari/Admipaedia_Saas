...
class FeeService:
    ...
    def student_requires_manual_fee_assignment(student):
        ...
        scholarship_details = str(
            getattr(student, "scholarship_details", "") or ""
        ).strip()
        scholarship_markers = ("scholarship", "bursary", "waiver", "sponsored", "grant")
        return any(marker in fee_category for marker in scholarship_markers) or bool(
            scholarship_details
        )
    ...
    def auto_apply_current_fee_templates(student, commit=True):
        if (
            not student
            or not getattr(student, "id", None)
            or not getattr(student, "class_id", None)
        ):
            return 0
        ...
        return FeeService.assign_fee_structures_to_students(
            structures, [student], commit=commit
        )
