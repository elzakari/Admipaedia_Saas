...
class AnalyticsService:
    ...
    def get_teacher_dashboard_analytics(teacher_id):
        ...
        try:
            ...
            assignment_stats = AnalyticsService._get_assignment_analytics(
    ...
    def _get_attendance_analytics(class_ids):
        ...
        try:
            ...
            present_count = sum(
                count for status, count in attendance_by_status if status == "present"
            )
            ...
            attendance_trend = (
                ...
                .group_by(func.date(Attendance.date), Attendance.status)
                .order_by(func.date(Attendance.date))
                .all()
            )
    ...
    def _get_performance_analytics(class_ids):
        ...
        try:
            # Get average grades
            avg_grade = (
                db.session.query(func.avg(Grade.percentage).label("avg_percentage"))
                .filter(Grade.class_id.in_(class_ids))
                .scalar()
                or 0
            )
            ...
            grade_distribution = (
                db.session.query(
                    Grade.grade_letter, func.count(Grade.id).label("count")
                )
                .filter(Grade.class_id.in_(class_ids))
            ...
            performance_trend = (
                ...
                .filter(Grade.class_id.in_(class_ids))
                .group_by(Grade.term, Grade.assessment_type)
                .all()
            )
    ...
    def _get_assignment_analytics(class_ids, teacher_id):
        ...
        try:
            ...
            total_assignments = Assignment.query.filter(
                Assignment.class_id.in_(class_ids), Assignment.teacher_id == teacher_id
            ).count()
    ...
    def _get_at_risk_students(class_ids):
        ...
        try:
            ...
            students_with_grades = (
                db.session.query(
                    Grade.student_id, func.avg(Grade.percentage).label("avg_grade")
                )
                .filter(Grade.class_id.in_(class_ids))
            ...
            for student_id, attendance_rate in attendance_risk_students:
                ...
                at_risk_map[student_id]["risk_factors"].append(
                    f"Low attendance ({round(attendance_rate)}%)"
                )
