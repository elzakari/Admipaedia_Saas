...
from app.models.enhanced_auth import (AuthenticationAttempt, MFADevice,
                                      SecurityAuditLog, TrustedDevice,
                                      UserSecuritySettings)
from app.models.grading_system import FinalGrade, GradingScheme
from app.models.parent import Parent
from app.models.security import (APIKey, PasswordHistory, PasswordResetToken,
                                 SchoolRegistrationToken, SecurityEvent)
...
from app.models.tenant import (PlatformInvoice, PlatformPayment, Subscription,
                               Tenant, TenantInvitation, TenantMembership)
from app.models.tenant_academic_settings import TenantAcademicSettings
from app.models.user import LoginHistory, User, user_roles
...
class OrphanCleanupService:
    ...
    def _tenant_predicate(table_name: str, columns: list[str], tenant_uuid: uuid.UUID):
        ...
        predicates = [
            table.c[col]
            == OrphanCleanupService._tenant_value_for_column(table.c[col], tenant_uuid)
            for col in columns
            if col in table.c
        ]
        if not predicates:
    ...
    def _tenant_scoped_predicates(tenant_uuid: uuid.UUID):
        ...
        for table_name, columns in tenant_refs.items():
            predicate = OrphanCleanupService._tenant_predicate(
                table_name, sorted(columns), tenant_uuid
            )
        ...
        while changed:
            ...
            for table_name, table in metadata_tables.items():
                ...
                for fk_constraint in table.foreign_key_constraints:
                    referred_table = getattr(
                        getattr(fk_constraint, "referred_table", None), "name", None
                    )
                    ...
                    predicates.append(
                        child_column.in_(
                            select(parent_column).where(
                                scoped_predicates[referred_table]
                            )
                        )
                    )
    ...
    def _tenant_ref_clause(columns: list[str], tenant_uuid: uuid.UUID):
        ...
        for index, col in enumerate(columns):
            ...
            predicates.append(
                f"{OrphanCleanupService._quote_ident(col)} = :{param_name}"
            )
        ...
        for t, columns in tenant_refs.items():
            ...
            try:
                ...
                if predicate is not None:
                    ...
                    where_clause, params = OrphanCleanupService._tenant_ref_clause(
                        ordered_columns, tenant_uuid
                    )
    ...
    def _user_fk_references_db(user_id: int):
        ...
        for table in table_names:
            ...
            try:
                ...
                cols = []
            col_nullable = {
                c.get("name"): bool(c.get("nullable", True))
                for c in cols
                if c.get("name")
            }
            ...
            for fk in fks:
                ...
                for col in fk.get("constrained_columns") or []:
                    ...
                    try:
                        ...
                        cnt = db.session.execute(
                            text(
                                f"SELECT COUNT(*) FROM {q_table} WHERE {q_col} = :uid"
                            ),
                            {"uid": user_id},
                        ).scalar()
                        cnt = int(cnt or 0)
    ...
    def _user_fk_references(user_id: int):
        blocking, detachable, unknown = OrphanCleanupService._user_fk_references_db(
            user_id
        )
        ...
        for cls in OrphanCleanupService._iter_mapped_models():
            ...
            for col in table.columns:
                if not col.foreign_keys:
                    continue
                if not any(
                    getattr(fk.column, "table", None) is not None
    ...
    def get_orphan_tenant_status(tenant_id: str) -> Dict[str, Any]:
        ...
        for key, fn in [
            (
                "memberships",
                lambda: TenantMembership.query.filter_by(tenant_id=tid).count(),
            ),
            (
                "invitations",
                lambda: TenantInvitation.query.filter_by(tenant_id=tid).count(),
            ),
            (
                "subscriptions",
                lambda: Subscription.query.filter_by(tenant_id=tid).count(),
            ),
            (
                "platform_invoices",
                lambda: PlatformInvoice.query.filter_by(tenant_id=tid).count(),
            ),
            (
                "platform_payments",
                lambda: PlatformPayment.query.filter_by(tenant_id=tid).count(),
            ),
            ...
            (
                "grading_schemes",
                lambda: GradingScheme.query.filter_by(tenant_id=tid).count(),
            ),
            (
                "educational_system_config",
                lambda: EducationalSystemConfig.query.filter_by(tenant_id=tid).count(),
            ),
            ("grade_levels", lambda: GradeLevel.query.filter_by(tenant_id=tid).count()),
            (
                "academic_terms",
                lambda: AcademicTerm.query.filter_by(tenant_id=tid).count(),
            ),
            (
                "tenant_academic_settings",
                lambda: TenantAcademicSettings.query.filter_by(tenant_id=tid).count(),
            ),
        ]:
        ...
        if count_errors:
            reasons.append(
                "Unable to verify all tenant references due to schema mismatch"
            )
    ...
    def delete_orphan_tenant(tenant_id: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        for key, fn in [
            (
                "platform_payments",
                lambda: PlatformPayment.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "platform_invoices",
                lambda: PlatformInvoice.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "subscriptions",
                lambda: Subscription.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "tenant_invitations",
                lambda: TenantInvitation.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "tenant_memberships",
                lambda: TenantMembership.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "grading_schemes",
                lambda: GradingScheme.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "tenant_academic_settings",
                lambda: TenantAcademicSettings.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "academic_terms",
                lambda: AcademicTerm.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "grade_levels",
                lambda: GradeLevel.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "educational_system_config",
                lambda: EducationalSystemConfig.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "subjects",
                lambda: Subject.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "departments",
                lambda: Department.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "classes",
                lambda: Class.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "staff",
                lambda: Staff.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "parents",
                lambda: Parent.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "teachers",
                lambda: Teacher.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
            (
                "students",
                lambda: Student.query.filter_by(tenant_id=tid).delete(
                    synchronize_session=False
                ),
            ),
        ]:
            try:
                ...
                return False, {
                    ...
                    "can_delete": False,
                    "reasons": [
                        "Failed to delete tenant due to a database/schema mismatch"
                    ],
                    "failed_on": key,
    ...
    def get_tenant_purge_status(tenant_id: str) -> Dict[str, Any]:
        ...
        ok, student_count = OrphanCleanupService._safe_count(
            lambda: Student.query.filter_by(tenant_id=tid).count()
        )
    ...
    def purge_tenant(tenant_id: str, actor_user_id: int) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            current_table = None
            tenant_scoped_predicates = OrphanCleanupService._tenant_scoped_predicates(
                tid
            )
            tenant_refs = OrphanCleanupService._tenant_reference_columns()
            if not ordered_tables:
                ordered_tables = list(
                    tenant_scoped_predicates.keys() or tenant_refs.keys()
                )
            if tenant_scoped_predicates:
                ordered_tables = OrphanCleanupService._tenant_tables_order(
                    tenant_scoped_predicates.keys()
                )
    ...
    def get_orphan_user_status(user_id: int) -> Dict[str, Any]:
        ...
        try:
            blocking_fk, detachable_fk, unknown_fk = (
                OrphanCleanupService._user_fk_references(user_id)
            )
            count_errors = []
            ...
            for key, fn in [
                (
                    "memberships",
                    lambda: TenantMembership.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "invited_memberships",
                    lambda: TenantMembership.query.filter_by(
                ...
                (
                    "session_tokens",
                    lambda: SessionToken.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "login_history",
                    lambda: LoginHistory.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "password_history",
                    lambda: PasswordHistory.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "password_reset_tokens",
                    lambda: PasswordResetToken.query.filter_by(user_id=user_id).count(),
                ),
                ("api_keys", lambda: APIKey.query.filter_by(user_id=user_id).count()),
                (
                    "notifications",
                    lambda: Notification.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "user_profile",
                    lambda: UserProfile.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "user_preferences",
                    lambda: UserPreferences.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "mfa_devices",
                    lambda: MFADevice.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "trusted_devices",
                    lambda: TrustedDevice.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "auth_attempts",
                    lambda: AuthenticationAttempt.query.filter_by(
                        user_id=user_id
                    ).count(),
                ),
                (
                    "user_security_settings",
                    lambda: UserSecuritySettings.query.filter_by(
                        user_id=user_id
                    ).count(),
                ),
                (
                    "security_audit_logs",
                    lambda: SecurityAuditLog.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "staff_leaves",
                    lambda: StaffLeave.query.filter_by(user_id=user_id).count(),
                ),
                (
                    "final_grades_computed",
                    lambda: FinalGrade.query.filter_by(computed_by=user_id).count(),
                ),
                (
                    "registration_tokens_created",
                    lambda: SchoolRegistrationToken.query.filter_by(
            ...
            if (
                counts.get("final_grades_computed")
                and counts["final_grades_computed"] > 0
            ):
                reasons.append("User is referenced by computed grades")
            if (
                counts.get("registration_tokens_created")
                and counts["registration_tokens_created"] > 0
            ):
            ...
            if unknown_fk or count_errors:
                reasons.append(
                    "Unable to verify all user references due to schema mismatch"
                )
    ...
    @staticmethod
    def delete_orphan_user(
        user_id: int, actor_user_id: int
        ...
        ok, result = SecurePurgeService.force_purge_user(
    ...
    def purge_user(user_id: int, actor_user_id: int) -> Tuple[bool, Dict[str, Any]]:
        ...
        if user.role == "super_admin":
            return False, {
                ...
                "user_id": user_id,
                "can_delete": False,
                "reasons": ["Cannot delete a super admin account"],
            }
        ...
        try:
            ...
            return False, {
                ...
                "user_id": user_id,
                "can_delete": False,
                "reasons": ["Unable to inspect database"],
                "error": type(e).__name__,
            }
        ...
        for ref in fk_refs:
            ...
            try:
                if table in delete_row_tables:
                    db.session.execute(
                        text(f"DELETE FROM {q_table} WHERE {q_col} = :uid"),
                        {"uid": user_id},
                    )
                elif nullable:
                    db.session.execute(
                        text(
                            f"UPDATE {q_table} SET {q_col} = NULL WHERE {q_col} = :uid"
                        ),
                        {"uid": user_id},
                    )
                else:
                    db.session.execute(
                        text(
                            f"UPDATE {q_table} SET {q_col} = :actor_uid WHERE {q_col} = :uid"
                        ),
                        {"uid": user_id, "actor_uid": actor_user_id},
                    )
            except Exception as e:
                db.session.rollback()
                return False, {
        ...
        try:
            ...
            db.session.execute(
                text("DELETE FROM students WHERE user_id = :uid"), {"uid": user_id}
            )
            ...
            db.session.rollback()
            blocking_after, detachable_after, unknown_after = (
                OrphanCleanupService._user_fk_references(user_id)
            )
            return False, {
                "exists": True,
    ...
    def anonymize_user(user_id: int, actor_user_id: int) -> Tuple[bool, Dict[str, Any]]:
        if user_id == actor_user_id:
            return False, {
                ...
                "user_id": user_id,
                "can_anonymize": False,
                "reasons": ["Cannot anonymize your own account"],
            }
        ...
        if user.role == "super_admin":
            return False, {
                ...
                "user_id": user_id,
                "can_anonymize": False,
                "reasons": ["Cannot anonymize a super admin account"],
            }
        ...
        AuthenticationAttempt.query.filter_by(user_id=user.id).delete(
            synchronize_session=False
        )
        UserSecuritySettings.query.filter_by(user_id=user.id).delete(
            synchronize_session=False
        )
