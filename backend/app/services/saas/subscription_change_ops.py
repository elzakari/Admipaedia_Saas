...
def serialize_change_request(r: SubscriptionChangeRequest) -> dict:
    ...
    effective_term = (
        AcademicTerm.query.get(int(r.effective_academic_term_id))
        if r.effective_academic_term_id
        else None
    )
    current_plan, _ = (
        _get_current_plan_for_tenant(r.school_id) if r.school_id else (None, None)
    )
    created_by_user = (
        User.query.get(int(r.created_by_user_id)) if r.created_by_user_id else None
    )
    approved_by_user = (
        User.query.get(int(r.approved_by_user_id)) if r.approved_by_user_id else None
    )
    return {
        "id": int(r.id),
        ...
        "status": r.status,
        "effective_academic_term_id": (
            int(r.effective_academic_term_id) if r.effective_academic_term_id else None
        ),
        ...
        "requested_plan": (
            serialize_plan(r.requested_plan) if r.requested_plan else None
        ),
...
def list_active_plans() -> list[Plan]:
    return (
        ...
        .order_by(Plan.price_per_student.asc(), Plan.id.asc())
        .all()
    )
...
def _get_current_plan_for_tenant(tenant_id) -> tuple[Optional[Plan], Optional[str]]:
    ...
    latest_subscription = (
        ...
        .order_by(
            SchoolPlanSubscription.starts_at.desc(), SchoolPlanSubscription.id.desc()
        )
        .first()
    )
...
def _plan_order_map() -> dict[str, int]:
    ...
    return {
        str(plan.slug or "").strip().lower(): index for index, plan in enumerate(plans)
    }
...
def create_upgrade(
    *,
    ...
    plan = Plan.query.filter_by(
        slug=(plan_slug or "").strip().lower(), is_active=True
    ).first()
    if not plan:
    ...
    if (
        current_slug in plan_order
        and target_slug in plan_order
        and plan_order[target_slug] <= plan_order[current_slug]
    ):
        return None, "Use downgrade request for lower or equal plans"

    existing_pending = SubscriptionChangeRequest.query.filter(
...
def request_downgrade(
    *,
    ...
    term = AcademicTerm.query.filter_by(
        id=int(effective_academic_term_id), tenant_id=tenant.id
    ).first()
    ...
    pending = SubscriptionChangeRequest.query.filter_by(
        school_id=tenant.id, status="pending", request_type="downgrade"
    ).first()
    if pending:
...
def list_school_requests(tenant_id) -> list[SubscriptionChangeRequest]:
    return (
        SubscriptionChangeRequest.query.filter_by(school_id=tenant_id)
...
def approve_request(
    *, request_id: int, reviewer_user_id: int
) -> Tuple[Optional[SubscriptionChangeRequest], Optional[str]]:
    req = (
        SubscriptionChangeRequest.query.filter_by(id=int(request_id))
        .with_for_update()
        .first()
    )
    if not req:
    ...
    term = (
        AcademicTerm.query.filter_by(
            id=int(req.effective_academic_term_id), tenant_id=tenant.id
        ).first()
        if req.effective_academic_term_id
        else None
    )
    if not term or not term.start_date:
...
def reject_request(
    *, request_id: int, reviewer_user_id: int, note: Optional[str]
) -> Tuple[Optional[SubscriptionChangeRequest], Optional[str]]:
    req = (
        SubscriptionChangeRequest.query.filter_by(id=int(request_id))
        .with_for_update()
