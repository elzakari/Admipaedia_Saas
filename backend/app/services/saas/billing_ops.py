...
from app.models.billing import BillingInvoice, Plan, SchoolPlanSubscription
from app.models.payments import Payment as BillingPayment
from app.models.tenant import (PlatformInvoice, PlatformPayment, Tenant,
                               TenantMembership)
...
def list_invoices(user_id: int, tenant_id):
    ...
    invoices = (
        PlatformInvoice.query.filter_by(tenant_id=tenant.id)
...
def record_payment(
    user_id: int, tenant_id, invoice_id, amount, method: str, reference: str, paid_on
):
    tenant, membership, err = get_tenant_for_user(user_id, tenant_id)
    if err:
        return None, err
    ...
    if invoice_id:
        try:
            invoice_uuid = (
                invoice_id
                if isinstance(invoice_id, uuid.UUID)
                else uuid.UUID(str(invoice_id))
            )
...
def list_payments(user_id: int, tenant_id):
    ...
    if not membership:
        return None, "Unauthorized"
    payments = (
        PlatformPayment.query.filter_by(tenant_id=tenant.id)
...
def platform_get_tenant_detail(tenant_id):
    ...
    invoices = (
        PlatformInvoice.query.options(joinedload(PlatformInvoice.tenant))
    ...
    payments = (
        PlatformPayment.query.options(joinedload(PlatformPayment.tenant))
        .filter_by(tenant_id=tenant.id)
        .order_by(PlatformPayment.created_at.desc())
        .limit(10)
        .all()
    ...
    legacy_invoice_total = (
        db.session.query(func.coalesce(func.sum(PlatformInvoice.amount), 0))
        .filter(PlatformInvoice.tenant_id == tenant.id)
        .scalar()
        or 0
    )
    legacy_payment_total = (
        db.session.query(func.coalesce(func.sum(PlatformPayment.amount), 0))
        .filter(PlatformPayment.tenant_id == tenant.id)
        .scalar()
        or 0
    )
    school_billing_invoice_total = (
        db.session.query(func.coalesce(func.sum(BillingInvoice.total_amount), 0))
        .filter(BillingInvoice.tenant_id == tenant.id)
        .scalar()
        or 0
    )
    school_billing_payment_total = (
        ...
        .filter(
            BillingPayment.school_id == tenant.id, BillingPayment.status == "successful"
        )
        .scalar()
        or 0
    )
    ...
    return {
        ...
        "recent_invoices": [
            {**serialize_invoice(i), "tenant_name": i.tenant.name if i.tenant else None}
            for i in invoices
        ],
        "recent_payments": [
            {**serialize_payment(p), "tenant_name": p.tenant.name if p.tenant else None}
            for p in payments
        ],
    }, None
...
def platform_kpis():
    ...
    try:
        platform_invoice_total = float(
            db.session.query(
                func.coalesce(func.sum(PlatformInvoice.amount), 0)
            ).scalar()
            or 0
        )
        platform_payment_total = float(
            db.session.query(
                func.coalesce(func.sum(PlatformPayment.amount), 0)
            ).scalar()
            or 0
        )
        invoices_count = PlatformInvoice.query.count()
        paid_invoices = PlatformInvoice.query.filter(
            PlatformInvoice.status == "paid"
        ).count()
        sent_invoices = PlatformInvoice.query.filter(
    ...
    try:
        ...
        logger.error(
            f"Error fetching school-fee billing invoice/payment metrics: {str(e)}"
        )
...
def platform_list_payments(
    ...
    query = PlatformPayment.query.options(
        joinedload(PlatformPayment.tenant), joinedload(PlatformPayment.invoice)
    )
    ...
    if q:
        ...
        query = query.filter(
            or_(
                PlatformPayment.reference.ilike(term),
                PlatformPayment.method.ilike(term),
            )
        )
...
def platform_financial_summary():
    ...
    try:
        billing_inv_by_tenant = (
            db.session.query(
                BillingInvoice.tenant_id, func.sum(BillingInvoice.total_amount)
            )
        ...
        "by_tenant": [
            {
                **item,
                "outstanding_total": max(
                    0.0,
                    float(item.get("invoice_total") or 0)
                    - float(item.get("payment_total") or 0),
                ),
            }
