...
from app.models.tenant import (TENANT_MEMBER_ROLES, Tenant, TenantInvitation,
                               TenantMembership)
...
def _create_tenant_and_membership(
    user: User, name: str, slug: str, country_code: str, currency: str = "USD"
):
    ...
    membership = TenantMembership(
        tenant_id=tenant.id, user_id=user.id, role="school_admin", status="active"
    )
...
def create_tenant(
    creator_user_id: int, name: str, slug: str, country_code: str, currency: str = "USD"
):
    user = User.query.get(int(creator_user_id))
    if not user:
        return None, "User not found"

    tenant, _, err = _create_tenant_and_membership(
        user, name, slug, country_code, currency
    )
    if err:
        return None, err
...
def get_user_tenants(user_id: int):
    rows = (
        ...
        .order_by(
            TenantMembership.status.asc(), Tenant.name.asc(), Tenant.created_at.desc()
        )
        .all()
    )
...
def update_tenant(
    user_id: int, tenant_id, updates: dict, allow_platform_admin: bool = False
):
...
def _ensure_not_last_active_school_admin(
    tenant_id,
    membership_to_remove_id=None,
    membership_new_role=None,
    membership_new_status=None,
):
...
def platform_upsert_member(tenant_id, email: str, role: str, status: str = "active"):
    ...
    if membership:
        ...
        membership = TenantMembership(
            tenant_id=tenant.id, user_id=user.id, role=role, status=status
        )
        db.session.add(membership)
...
def platform_update_membership(tenant_id, membership_id, updates: dict):
    try:
        ...
        membership_uuid = (
            membership_id
            if isinstance(membership_id, uuid.UUID)
            else uuid.UUID(str(membership_id))
        )
    except Exception:
    ...
    if not _ensure_not_last_active_school_admin(
        tenant.id,
        membership_to_remove_id=membership.id,
        membership_new_role=next_role,
        membership_new_status=next_status,
    ):
        return None, "Tenant must have at least one active school admin"
...
def platform_delete_membership(tenant_id, membership_id):
    ...
    if not _ensure_not_last_active_school_admin(
        tenant.id, membership_to_remove_id=membership.id
    ):
