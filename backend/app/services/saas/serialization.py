...
def serialize_tenant(t: Tenant):
    ...
    if isinstance(settings, dict):
        logo_url = (
            ...
            or settings.get("logo")
            or settings.get("school_logo")
        )
    return {
        "id": str(t.id),
        "slug": t.slug,
        ...
        "plan": t.plan,
        "plan_expires_at": (
            t.plan_expires_at.isoformat()
            if getattr(t, "plan_expires_at", None)
            else None
        ),
        "trial_ends_at": (
            t.trial_ends_at.isoformat() if getattr(t, "trial_ends_at", None) else None
        ),
        "status": t.status,
        "currency": t.currency,
