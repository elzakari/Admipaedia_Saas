...
from app.models.billing import (BillingInvoice, PendingInvoiceAdjustment, Plan,
...
class PaymentService:
    ...
    def _months_for_term(tenant_id, academic_term_id: int, *, min_months: int) -> int:
        ...
        term = AcademicTerm.query.filter_by(
            id=int(academic_term_id), tenant_id=tenant_id
        ).first()
    ...
    def serialize_payment(p: Payment) -> dict[str, Any]:
        ...
        return {
            ...
            "payment_gateway_id": (
                int(p.payment_gateway_id) if p.payment_gateway_id is not None else None
            ),
    ...
    @staticmethod
    def get_best_gateway_for_school(
        school_id,
    ...
    def list_gateways() -> list[PaymentGateway]:
        return PaymentGateway.query.order_by(
            PaymentGateway.name.asc(), PaymentGateway.country_code.asc().nullsfirst()
        ).all()
    ...
    def upsert_gateway(
        *,
        ...
        gw = (
            PaymentGateway.query.get(int(gateway_id))
            if gateway_id
            else PaymentGateway(name=key)
        )
        if not gw:
        ...
        if cc is None:
            duplicate_query = duplicate_query.filter(
                PaymentGateway.country_code.is_(None)
            )
        else:
        ...
        if duplicate_query.first():
            return (
                None,
                "A gateway already exists for this name, country, currency, and environment",
            )
        ...
        if gw.is_default:
            q = PaymentGateway.query.filter(
                PaymentGateway.id != gw.id, PaymentGateway.name == gw.name
            )
    ...
    def generate_invoice_for_school_term(
        *,
        ...
        if (err or not active) and not plan_override:
            ...
            sub = (
                SchoolPlanSubscription.query.filter_by(school_id=tenant.id)
        ...
        count = EntitlementService.count_active_registered_students_for_term(
            str(tenant.id), int(academic_term_id)
        )
        ...
        settings_rec = TenantAcademicSettings.query.filter_by(
            tenant_id=tenant.id
        ).first()
        if settings_rec and isinstance(settings_rec.settings, dict):
            settings = settings_rec.settings
            peak_keys = (
                "billing_peak_students",
                "peak_student_count",
                "historical_peak_active_students",
            )
            avg_keys = (
                "billing_average_students",
                "average_student_count",
                "running_average_active_students",
            )
        ...
        if months is None:
            months_to_bill = PaymentService._months_for_term(
                tenant.id, int(academic_term_id), min_months=min_months
            )
        else:
        ...
        adjustments = PendingInvoiceAdjustment.query.filter_by(
            tenant_id=tenant.id, status="pending"
        ).all()
        adjustment_total = sum(
            float(adj.amount) for adj in adjustments if adj.currency == currency
        )
        ...
        invoice = BillingInvoice.query.filter_by(
            tenant_id=tenant.id, academic_term_id=int(academic_term_id)
        ).first()
        if invoice:
            if (
                plan_override
                and float(invoice.amount_paid or 0) > 0
                and int(invoice.plan_id) != int(target_plan.id)
            ):
                return (
                    None,
                    "Existing invoice already has payments recorded for this term",
                )
    ...
    @staticmethod
    def _activate_paid_invoice_plan(
        inv: BillingInvoice, *, actor_user_id: Optional[int] = None
    ):
        ...
        upgrade_request = (
            SubscriptionChangeRequest.query.filter(
        ...
        if upgrade_request:
            ...
            upgrade_request.decision_note = (
                "Activated after invoice payment confirmation"
            )

    @staticmethod
    def list_invoices_for_tenant(tenant_id) -> list[BillingInvoice]:
        return (
            BillingInvoice.query.filter_by(tenant_id=tenant_id)
    ...
    def list_payments_for_tenant(tenant_id) -> list[Payment]:
        return (
            Payment.query.filter_by(school_id=tenant_id)
    ...
    def initializeInvoicePayment(
        *,
        ...
        if (
            str(inv.payment_status or "").lower() == "paid"
            or float(inv.balance_due or 0) <= 0
        ):
    ...
    def apply_payment_success(
        ...
        *,
        ...
        verified_currency: Optional[str],
    ):
        inv = (
            BillingInvoice.query.filter_by(id=int(payment.invoice_id))
            .with_for_update()
            .first()
        )
        if not inv:
        ...
        if (
            verified_currency
            and str(verified_currency).upper() != str(inv.currency).upper()
        ):
            return
        ...
        inv.balance_due = max(
            0.0, float(inv.total_amount or 0) - float(inv.amount_paid or 0)
        )
        if inv.balance_due <= 0.01:
            ...
            try:
                PaymentService._activate_paid_invoice_plan(
                    inv,
                    actor_user_id=getattr(payment, "reviewed_by_user_id", None)
                    or getattr(payment, "submitted_by_user_id", None),
                )
            except Exception as exc:
                current_app.logger.warning(
                    "post-payment plan activation failed for invoice %s: %s",
                    inv.id,
                    exc,
                )
        else:
    ...
    def verify_payment(payment_id: int) -> tuple[Optional[Payment], Optional[str]]:
        ...
        gw = (
            PaymentGateway.query.get(int(p.payment_gateway_id))
        ...
        if res.paid:
            ...
            PaymentService.apply_payment_success(
                p, verified_amount=res.amount, verified_currency=res.currency
            )
        else:
            p.status = "failed"
    ...
    @staticmethod
    def extract_reference_from_webhook(
        gateway_name: str, payload: Any
    ) -> Optional[str]:
        ...
        if g == "flutterwave":
            data = (payload or {}).get("data") if isinstance(payload, dict) else None
            ref = (
                (data or {}).get("tx_ref") or (payload or {}).get("tx_ref")
    ...
    @staticmethod
    def handle_webhook(
        *, gateway_name: str, headers: dict[str, str], body: bytes
    ) -> tuple[bool, str]:
        gname = (gateway_name or "").strip().lower()
        gw = (
            PaymentGateway.query.filter_by(name=gname, is_active=True)
            .order_by(PaymentGateway.is_default.desc())
            .first()
        )
        if not gw:
        ...
        p = (
            Payment.query.filter_by(gateway_name=gname, payment_reference=str(ref))
            .with_for_update()
            .first()
        )
        if not p:
    ...
    def submit_manual_payment(
        *,
        ...
        inv = (
            BillingInvoice.query.filter_by(id=int(invoice_id), tenant_id=tenant_id)
            .with_for_update()
            .first()
        )
        if not inv:
        ...
        if (
            float(inv.balance_due or 0) <= 0
            or str(inv.payment_status or "").lower() == "paid"
        ):
            return None, "Invoice already paid"

        manual = (
            PaymentGateway.query.filter_by(name="manual", is_active=True)
            .order_by(PaymentGateway.is_default.desc())
            .first()
        )
        if not manual:
    ...
    def review_manual_payment(
        *, payment_id: int, reviewer_id: int, approve: bool, note: Optional[str]
    ) -> tuple[Optional[Payment], Optional[str]]:
        p = Payment.query.filter_by(id=int(payment_id)).with_for_update().first()
        if not p:
            return None, "Payment not found"
        ...
        if approve:
            ...
            PaymentService.apply_payment_success(
                p, verified_amount=float(p.amount or 0), verified_currency=p.currency
            )
