    ...
    def enqueue_class_fanout(class_id, title, message, notification_type="info"):
        ...
        try:
            ...
            if notifications:
                ...
                db.session.flush()
                logger.info(
                    "Durable audience fanout flushed successfully",
                    ...
                    for notif in notifications:
                        try:
                            notif_data = {
                                ...
                                "time": (
                                    notif.time.isoformat()
                                    if getattr(notif, "time", None)
                                    else (
                            ...
                            logger.warning(
                                "Socket.IO notification emission failed for user",
                                user_id=notif.recipient_id,
                                error=str(socket_err),
                            )
                except Exception as import_err:
                    logger.warning(
                        "Failed to initialize Socket.IO broadcast for fanout",
                        error=str(import_err),
                    )

        except Exception as e:
            logger.error(
                "Error in enqueue_class_fanout", error=str(e), class_id=class_id
            )
            raise e
...
def execute_durable_audience_fanout(class_id, title, message):
    ...
    try:
        ...
        logger.error(
            "Error in execute_durable_audience_fanout", error=str(e), class_id=class_id
        )
        raise e
