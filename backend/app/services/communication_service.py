...
class CommunicationService:
    ...
    def create_group_message(
        ...
        try:
            ...
            # Get group members
            group_members = CommunicationService._get_group_members(
                group_type, group_id
            )
    ...
    def get_user_messages(
        user_id: int, message_type: str = "all", limit: int = 50, offset: int = 0
    ) -> Dict[str, Any]:
        """
    ...
    def mark_message_read(message_id: str, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if message_id.startswith("notification_"):
                ...
                if notification and (
                    notification.user_id == user_id or notification.user_id is None
                ):
    ...
    @staticmethod
    def _get_targeted_users(
        target_roles: List[str], target_classes: List[int]
    ) -> List[User]:
...
def handle_join_communication_room(data):
    ...
    try:
        ...
        logger.info("User joined communication room", user_id=user_id, room=room_name)
...
def handle_leave_communication_room(data):
    ...
    try:
        ...
        logger.info("User left communication room", user_id=user_id, room=room_name)
...
def handle_real_time_message(data):
    ...
    try:
        ...
        recipient_type = (
            MessageService._get_user_type(recipient) if recipient else "unknown"
        )
        ...
        logger.info(
            "Real-time message saved to DB and sent",
            message_id=real_time_message["id"],
            sender_id=sender_id,
            recipient_id=db_message.recipient_id,
        )
...
class CommunicationService:
    ...
    def create_bulk_message(
        ...
        for recipient_id in recipient_ids:
            try:
                ...
                failed_messages.append({"recipient_id": recipient_id, "error": str(e)})
        ...
        try:
            ...
            for message in paginated_messages.items:
                message_data = {
                    ...
                    "sender_name": CommunicationService._get_user_name(
                        message.sender_id
                    ),
                    ...
                    "attachments": CommunicationService._get_message_attachments(
                        message.id
                    ),
                    "reactions": CommunicationService._get_message_reactions(
                        message.id
                    ),
                }
    ...
    def mark_message_as_read(message_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            logger.info(
                "Message marked as read", message_id=message_id, user_id=user_id
            )
    ...
    def delete_message(message_id: int, user_id: int, permanent: bool = False) -> bool:
        ...
        try:
            ...
            if permanent:
                ...
                attachments = MessageAttachment.query.filter_by(
                    message_id=message_id
                ).all()
            ...
            logger.error(
                "Failed to delete message", error=str(e), message_id=message_id
            )
            db.session.rollback()
            return False
    ...
    def create_chat_room(
        name: str, room_type: str, created_by: int, participants: List[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "Chat room created", room_id=chat_room.id, created_by=created_by
            )
    ...
    def handle_typing_indicator(user_id: int, chat_room_id: str, is_typing: bool):
        ...
        try:
            ...
            if is_typing:
                socketio.emit(
                    "user_typing",
                    {"user": {"id": user_id, "name": user_name, "isTyping": True}},
                    room=chat_room_id,
                    namespace="/chat",
                )
            else:
    ...
    def get_chat_statistics(user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Total messages sent
            sent_count = Message.query.filter(Message.sender_id == user_id).count()
    ...
    def _send_realtime_message(message_data: Dict, chat_room_id: str = None):
        ...
        try:
            if chat_room_id:
                socketio.emit(
                    "new_message", message_data, room=chat_room_id, namespace="/chat"
                )
            else:
...
def handle_leave_chat(data):
    ...
    if chat_id and user_id:
        ...
        emit("user_left", {"user_id": user_id, "chat_id": chat_id}, room=chat_id)
