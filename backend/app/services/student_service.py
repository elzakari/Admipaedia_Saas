...
class StudentService:
    ...
    def get_all_students(
        ...
        return query.order_by(Student.admission_number).paginate(
    ...
    def _update_admission_snapshot(student: Optional[Student]) -> None:
        ...
        phone_value = (
            getattr(student, "phone", None) or getattr(student, "telephone", None) or ""
        )
        address_value = (
            getattr(student, "address", None)
            or getattr(student, "residential_address", None)
            or ""
        )
        ...
        form_data.update(
            {
                ...
                or form_data.get("gender", ""),
                "blood_group": getattr(student, "blood_group", None)
                or form_data.get("blood_group", ""),
                "religion": getattr(student, "religious_denomination", None)
                or form_data.get("religion", ""),
                "religious_denomination": getattr(
                    student, "religious_denomination", None
                )
                or form_data.get("religious_denomination", ""),
                "nationality": getattr(student, "nationality", None)
                or form_data.get("nationality", ""),
                ...
                "emergency_contact": phone_value
                or form_data.get("emergency_contact", ""),
                "prev_school_name": getattr(student, "previous_school", None)
                or form_data.get("prev_school_name", ""),
                "prev_school_class": getattr(student, "previous_class", None)
    ...
    def create_student(
        self, student_data: Dict[str, Any], tenant_id=None
        ...
        try:
            ...
            if not user:
                logger.warning(
                    "Attempted to create student with non-existent user",
            ...
            if (
                "admission_number" not in student_data
                or not student_data["admission_number"]
            ):
                student_data["admission_number"] = Student.generate_admission_number(
                    tenant_id=tenant_id
                )
                logger.info(
                ...
                if existing_student:
                    logger.warning(
                        "Attempted to create student with existing admission number",
                        admission_number=student_data["admission_number"],
                    )
                    return None, "Admission number already exists"

            # Check if user already has a student profile
            existing_profile = Student.query.filter_by(
                user_id=student_data["user_id"]
            ).first()
            if existing_profile:
                logger.warning(
                    "Attempted to create duplicate student profile",
                    user_id=student_data["user_id"],
            ...
            try:
                ...
                if tenant_id is not None:
                    existing = TenantMembership.query.filter_by(
                        user_id=new_student.user_id, tenant_id=tenant_id
                    ).first()
                    if not existing:
                        self.db_session.add(
                            TenantMembership(
            ...
            error_msg = str(e)
            logger.error(
                "Database error creating student",
                error=error_msg,
    ...
    def update_student(
        ...
        try:
            ...
            previous_fee_category = (
                str(getattr(student, "fee_category", "") or "").strip().lower()
            )
            ...
            if "admission_number" in student_data and student_data["admission_number"]:
                if student_data["admission_number"] != student.admission_number:
                    ...
                    if existing_student and existing_student.id != student_id:
                        logger.warning(
                            "Attempted to update student with existing admission number",
                            student_id=student_id,
                            admission_number=student_data["admission_number"],
                        )
                        return None, "Admission number already exists"
            ...
            for key, value in student_data.items():
                if hasattr(student, key):
                    ...
                    logger.warning(
                        "Attempted to set unknown attribute on student",
                        student_id=student_id,
                        attribute=key,
                    )
            ...
            should_auto_apply_fees = (
                ...
                "fee_category" in student_data
                and previous_fee_category != new_fee_category
            )
            ...
            error_msg = str(e)
            logger.error(
                "Database error updating student",
                error=error_msg,
                student_id=student_id,
            )
            return None, f"Database error: {error_msg}"
            ...
            error_msg = str(e)
            logger.error(
                "Unexpected error updating student",
                error=error_msg,
                student_id=student_id,
                data_keys=(
    ...
    def delete_student(self, student_id: int) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(
                "Database error deleting student",
                error=error_msg,
                student_id=student_id,
            )
            ...
            error_msg = str(e)
            logger.error(
                "Unexpected error deleting student",
                error=error_msg,
                student_id=student_id,
            )
    ...
    def assign_class(
        ...
        try:
            ...
            if not student:
                logger.warning(
                    "Attempted to assign non-existent student to class",
                    student_id=student_id,
                    class_id=class_id,
                )
                return None, "Student not found"
            ...
            if not class_obj:
                logger.warning(
                    "Attempted to assign student to non-existent class",
                    student_id=student_id,
                    class_id=class_id,
                )
                return None, "Class not found"

            if getattr(student, "tenant_id", None) and getattr(
                class_obj, "tenant_id", None
            ):
            ...
            if not max_capacity:
                try:
                    config = AcademicConfigurationService.build_harmonized_config(
                        class_obj.tenant_id
                    )
            ...
            error_msg = str(e)
            logger.error(
                "Database error assigning student to class",
                error=error_msg,
                student_id=student_id,
                class_id=class_id,
            )
            ...
            error_msg = str(e)
            logger.error(
                "Unexpected error assigning student to class",
                error=error_msg,
                student_id=student_id,
                class_id=class_id,
            )
