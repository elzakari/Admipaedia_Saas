...
class CalendarService:
    ...
    def sync_with_exams(exam_id=None):
        ...
        try:
            ...
            for exam in exams:
                ...
                description = (
                    exam.description
                    or f"Exam for {exam.subject.name if exam.subject else 'Unknown Subject'}"
                )
                ...
                if existing_event:
                    ...
                    end_time = (exam_date + timedelta(minutes=exam.duration)).strftime(
                        "%H:%M"
                    )
                    existing_event.end_time = end_time
                    db.session.commit()
                    ...
                    event = CalendarEvent(
                        ...
                        end_time=(
                            exam_date + timedelta(minutes=exam.duration)
                        ).strftime("%H:%M"),
    ...
    @staticmethod
    def get_events_with_sync_token(
        user_id, sync_token=None, start_date=None, end_date=None
    ):
        ...
        try:
            ...
            return {"events": result, "sync_token": new_sync_token}
    ...
    def resolve_conflicts(server_event, client_event):
        ...
        server_updated = datetime.fromisoformat(
            server_event.get("updated_at", "2000-01-01T00:00:00").replace("Z", "+00:00")
        )
        client_updated = datetime.fromisoformat(
            client_event.get("updated_at", "2000-01-01T00:00:00").replace("Z", "+00:00")
        )
            ...
            for event_data in client_events:
                ...
                if event_id and event_id.startswith("temp_"):
                    ...
                    try:
                        ...
                        results["errors"].append({"temp_id": event_id, "error": str(e)})
                elif event_id:
                    ...
                    try:
                        ...
                        if server_event:
                            ...
                            if server_event.updated_at and "updated_at" in event_data:
                                ...
                                client_updated = datetime.fromisoformat(
                                    event_data["updated_at"].replace("Z", "+00:00")
                                )
                            ...
                            results["updated"].append(
                                {"id": updated_event.id, "title": updated_event.title}
                            )
                        else:
                            # Event doesn't exist on server
                            results["errors"].append(
                                {"id": event_id, "error": "Event not found on server"}
                            )
                    except Exception as e:
                        results["errors"].append({"id": event_id, "error": str(e)})
