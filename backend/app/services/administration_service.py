    ...
    def get_financial_summary(
        ...
        try:
            ...
            total_fee_collections = sum(
                (getattr(fr, "paid_amount", None) or Decimal("0")) for fr in fee_records
            )
            outstanding_fees = sum(
                (getattr(fr, "balance", None) or Decimal("0")) for fr in fee_records
            )
            ...
            if academic_year:
                year_start = datetime.strptime(
                    f"{academic_year}-01-01", "%Y-%m-%d"
                ).date()
                year_end = datetime.strptime(
                    f"{academic_year}-12-31", "%Y-%m-%d"
                ).date()
            ...
            net_balance = (total_income or Decimal("0")) - (
