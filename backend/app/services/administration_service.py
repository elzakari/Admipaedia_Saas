...
from app.models.administration import (Asset, Budget, Facility,
                                       MaintenanceRequest, Transaction)
...
class AdministrationService:
    ...
    def get_all_budgets(
        self, page: int = 1, per_page: int = 20, academic_year: Optional[str] = None
    ) -> Pagination:
        ...
        return query.order_by(Budget.created_at.desc()).paginate(
    ...
    def create_budget(
        ...
        try:
            ...
            if existing_budget:
                logger.warning(
                    "Attempted to create duplicate budget",
                    category=budget_data["category"],
                    academic_year=budget_data["academic_year"],
                )
                return (
                    None,
                    f"Budget for {budget_data['category']} in {budget_data['academic_year']} already exists",
                )
    ...
    def update_budget(
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(
                "Database error updating budget", error=error_msg, budget_id=budget_id
            )
            return None, f"Database error: {error_msg}"
            ...
            error_msg = str(e)
            logger.error(
                "Unexpected error updating budget", error=error_msg, budget_id=budget_id
            )
            return None, f"Unexpected error: {error_msg}"
    ...
    def delete_budget(self, budget_id: int) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(
                "Database error deleting budget", error=error_msg, budget_id=budget_id
            )
            ...
            error_msg = str(e)
            logger.error(
                "Unexpected error deleting budget", error=error_msg, budget_id=budget_id
            )
    ...
    def create_transaction(
        ...
        try:
            ...
            if (
                "reference_number" not in transaction_data
                or not transaction_data["reference_number"]
            ):
    ...
    def get_financial_summary(
        ...
        Uses Student Payments (student_payments) as authoritative source for
        fee-collection revenue.  Transaction ledger income/expenses are reported
        alongside so generic admin cashbook views still work.
        ...
        try:
            ...
            total_allocated = sum((b.allocated_amount or zero) for b in budgets)
            total_spent = sum((b.spent_amount or zero) for b in budgets)
            budget_utilization = (
                (total_spent / total_allocated * Decimal("100"))
                if total_allocated
                else zero
            )
            ...
            total_fee_collections = sum(
                (getattr(fr, "paid_amount", None) or zero) for fr in fee_records
            )
            ...
            authoritative_revenue = (
                ...
                if total_collected_payments > zero
                else total_fee_collections
            )

            collection_rate = (
                (authoritative_revenue / total_billed * Decimal("100"))
                if total_billed
                else zero
            )
            ...
            if academic_year:
                ...
                if year_str.isdigit():
                    try:
                        year_start = date(int(year_str), 1, 1)
            ...
            total_income = sum(
                ...
                if tx_type_val(t.transaction_type) == "income"
            ) or zero
            total_expenses = sum(
                t.amount
                for t in transactions
                if tx_type_val(t.transaction_type) == "expense"
            ) or zero
    ...
    def get_all_fee_records(
        ...
        return query.order_by(StudentFee.id.desc()).paginate(
            page=page, per_page=per_page, error_out=False
        )

    def get_student_fee_records(
        self, student_id: int, academic_year: Optional[str] = None
    ) -> List[StudentFee]:
        from app.models.finance import FeeStructure, StudentFee
    ...
    def get_all_facilities(
        ...
        return query.order_by(Facility.created_at.desc()).paginate(
            page=page, per_page=per_page
        )
    ...
    def update_facility(
        self, facility_id: int, update_data: Dict[str, Any]
    ) -> Tuple[bool, Union[Facility, str]]:
    ...
    def delete_facility(self, facility_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if facility.maintenance_requests.count() > 0 or facility.assets.count() > 0:
                return (
                    False,
                    "Cannot delete facility with associated maintenance requests or assets",
                )
    ...
    def get_all_maintenance_requests(
        ...
        return query.order_by(MaintenanceRequest.created_at.desc()).paginate(
            page=page, per_page=per_page
        )
    ...
    ) -> Tuple[bool, Union[MaintenanceRequest, str]]:
    ...
    def update_maintenance_request(
        ...
        try:
            ...
            self.db_session.commit()
            logger.info(
                f"Maintenance request updated successfully", request_id=request_id
            )
            return True, maintenance_request
            ...
            self.db_session.rollback()
            logger.error(
                f"Unexpected error updating maintenance request {request_id}: {str(e)}"
            )
            return False, f"Unexpected error: {str(e)}"
    ...
    def update_asset(
        self, asset_id: int, update_data: Dict[str, Any]
    ) -> Tuple[bool, Union[Asset, str]]:
    ...
    def get_assets_needing_service(self) -> List[Asset]:
        ...
        return (
            Asset.query.filter(
                Asset.next_service_date <= today, Asset.is_active == True
            )
            .options(db.joinedload(Asset.facility))
            .all()
        )
    ...
    def get_assets_with_expired_warranty(self) -> List[Asset]:
        ...
        today = date.today()
        return (
            Asset.query.filter(Asset.warranty_expiry < today, Asset.is_active == True)
            .options(db.joinedload(Asset.facility))
            .all()
        )
    ...
    def get_infrastructure_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_facilities = Facility.query.filter(
                Facility.status == "active"
            ).count()
            facilities_under_maintenance = Facility.query.filter(
                Facility.status == "under_maintenance"
            ).count()

            total_maintenance_requests = MaintenanceRequest.query.count()
            pending_maintenance_requests = MaintenanceRequest.query.filter(
            ...
            total_asset_value = db.session.query(
                db.func.sum(Asset.current_value)
            ).filter(Asset.is_active == True).scalar() or Decimal("0.00")
