    ...
    def get_resources_by_class(class_id, page=1, per_page=20):
        ...
        return (
            Resource.query.options(
                joinedload(Resource.class_), joinedload(Resource.uploaded_by)
            )
    ...
    def update_resource(resource_id, resource_data, class_id, teacher_id, file=None):
        ...
        try:
            ...
            if file:
                ...
                if resource.file_path and os.path.exists(
                    os.path.join(current_app.root_path, resource.file_path)
                ):
