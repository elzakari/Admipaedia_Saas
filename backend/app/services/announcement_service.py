...
class AnnouncementService:
    ...
    def normalize_target_roles(target_roles):
        if isinstance(target_roles, str):
            raw_roles = [
                item.strip().lower() for item in target_roles.split(",") if item.strip()
            ]
        elif isinstance(target_roles, (list, tuple, set)):
            raw_roles = [
                str(item).strip().lower() for item in target_roles if str(item).strip()
            ]
        else:
            raw_roles = []
    ...
    def build_tenant_filter(tenant_id):
        ...
        return or_(
            ...
            and_(
                Announcement.tenant_id.is_(None),
                Class.tenant_id == normalized_tenant_id,
            ),
        )
    ...
    def prepare_announcement_payload(announcement_data):
        ...
        payload["scope"] = scope
        payload["target_roles"] = AnnouncementService.serialize_target_roles(
            payload.get("target_roles")
        )
        payload["recipients"] = AnnouncementService.derive_recipients(
            payload.get("target_roles")
        )
        ...
        if scope == "class_bound":
            ...
            expected_tenant_id = AnnouncementService.normalize_tenant_id(
                getattr(class_obj, "tenant_id", None)
            )
            provided_tenant_id = AnnouncementService.normalize_tenant_id(
    ...
    def announcement_targets_role(announcement, role_key):
        normalized = AnnouncementService.normalize_target_roles(
            getattr(announcement, "target_roles", None)
        )
        if "all" in normalized:
            return True
        ...
        recipients = (
            (getattr(announcement, "recipients", None) or "all").strip().lower()
        )
        if recipients == "all":
            return True
    ...
    def create_announcement(announcement_data, broadcast=True):
        ...
        """
        try:
            prepared_payload, error = AnnouncementService.prepare_announcement_payload(
                announcement_data
            )
            if error:
                return None, error
    ...
    def update_announcement(
        announcement_id, announcement_data, class_id, teacher_id, broadcast=True
    ):
        ...
        try:
            ...
            if "target_roles" in announcement_data:
                announcement_data["target_roles"] = (
                    AnnouncementService.serialize_target_roles(
    ...
    def update_announcement_admin(announcement_id, announcement_data, broadcast=True):
        try:
            ...
            merged_payload = {
                ...
                or ("class_bound" if announcement.class_id else "global"),
                ...
                "target_roles": announcement_data.get(
                    "target_roles", announcement.target_roles
                ),
                "is_published": announcement_data.get(
            ...
            db.session.rollback()
            logger.error(
                "Error updating announcement (admin)",
                error=str(e),
                announcement_id=announcement_id,
            )
            return None, str(e)
    ...
    def delete_announcement_admin(announcement_id):
        try:
            ...
            AnnouncementService.broadcast_announcement_deletion(
                announcement_id_for_broadcast, class_id_for_broadcast
            )
            return True, None
        except SQLAlchemyError as e:
            db.session.rollback()
            logger.error(
                "Error deleting announcement (admin)",
                error=str(e),
                announcement_id=announcement_id,
            )
            return False, str(e)
    ...
    def get_announcements_for_user(user_id, page=1, per_page=10, tenant_id=None):
        ...
        try:
            ...
            query = Announcement.query.outerjoin(
            ...
            if user.role == "student":
                ...
                if student:
                    effective_tenant_id = (
                        requested_tenant_id
                        or AnnouncementService.normalize_tenant_id(student.tenant_id)
                    )
                    tenant_filter = AnnouncementService.build_tenant_filter(
                        effective_tenant_id
                    )
                ...
                if parent:
                    ...
                    class_ids = [
                        student.class_id for student in students if student.class_id
                    ]
                    effective_tenant_id = requested_tenant_id or (
                        AnnouncementService.normalize_tenant_id(students[0].tenant_id)
                        if students
                        else None
                    )
                    tenant_filter = AnnouncementService.build_tenant_filter(
                ...
                effective_tenant_id = (
                    requested_tenant_id
                    or AnnouncementService.normalize_tenant_id(
                ...
                if requested_tenant_id:
                    effective_tenant_ids = [
                        AnnouncementService.normalize_tenant_id(requested_tenant_id)
                    ]
                else:
                    ...
                    effective_tenant_ids = [
                        AnnouncementService.normalize_tenant_id(membership.tenant_id)
                        for membership in memberships
                    ]

                effective_tenant_ids = [
                    tenant for tenant in effective_tenant_ids if tenant
                ]
                if effective_tenant_ids:
                    tenant_filters = [
                        AnnouncementService.build_tenant_filter(tenant)
                        for tenant in effective_tenant_ids
                    ]
                    tenant_filters = [
                        tenant_filter
            ...
            if user.role in ("student", "parent"):
                query = query.filter(
                    ...
                    & (
                        ...
                        | (Announcement.scheduled_date <= now)
                    )
                )
            elif user.role == "teacher":
                query = query.filter(
            ...
            if user.role in ("student", "parent"):
                ...
                all_items = [
                    ...
                    if AnnouncementService.announcement_targets_role(
                        announcement, role_key
                    )
                ]
            ...
            logger.error(
                "Error fetching announcements for user", error=str(e), user_id=user_id
            )
            return [], 0
    ...
    def broadcast_announcement(announcement, is_update=False):
        ...
        try:
            ...
            announcement_data = {
                ...
                "scheduled_date": (
                    announcement.scheduled_date.isoformat()
                    if getattr(announcement, "scheduled_date", None)
                    else None
                ),
            ...
            normalized_roles = AnnouncementService.normalize_target_roles(
                getattr(announcement, "target_roles", None)
            )

            if normalized_roles == ["all"]:
                target_rooms.extend(
                    ["role_students", "role_parents", "role_teachers", "role_admins"]
                )
            else:
                target_rooms.extend(
                    [f"role_{role}" for role in normalized_roles if role != "all"]
                )
    ...
    def broadcast_announcement_deletion(announcement_id, class_id):
        ...
        try:
            ...
            logger.info(
                "Announcement deletion broadcasted",
                announcement_id=announcement_id,
                class_id=class_id,
            )
            ...
            logger.error(
                "Error broadcasting announcement deletion",
                error=str(e),
                announcement_id=announcement_id,
            )
            return False
