...
class EnhancedAuthService:
    ...
    @classmethod
    def register_user_with_security(
        cls,
        username: str,
        email: str,
        password: str,
        ...
        try:
            ...
            if require_verification:
                ...
                new_user.email_verification_expires = datetime.utcnow() + timedelta(
                    hours=24
                )
    ...
    def authenticate_with_security(
        ...
        try:
            ...
            session_duration = (
                cls.REMEMBER_ME_DURATION if remember_me else cls.SESSION_TIMEOUT
            )
            ...
            try:
                ...
                if tenant_ids:
                    ...
                    tenants = [
                        {"id": str(t.id), "name": t.name, "slug": t.slug} for t in trows
                    ]
                    if len(tenant_ids) == 1:
    ...
    def setup_mfa(cls, user_id: int) -> Dict:
        ...
        try:
            ...
            totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
                name=user.email, issuer_name=cls.MFA_ISSUER
            )
            ...
            backup_codes = [
                secrets.token_hex(4).upper() for _ in range(cls.MFA_BACKUP_CODES_COUNT)
            ]
    ...
    @classmethod
    def verify_mfa(
        cls, mfa_token: str, code: str, is_backup_code: bool = False
    ) -> Dict:
        """
        ...
        try:
            ...
            if not verified:
                cls._log_security_event(
                    ...
                    {"user_id": user.id, "is_backup_code": is_backup_code},
                )
