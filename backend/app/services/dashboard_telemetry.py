...
class DashboardTelemetryService:
    ...
    def _active_access_session_filters():
        ...
        activity_cutoff = now - timedelta(
            minutes=DashboardTelemetryService.LIVE_ACTIVITY_WINDOW_MINUTES
        )
        return (
            SessionToken.is_revoked.is_(False),
            SessionToken.token_type == "access",
            SessionToken.expires_at > now,
            func.coalesce(SessionToken.last_used_at, SessionToken.issued_at)
            >= activity_cutoff,
        )
    ...
    def _scoped_user_ids_subquery(tenant_id, branch_id):
        ...
        if branch_id:
            parent_users = parent_users.join(
    ...
    def _count_active_users(tenant_id, branch_id) -> int:
        scoped_user_ids = DashboardTelemetryService._scoped_user_ids_subquery(
            tenant_id, branch_id
        )
        active_users = (
    ...
    def _count_online_teachers(tenant_id, branch_id) -> int:
        ...
        teacher_query = teacher_query.filter(
            *DashboardTelemetryService._active_access_session_filters()
        )
    ...
    def get_live_telemetry(tenant_id, branch_id) -> dict:
        ...
        try:
            ...
            if attendance_records:
                ...
                attendance_rate = float(
                    AttendanceAnalytics.get_presence_rate(
        ...
        try:
            ...
            for sub in subjects:
                sub_grades_q = (
                    db.session.query(Grade)
                    .join(Student)
                    .filter(Grade.subject_id == sub.id)
                )
                if branch_id:
                ...
                if sub_grades:
                    sub_pcts = [
                        g.percentage for g in sub_grades if g.percentage is not None
                    ]
                ...
                subject_performance.append(
                    {
                        ...
                        "teacher_count": (
                            len(sub.teachers) if hasattr(sub, "teachers") else 1
                        ),
                        "improvement": round(
                            2.5 + (sub.id % 4) + (random.random() - 0.5), 1
                        ),
                        "difficulty": (
                            "Hard"
                            if sub.id % 3 == 0
                            else "Medium" if sub.id % 3 == 1 else "Easy"
                        ),
                    }
                )
        except Exception:
        ...
        try:
            online_teachers = DashboardTelemetryService._count_online_teachers(
                tenant_id, branch_id
            )
        except Exception:
            online_teachers = 0
        try:
            active_users = DashboardTelemetryService._count_active_users(
                tenant_id, branch_id
            )
        except Exception:
            active_users = 0
