...
def _load_active_email_provider_config(
    ...
    if not db_provider:
        db_provider = (
            PlatformServiceProviderConfig.query.filter_by(
                service_type="email", is_active=True
            )
            .order_by(
                PlatformServiceProviderConfig.priority.asc(),
                PlatformServiceProviderConfig.id.asc(),
            )
            .first()
        )
    ...
    if db_provider:
        ...
        selected_provider = (
            provider_override or db_provider.provider_key or selected_provider
        ).lower()
...
def send_email(
    ...
    if supress_send:
        ...
        logger.info(
            "Email sending suppressed by configuration",
            subject=subject,
        ...
        return EmailResult(
            ok=True,
            provider="suppressed",
            message="Sending suppressed by configuration",
            message_id="",
    ...
    try:
        default_from = (
            current_app.config.get("EMAIL_FROM_ADDRESS")
            or current_app.config.get("MAIL_DEFAULT_SENDER")
            or current_app.config.get("MAIL_USERNAME")
        )
        default_name = current_app.config.get("EMAIL_FROM_NAME", "Admipaedia Support")
    except Exception:
        default_from = os.environ.get("EMAIL_FROM_ADDRESS") or os.environ.get(
            "MAIL_DEFAULT_SENDER"
        )
    ...
    try:
        ...
        if cfg:
            logger.info(
                "Retrieved active email integration settings from DB",
                provider_key=selected_provider,
            )
    except Exception as e:
        logger.warning(
            f"Could not load email configuration from database, falling back to env/defaults: {str(e)}"
        )
    ...
    if selected_provider == "ses_api":
        ...
        aws_key = (
            cfg.get("awsAccessKeyId")
            or cfg.get("aws_access_key")
            or os.environ.get("AWS_ACCESS_KEY_ID")
        )
        aws_secret = (
            cfg.get("awsSecretAccessKey")
            or cfg.get("aws_secret_key")
        ...
        resend_key = (
            cfg.get("apiKey") or cfg.get("api_key") or os.environ.get("RESEND_API_KEY")
        )
        ...
        try:
            ...
            logger.warning(
                f"Failed to query legacy SystemSettings SMTP fallback block: {str(e)}"
            )

        smtp_host = _sanitize_runtime_smtp_host(
            cfg.get("smtpHost")
            or cfg.get("smtp_host")
            or db_smtp_host
            or os.environ.get("MAIL_SERVER", "localhost")
        )
        smtp_port = cfg.get("smtpPort") or cfg.get("smtp_port") or db_smtp_port
        smtp_user = (
            cfg.get("smtpUsername")
            or cfg.get("smtp_username")
            or db_smtp_user
            or os.environ.get("MAIL_USERNAME")
        )
        smtp_pass = (
            cfg.get("smtpPassword")
            or cfg.get("smtp_password")
            or db_smtp_pass
            or os.environ.get("MAIL_PASSWORD")
        )
        smtp_enc = (
            cfg.get("smtpEncryption")
            or cfg.get("smtp_encryption")
            or db_smtp_enc
            or os.environ.get("MAIL_USE_TLS", "tls")
        )
        ...
        if not ok:
            fallback_provider = (
                str(
                    os.environ.get("FALLBACK_EMAIL_PROVIDER")
                    or cfg.get("fallbackProvider")
                    or cfg.get("fallback_provider")
                    or ""
                )
                .strip()
                .lower()
            ...
            if fallback_provider == "resend":
                logger.warning(
                    f"⚠️ Primary SMTP failed: {message}. Activating Resend fallback..."
                )
                resend_key = (
                    cfg.get("apiKey")
                    or cfg.get("api_key")
                ...
                if ok_fb:
                    ok = True
                    message = (
                        f"Resend API fallback succeeded. (Primary failure: {message})"
                    )
    ...
    if ok:
        logger.info(
            "Polymorphic email sent successfully",
        ...
        logger.error(
            "Failed to send polymorphic email",
            provider=selected_provider,
            error=message,
            subject=subject,
            duration_ms=duration,
...
def _send_via_ses_api(
    ...
    try:
        ...
        if attachments or not use_v2:
            ...
            if html_body:
                simple_content["Simple"]["Body"]["Html"] = {
                    "Data": html_body,
                    "Charset": "UTF-8",
                }
                ...
                params["attachments"].append(
                    {
                        "filename": filename,
                        "content": (
                            list(data) if isinstance(data, (bytes, bytearray)) else data
                        ),
                    }
                )
...
def send_school_registration_email(
    user_email: str,
