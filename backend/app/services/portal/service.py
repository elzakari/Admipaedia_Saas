...
class ParentPortalService:
    ...
    def get_authorized_child_dashboard(user_id, student_id, tenant_id=None):
        ...
        if student.parent_id != parent.id:
            return (
                None,
                "You are not authorized to access this student's dashboard",
                403,
            )
    ...
    def get_child_dashboard(student_id):
        ...
        att_rate = (
            (att_summary.get("present", 0) / total_att * 100) if total_att > 0 else 0
        )
        ...
        recent_grades = (
            FinalGrade.query.filter_by(student_id=student_id)
            .order_by(FinalGrade.computed_at.desc())
            .limit(5)
            .all()
        )
        ...
        return {
            ...
            "attendance": {"rate": round(att_rate, 1), "summary": att_summary},
            "finance": {"outstanding": outstanding, "currency": currency},
            "recent_grades": [
                {
    ...
    def complete_child_setup_task(user_id, task_id, tenant_id=None):
        ...
        task = ParentChildSetupTask.query.filter_by(
            id=task_id, parent_id=parent.id
        ).first()
        if not task:
