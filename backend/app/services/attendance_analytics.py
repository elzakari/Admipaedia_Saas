...
class AttendanceAnalytics:
    ...
    def get_presence_rate(
        ...
        if total == 0:
            return Decimal("0.00")
        return (Decimal(present + late) / Decimal(total) * Decimal("100.00")).quantize(
            Decimal("0.01")
        )

    @staticmethod
    def get_monthly_student_stats(
    ...
    @staticmethod
    def get_monthly_branch_stats(
        branch_id: str, year: int, month: int, class_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
