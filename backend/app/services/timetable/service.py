...
class TimetableService:
    ...
    def get_teacher_display_name(teacher):
        ...
        first_name = (
            ...
            or getattr(teacher, "first_name", None)
            or ""
        )
        last_name = (
            getattr(user, "last_name", None)
            or getattr(teacher, "last_name", None)
            or ""
        )
    ...
    def validate_slot_relationships(data):
        ...
        if not any(mapped_class.id == class_obj.id for mapped_class in subject.classes):
            ...
            return (
                class_obj,
                subject,
                "Selected subject is not assigned to this class. Assign it in Settings > Academic > Subjects first.",
            )
        ...
        if not any(
            ...
            return (
                class_obj,
                subject,
                "Selected teacher is not assigned to this subject. Update the subject setup before creating the timetable slot.",
            )
    ...
    def get_class_schedule_bounds(class_obj=None):
        ...
        start_time = (
            TimetableService.parse_time_string(getattr(class_obj, "start_time", None))
            or default_start
        )
        end_time = TimetableService.parse_time_string(
            getattr(class_obj, "end_time", None)
        )
        if end_time and end_time > start_time:
            return start_time, end_time

        derived_end = datetime.combine(
            datetime.utcnow().date(), start_time
        ) + timedelta(hours=8)
    ...
    def ensure_periods_for_class(class_id=None):
        ...
        class_obj = (
            Class.query.get(class_id)
            if class_id
            else Class.query.order_by(Class.id).first()
        )
        ...
        if not generated_periods:
            fallback_start = datetime.combine(
                datetime.utcnow().date(), default_start := time(8, 0)
            )
    ...
    @staticmethod
    def get_consecutive_period_block(
        start_period_id, required_period_count, periods=None
    ):
    ...
    def get_conflict_records(data, current_slot_id=None, required_period_count=None):
        ...
        if required_period_count is None:
            if current_slot_id:
                ...
                required_period_count = (
                    TimetableService.calculate_required_period_count(subject)
                )
        ...
        if term_aliases:
            class_conflict_query = class_conflict_query.filter(
                TimetableSlot.term.in_(term_aliases)
            )
            teacher_conflict_query = teacher_conflict_query.filter(
                TimetableSlot.term.in_(term_aliases)
            )

        if current_slot_id:
            class_conflict_query = class_conflict_query.filter(
                TimetableSlot.id != current_slot_id
            )
            teacher_conflict_query = teacher_conflict_query.filter(
                TimetableSlot.id != current_slot_id
            )
        ...
        return {
            ...
            "class_conflict": class_conflict_query.order_by(
                TimetableSlot.period_id
            ).first(),
            "teacher_conflict": teacher_conflict_query.order_by(
                TimetableSlot.period_id
            ).first(),
            "required_period_count": required_period_count,
        }
    ...
    def get_period_option_payload(
        ...
        required_period_count = (
            1
            if current_slot_id
            else TimetableService.calculate_required_period_count(subject)
        )
        ...
        for period in periods:
            period_block = TimetableService.get_consecutive_period_block(
                period.id, required_period_count, periods
            )
            blocked_reason = None
            disabled = False
            ...
            if getattr(period, "is_break", False):
                ...
                disabled = True
                blocked_reason = (
                    f"Requires {required_period_count} consecutive timeframe(s)."
                )
            ...
            block_start = (
                period_block[0].start_time if period_block else period.start_time
            )
            block_end = period_block[-1].end_time if period_block else period.end_time
            if (
                recommended_period_id is None
                and not disabled
                and block_start >= class_start_time
            ):
            ...
            serialized_periods.append(
                {
                    ...
                    "span_period_ids": (
                        [item.id for item in period_block] if period_block else []
                    ),
                }
            )
        ...
        return {
            ...
            "meta": {
                "class_start_time": TimetableService.format_time_value(
                    class_start_time
                ),
                "required_period_count": required_period_count,
    ...
    def create_slot(data):
        ...
        class_obj, subject, relationship_error = (
            TimetableService.validate_slot_relationships(data)
        )
        if relationship_error:
            return None, relationship_error
        required_period_count = TimetableService.calculate_required_period_count(
            subject
        )
        conflict_result = TimetableService.get_conflict_records(
            data, required_period_count=required_period_count
        )
        ...
        if not period_block:
            return (
                None,
                f"Selected timeframe does not provide {required_period_count} consecutive hourly slot(s) from the class start schedule.",
            )

        if conflict_result["class_conflict"]:
            return (
                None,
                f"Class already has a lesson at this timeframe ({conflict_result['class_conflict'].subject.name})",
            )

        if conflict_result["teacher_conflict"]:
            return (
                None,
                f"Teacher is already teaching {conflict_result['teacher_conflict'].class_.name} at this timeframe",
            )
    ...
    def get_all_slots(filters=None):
        ...
        if filters:
            ...
            if filters.get("term"):
                query = query.filter(
                    TimetableSlot.term.in_(
                        TimetableService.get_term_aliases(filters["term"])
                    )
                )
    ...
    def update_slot(slot_id, data):
        ...
        try:
            ...
            _, _, relationship_error = TimetableService.validate_slot_relationships(
                payload
            )
            if relationship_error:
                return None, relationship_error
    ...
    def check_conflicts(data):
        ...
        conflicts = []
        conflict_result = TimetableService.get_conflict_records(
            data, current_slot_id=data.get("id")
        )
    ...
    def auto_schedule_greedy(term, academic_year):
        ...
        for class_ in classes:
            ...
            for subject in subjects:
                ...
                if not teacher:
                    errors.append(
                        f"No teacher found for {subject.name} in {class_.name}"
                    )
                    continue
