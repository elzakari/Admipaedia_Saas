...
class RealtimeService:
    ...
    async def broadcast_announcement(
        self, announcement_data: Dict, target_roles: List[str] = None
    ):
        ...
        try:
            ...
            if target_roles:
                ...
                for user_id in self.connection_manager.get_connected_users():
                    user_role = self.connection_manager.user_metadata.get(
                        user_id, {}
                    ).get("role")
        ...
        try:
            ...
            try:
                ...
                parent_users = (
                    ...
                    .filter(student_parent_association.c.student_id == student_id)
                    .all()
                )
                ...
                parent_message = {
                    ...
                    "data": {**grade_data, "student_id": student_id},
                    "timestamp": datetime.utcnow().isoformat(),
                }
                ...
                for parent_user in parent_users:
                    if parent_user.user_id:
                        await self.connection_manager.send_personal_message(
                            parent_message, parent_user.user_id
                        )

                logger.info(
                    f"Sent grade update notification to student {student_id} and {len(parent_users)} parents"
                )

            except Exception as parent_error:
                logger.warning(
                    f"Could not send grade update to parents for student {student_id}: {parent_error}"
                )
    ...
    async def send_system_message(self, message: str, user_ids: List[int] = None):
        ...
        try:
            ...
            logger.info(
                f"Sent system message to {len(user_ids) if user_ids else 'all'} users"
            )
