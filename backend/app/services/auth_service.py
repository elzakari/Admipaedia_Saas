...
from app.extensions import (  # Import from extensions, not from __init__
    bcrypt, db)
...
class AuthService:
    ...
    def record_login_attempt(
        user_id=None, email=None, success=True, ip_address=None, user_agent=None
    ):
    ...
    def get_portal_data(role):
        ...
        portals = {
            ...
            "user": {"defaultRoute": "/dashboard", "allowedPages": []},
        }
    ...
    def is_admin_creator(request=None):
        ...
        try:
            ...
            if h is not None:
                ...
                if get_fn:
                    val = (
                        get_fn("X-Admin-Secret-Key")
                        or get_fn("ADMIN_SECRET_KEY")
                        or get_fn("admin-secret-key")
                    )
                    if val == secret_key:
                        return True
