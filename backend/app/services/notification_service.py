...
class NotificationService:
    ...
    @staticmethod
    def send_sms(
        tenant_id, branch_id, phone_number: str, message: str
    ) -> NotificationLog:
        """
        ...
        if is_testing:
            ...
            try:
                ...
                headers = {
                    "Authorization": f"Bearer {current_app.config.get('SMS_PROVIDER_KEY', 'mock-key')}"
                }
                payload = {
                    "to": phone_number,
                    "message": message,
    ...
    @staticmethod
    def send_email(
        tenant_id, branch_id, email_address: str, subject: str, content: str
    ) -> NotificationLog:
        """
        ...
        if is_testing:
            logger.info(
                f"[TEST MOCK EMAIL] Sending to {email_address} with subject '{subject}': {content}"
            )
        else:
            try:
                ...
                msg["From"] = current_app.config.get(
                    "SMTP_FROM", "no-reply@admipaedia.com"
                )
            ...
            try:
                ...
                for n in notifications:
                    socketio.emit(
                        ...
                        room=f"user_{n.user_id}",
                        namespace="/notifications",
                    )
            except Exception as e:
                logger.warning(
                    f"Failed to emit WebSocket for bulk notifications: {str(e)}"
                )
