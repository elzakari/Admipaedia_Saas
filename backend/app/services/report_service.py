...
from reportlab.graphics.shapes import Drawing, Ellipse, Group, Rect, String
...
from app.models.administration import (Asset, Facility, FacilityStatus,
...
class ReportService:
    ...
    def _resolve_student_photo_for_pdf(photo_value: Optional[str]) -> Optional[str]:
        ...
        if normalized.startswith("/api/v1/enhanced-students/profile-picture/"):
            filename = normalized.split(
                "/api/v1/enhanced-students/profile-picture/", 1
            )[1]
            candidates = [
                os.path.join(
                ...
                os.path.join(
                    current_app.root_path, "uploads", "profile_pictures", filename
                ),
            ]
            for candidate in candidates:
                if os.path.isfile(candidate):
    ...
    def get_administrative_report_data(
        ...
        return {
            ...
            "enrollment": {
                ...
                "growth_rate": (
                    (new_enrollments / total_students * 100)
                    if total_students > 0
                    else 0
                ),
            },
            ...
            trends.append(
                {
                    ...
                    "pending": float(
                        pending_fees / 6
                    ),  # Placeholder logic for pending trends
                }
            )
    ...
    def _get_academic_data(filters, start_date, end_date, visualizations):
        ...
        if "average_grade" in visualizations.get("metrics", []):
            avg_score = (
                db.session.query(func.avg(Grade.marks_obtained))
                .filter(Grade.id.in_([g.id for g in grades]))
                .scalar()
            )
            sections.append(
                {
        ...
        if "student_grades" in visualizations.get("tables", []):
            ...
            sections.append(
                {"type": "table", "title": "Student Grades", "data": table_data}
            )
        return sections
    ...
    def generate_student_report_card_pdf(report_data: Dict[str, Any]) -> io.BytesIO:
        ...
        school_tagline = SystemSetting.get_value(
            "school_tagline", "Nurturing Minds. Building Futures."
        )
        ...
        # Get Theme
        theme_name = report_data.get(
            "theme", SystemSetting.get_value("report_card_theme", "navy")
        )
        ...
        pill_badge_style = ParagraphStyle(
            "PillBadgeStyle",
            parent=styles["Normal"],
            fontSize=7,
            fontName="Helvetica-Bold",
        ...
        label_style = ParagraphStyle(
            ...
            parent=styles["Normal"],
            fontSize=8,
            textColor=ReportService.MUTED_COLOR,
            fontName="Helvetica-Bold",
        )
        value_style = ParagraphStyle(
            ...
            fontSize=9,
            fontName="Helvetica",
            textColor=ReportService.TEXT_COLOR,
        )
        ...
        header_drawing.add(
            Rect(0, 0, w, header_height, fillColor=primary_color, strokeColor=None)
        )
        ...
        header_drawing.add(
            Ellipse(
                ...
                fillColor=colors.Color(
                    accent_color.red, accent_color.green, accent_color.blue, alpha=0.15
                ),
                strokeColor=None,
            )
        )
        ...
        header_drawing.add(
            Rect(
                ...
                40,
                40,
                fillColor=ReportService.WHITE,
                strokeColor=ReportService.GOLD,
                strokeWidth=1,
                rx=4,
                ry=4,
            )
        ...
        header_text_table = Table(
            [
                ...
                [
                    Paragraph(
                        school_name.upper(),
                        ParagraphStyle("H", parent=title_style, fontSize=18),
                    ),
                    "",
                ],
                [
                    Paragraph(
                        school_tagline,
                        ParagraphStyle("T", parent=tagline_style, fontSize=8),
                    ),
                    "",
                ],
            ],
        ...
        contact_drawing.add(
            Rect(0, 0, w, 0.2 * inch, fillColor=secondary_color, strokeColor=None)
        )
        ...
        contact_table = Table(
            [
                [
                    Paragraph(
                        school_address,
                        ParagraphStyle("C", parent=contact_style, fontSize=7.5),
                    ),
                    Paragraph(
                        school_website,
                        ParagraphStyle("C", parent=contact_style, fontSize=7.5),
        ...
        elements.append(
            Paragraph(
                ...
                ParagraphStyle("RT", parent=pill_badge_style, fontSize=7, tracking=1.5),
            )
        )
        elements.append(Spacer(1, 0.05 * inch))

        photo_path = ReportService._resolve_student_photo_for_pdf(
            info.get("profile_picture")
        )
        photo_cell = (
            ...
            if photo_path
                ...
                style=[
                    ("BOX", (0, 0), (-1, -1), 1, ReportService.BORDER_COLOR),
                    ("BACKGROUND", (0, 0), (-1, -1), ReportService.LIGHT_BG),
                    ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                ],
            )
        )
            ...
            [
                    ...
                    [
                        ...
                        [
                            Paragraph(
                                info.get("name", "N/A"),
                                ParagraphStyle(
                                    "SV", fontSize=9.5, fontName="Helvetica-Bold"
                                ),
                            ),
                            Paragraph(
                                info.get("class", "N/A"),
                        ...
                        [
                            Paragraph(
                                info.get("academic_year", "N/A"),
                                ParagraphStyle(
                                    "SV", fontSize=8.5, fontName="Helvetica-Bold"
                                ),
                            ),
                            Paragraph(
                                info.get("term", "N/A"),
                ...
                Table(
                    [
                        ...
                        [
                            Paragraph(
                                info.get("educational_level", "N/A"),
                                ParagraphStyle(
                                    "SV", fontSize=8.5, fontName="Helvetica-Bold"
                                ),
                            ),
                            Paragraph(
                                info.get("admission_number", "N/A"),
                                ParagraphStyle(
                                    "SV",
                                    fontSize=8.5,
                                    fontName="Helvetica-Bold",
                        ...
                        [
                            Table(
                                ...
                                style=[
                                    ...
                                    ("ROUNDRECT", (0, 0), (-1, -1), 8, primary_color),
                                ],
                            ),
                            "",
                        ],
                        ...
                        [
                            Table(
                                [
                                    [
                                        Paragraph(
                                            f"GPA: {perf.get('overall_gpa', '0.00')} / 4.00",
                                            ParagraphStyle(
                                                "GPA",
                                                fontSize=15,
                                                fontName="Helvetica-Bold",
        ...
        student_table = Table(
            student_panel_data, colWidths=[3.0 * inch, 1.1 * inch, 2.8 * inch]
        )
            ...
            academic_data.append(
                [
                    Paragraph(
                        s.get("name"),
                        ParagraphStyle(
                            "SubName", fontSize=8.5, fontName="Helvetica-Bold"
                        ),
                    ),
                    Paragraph(
                        str(s.get("score")),
                        ParagraphStyle("Score", alignment=TA_CENTER, fontSize=8.5),
                    ),
                    Paragraph(
                        s.get("grade"),
                        ParagraphStyle(
        ...
        academic_table = Table(
            academic_data, colWidths=[1.8 * inch, 0.7 * inch, 0.7 * inch, 3.8 * inch]
        )
        academic_table.setStyle(
            TableStyle(
                [
                    ...
                    ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                    ("GRID", (0, 0), (-1, -1), 0.5, ReportService.BORDER_COLOR),
                ]
            )
        )
        ...
        gen_header.setStyle(
            TableStyle(
                [
                    ...
                    ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                    ("BOX", (0, 0), (-1, -1), 0.5, ReportService.BORDER_COLOR),
                ]
            )
        )
        ...
        att_metrics = [
            [
                ...
                Paragraph(
                    f"TOTAL DAYS<br/><font size=12>{att.get('total_days', 0)}</font>",
                    ParagraphStyle("AS", alignment=TA_CENTER, fontSize=7, leading=10),
                ),
                Paragraph(
                    f"PRESENT<br/><font size=12>{att.get('present_days', 0)}</font>",
                    ParagraphStyle("AS", alignment=TA_CENTER, fontSize=7, leading=10),
                ),
                Paragraph(
                    f"RATE<br/><font size=12>{att.get('attendance_rate', 0.0)}%</font>",
                    ParagraphStyle("AS", alignment=TA_CENTER, fontSize=7, leading=10),
                ),
            ]
        ]
        att_table = Table(
            att_metrics, colWidths=[1.8 * inch, 1.6 * inch, 1.6 * inch, 2.0 * inch]
        )
        att_table.setStyle(
            TableStyle(
                [
        ...
        soc_header_data = [
            [Paragraph("PSYCHOLOGICAL AND SOCIAL PERSPECTIVE", section_header_style)]
        ]
        ...
        comp_cards = [
            [
                create_comp_card("ATTITUDE", "Positive attitude.", secondary_color),
                create_comp_card("CONDUCT", "Respectful.", ReportService.BLUE),
                create_comp_card("INTEREST", "Keen interest.", ReportService.ORANGE),
            ]
        ]
        elements.append(
            Table(comp_cards, colWidths=[2.33 * inch, 2.33 * inch, 2.33 * inch])
        )
        elements.append(Spacer(1, 0.1 * inch))
        ...
        remarks_data = [
            [
                Table(
                    [
                        [Paragraph("TEACHER REMARKS", pill_badge_style)],
                        [
                            Paragraph(
                                report_data.get(
                                    "teacher_comments", "A hardworking student."
                                ),
                                ParagraphStyle(
                                    "RemText", fontSize=8, leading=10, italic=True
                                ),
                            )
                        ],
                    ],
                ...
                Table(
                    [
                        [Paragraph("PARENT REMARKS", pill_badge_style)],
        ...
        if subjects:
            radar.labels, radar.data = [s.get("name")[:8] for s in subjects[:5]], [
                [s.get("score", 0) for s in subjects[:5]]
            ]
            radar.strands.strokeColor, radar.strands.fillColor = (
                secondary_color,
                light_color,
            )
        ...
        elements.append(
            Table(
                [[chart_drawing]],
                colWidths=[w],
                style=[("BOX", (0, 0), (-1, -1), 0.5, ReportService.BORDER_COLOR)],
            )
        )
        elements.append(Spacer(1, 0.2 * inch))
        ...
        sig_data = [
            [
                Paragraph(
                    "_________________<br/>Teacher's Signature",
                    ParagraphStyle("S", alignment=TA_CENTER, fontSize=8),
                ),
                Paragraph(
                    "_________________<br/>Parent / Guardian",
                    ParagraphStyle("S", alignment=TA_CENTER, fontSize=8),
                ),
                Paragraph(
                    "_________________<br/>Principal's Signature",
                    ParagraphStyle("S", alignment=TA_CENTER, fontSize=8),
                ),
            ]
        ]
    ...
    def generate_excel(report_data: Dict[str, Any]) -> io.BytesIO:
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            for section in report_data.get("sections", []):
                ...
                if section["type"] == "metric":
                    df = pd.DataFrame(
                        [{"Metric": section["title"], "Value": section["value"]}]
                    )
                    ...
                    def flatten(d, prefix=""):
                        for k, v in d.items():
                            if isinstance(v, dict):
                                ...
                                flat_data.append(
                                    {f"{prefix}{k}": f"List of {len(v)} items"}
                                )
                            else:
