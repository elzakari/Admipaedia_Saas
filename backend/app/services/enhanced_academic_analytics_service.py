...
class EnhancedAcademicAnalyticsService:
    ...
    def _get_school_performance_overview(
        ...
        try:
            ...
            for (
                grade_letter,
                boundaries,
            ) in EnhancedAcademicAnalyticsService.GES_BOUNDARIES.items():
                count = len(
                    [s for s in scores if boundaries["min"] <= s <= boundaries["max"]]
                )
            ...
            for subject_id, subject_scores in subject_grades.items():
                ...
                if subject and subject_scores:
                    ...
                    subject_pass_rate = (
                        len([s for s in subject_scores if s and s >= 40])
                        / len([s for s in subject_scores if s is not None])
                        * 100
                    )
                    ...
                    subject_performance.append(
                        {
                            ...
                            "grade_distribution": EnhancedAcademicAnalyticsService._calculate_subject_grade_distribution(
                                subject_scores
                            ),
                        }
                    )
            ...
            return {
                ...
                "subject_performance": sorted(
                    subject_performance, key=lambda x: x["average_score"], reverse=True
                ),
                "trends": trends,
                "performance_indicators": {
                    "excellent_rate": grade_distribution.get("A", {}).get(
                        "percentage", 0
                    ),
                    "good_rate": grade_distribution.get("B", {}).get("percentage", 0)
                    + grade_distribution.get("C", {}).get("percentage", 0),
                    "at_risk_rate": grade_distribution.get("F", {}).get(
                        "percentage", 0
                    ),
                },
            }
    ...
    def _get_attendance_analytics(
        ...
        try:
            ...
            return {
                ...
                "class_comparison": sorted(
                    class_comparison, key=lambda x: x["attendance_rate"], reverse=True
                ),
                "risk_students": sorted(
                    risk_students, key=lambda x: x["attendance_rate"]
                ),
                "status_breakdown": {
                    "present": len(
                        [a for a in attendance_records if a.status == "present"]
                    ...
                    "absent": len(
                        [a for a in attendance_records if a.status == "absent"]
                    ),
                    "late": len([a for a in attendance_records if a.status == "late"]),
                    "excused": len(
                        [a for a in attendance_records if a.status == "excused"]
                    ),
                },
            }
    ...
    def _get_grade_distribution_analytics(
        ...
        try:
            ...
            statistical_analysis = {
                ...
                "mode": (
                    round(float(pd.Series(scores).mode().iloc[0]), 2)
                    if len(pd.Series(scores).mode()) > 0
                    else 0
                ),
                ...
                "quartiles": {
                    ...
                    "iqr": round(
                        np.percentile(scores, 75) - np.percentile(scores, 25), 2
                    ),
                },
            }
            ...
            performance_bands = {
                ...
                "good": {
                    "min": 60,
                    "max": 79,
                    "count": len([s for s in scores if 60 <= s < 80]),
                    "percentage": 0,
                },
                "satisfactory": {
                    "min": 40,
                    "max": 59,
                    "count": len([s for s in scores if 40 <= s < 60]),
                    "percentage": 0,
                },
                "needs_improvement": {
                    "max": 39,
                    "count": len([s for s in scores if s < 40]),
                    "percentage": 0,
                },
            }
            ...
            return {
                ...
                "grade_trends": EnhancedAcademicAnalyticsService._calculate_grade_trends(
                    grades, date_from, date_to
                ),
            }
    ...
    @staticmethod
    def _calculate_subject_grade_distribution(
        scores: List[float],
        ...
        for (
            ...
            count = len(
                [s for s in valid_scores if boundaries["min"] <= s <= boundaries["max"]]
            )
    ...
    def _calculate_performance_trends(
        grades: List[Grade], date_from: datetime, date_to: datetime
    ) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _calculate_grade_trends(
        grades: List[Grade], date_from: datetime, date_to: datetime
    ) -> List[Dict[str, Any]]:
        """Calculate grade trends over time."""
        return EnhancedAcademicAnalyticsService._calculate_performance_trends(
            grades, date_from, date_to
        )
        ...
        try:
                    ...
                    teacher_performance.append(
                        {
                            ...
                            "performance_rating": EnhancedAcademicAnalyticsService._calculate_teacher_rating(
                                avg_score, pass_rate, attendance_rate
                            ),
                        }
                    )
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_class_score": (
                        round(
                            sum(t["average_score"] for t in teacher_performance)
                            / len(teacher_performance),
                            2,
                        )
                    ...
                    "average_pass_rate": (
                        round(
                            sum(t["pass_rate"] for t in teacher_performance)
                            / len(teacher_performance),
                            2,
                        )
                        if teacher_performance
                    ...
                    "needs_support": [
                        t for t in teacher_performance if t["performance_rating"] < 3.0
                    ],
                },
            }
    ...
    @staticmethod
    def _calculate_teacher_rating(
        avg_score: float, pass_rate: float, attendance_rate: float
    ) -> float:
    ...
    def _get_class_comparison_analytics(
        ...
        try:
                    ...
                    class_performance.append(
                        {
                            ...
                            "teacher_name": (
                                f"{class_obj.teacher.first_name} {class_obj.teacher.last_name}"
            ...
            performance_ranking = sorted(
                class_performance, key=lambda x: x["average_score"], reverse=True
            )
            attendance_ranking = sorted(
                class_performance, key=lambda x: x["attendance_rate"], reverse=True
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "school_average": (
                        round(
                            sum(c["average_score"] for c in class_performance)
                    ...
                    "best_performing": (
                        performance_ranking[0] if performance_ranking else None
                    ),
                    "needs_attention": [
                        c
                        for c in class_performance
                        if c["average_score"] < 50 or c["attendance_rate"] < 75
                    ],
                },
            }
    ...
    def _get_risk_assessment(
        ...
        try:
            ...
            return {
                ...
                "low_risk_students": sorted(
                    low_risk, key=lambda x: x["risk_score"], reverse=True
                ),
                "summary": {
                    "percentage_at_risk": (
                        round((len(risk_assessment) / len(students) * 100), 2)
                        if students
                        else 0
                    ),
                    "most_common_factors": EnhancedAcademicAnalyticsService._get_common_risk_factors(
                        risk_assessment
                    ),
                    "intervention_priority": high_risk[
                        :10
    ...
    def _generate_student_recommendations(
        risk_factors: List[str], avg_score: float, attendance_rate: float
    ) -> List[str]:
    ...
    def _get_common_risk_factors(
        ...
        for factor, count in sorted(
            ...
            common_factors.append(
                {"factor": factor, "count": count, "percentage": round(percentage, 2)}
            )
    ...
    def _generate_admin_recommendations(
        ...
        risk_percentage = risk_assessment.get("summary", {}).get(
    ...
    def _get_teacher_class_performance(
        ...
        try:
            ...
            return {
                ...
                "performance_trend": EnhancedAcademicAnalyticsService._calculate_performance_trends(
                    grades, date_from, date_to
                ),
            }
                    ...
                    student_progress.append(
                        {
                            ...
                            "total_assessments": len(scores),
                            "trend": trend,
                            "current_grade": EnhancedAcademicAnalyticsService._get_ges_grade(
                                avg_score
                            ),
    ...
    def _get_single_class_analytics(
        ...
        try:
            ...
            return {
                ...
                "needs_attention": [
                    s for s in student_averages if s["average_score"] < 50
                ],
            }
    ...
    def _generate_teacher_recommendations(
        performance_summary: Dict[str, Any], student_progress: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        needs_attention = [
            s for s in student_progress if s.get("needs_attention", False)
        ]
    ...
    def _get_student_analytics(
        ...
        try:
            ...
            if scores:
                            ...
                            subjects.append(
                                {
                                    ...
                                    "trend": EnhancedAcademicAnalyticsService._calculate_subject_trend(
                                        valid_scores
                                    ),
                                }
                            )
                ...
                # Performance trend
                performance_trend = EnhancedAcademicAnalyticsService._calculate_student_performance_trend(
                    grades
                )
            ...
            return {
                ...
                "subject_performance": sorted(
                    subjects, key=lambda x: x["average_score"], reverse=True
                ),
    ...
    def _get_parent_analytics(
        ...
        try:
            ...
            return {
                ...
                "overall_summary": {
                    "average_performance": (
                        round(total_performance / len(children), 2) if children else 0
                    ),
                    "attendance_rate": (
                        round(total_attendance / len(children), 2) if children else 0
                    ),
                    "at_risk_children": at_risk_count,
                },
                "recommendations": EnhancedAcademicAnalyticsService._generate_parent_recommendations(
                    children_analytics
                ),
            }
        except Exception as e:
    ...
    def _generate_parent_recommendations(
        children_analytics: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_subject_trend(scores: List[float]) -> str:
        ...
        recent_avg = (
            sum(scores[-3:]) / len(scores[-3:]) if len(scores) >= 3 else scores[-1]
        )
        earlier_avg = (
    ...
    @staticmethod
    def _calculate_student_performance_trend(
        grades: List[Grade],
    ) -> List[Dict[str, Any]]:
                ...
                trend_data.append(
                    {
                        "month": month,
                        "average_score": round(avg_score, 2),
                        "grade": EnhancedAcademicAnalyticsService._get_ges_grade(
                            avg_score
                        ),
