...
class LessonService:
    ...
    def get_lesson_subject(materials):
        ...
        try:
            subject_id = int(item.get('subject_id')) if item.get('subject_id') is not None else None
    ...
    def serialize_lesson(lesson, extra=None):
        extra = extra or {}
        subject_meta = LessonService.get_lesson_subject(getattr(lesson, 'materials', None))
        teacher = getattr(lesson, 'teacher', None)
        teacher_name = ''
        if teacher:
            teacher_name = getattr(teacher, 'full_name', None) or f"{getattr(teacher, 'first_name', '')} {getattr(teacher, 'last_name', '')}".strip()
        ...
        payload = {
            ...
            'objectives': LessonService.get_material_value(lesson.materials, 'objectives'),
            'classwork': LessonService.get_material_value(lesson.materials, 'classwork'),
            ...
            'resources': LessonService.get_material_value(lesson.materials, 'resources', []),
    ...
    @staticmethod
    def get_lesson_monitoring(page=1, per_page=50, tenant_id=None, class_id=None, teacher_id=None, status=None, date_from=None, date_to=None):
        ...
        classes_logged_today = summary_query.filter(Lesson.date == today).with_entities(Lesson.class_id).distinct().count()

        summary = {
            'total_logs': summary_query.count(),
            'completed_logs': summary_query.filter(Lesson.status == 'completed').count(),
            'in_progress_logs': summary_query.filter(Lesson.status == 'in-progress').count(),
            'planned_logs': summary_query.filter(Lesson.status == 'planned').count(),
            'today_logs': summary_query.filter(Lesson.date == today).count(),
            'classes_covered': summary_query.with_entities(Lesson.class_id).distinct().count(),
            'teachers_reporting': summary_query.filter(Lesson.teacher_id.isnot(None)).with_entities(Lesson.teacher_id).distinct().count(),
            'classes_without_logs_today': max(total_classes - classes_logged_today, 0),
        }

        paginated_lessons = summary_query.order_by(Lesson.date.desc(), Lesson.created_at.desc()).paginate(
            page=page,
            per_page=per_page
        )
