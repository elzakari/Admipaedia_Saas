...
class SecurePurgeService:
    ...
    def force_purge_user(
        ...
        try:
            ...
            if student_ids:
                student_setup_tasks = ParentChildSetupTask.query.filter(
                    ParentChildSetupTask.student_id.in_(student_ids)
                ).all()
                for task in student_setup_tasks:
                    db.session.delete(task)
            if parent_ids:
                parent_setup_tasks = ParentChildSetupTask.query.filter(
                    ParentChildSetupTask.parent_id.in_(parent_ids)
                ).all()
                        ...
                        for col in fk.get("constrained_columns") or []:
                            ...
                            for val in target_vals:
                                if table in delete_row_tables:
                                    db.session.execute(
                                        text(
                                            f"DELETE FROM {q_table} WHERE {q_col} = :val"
                                        ),
                                        {"val": val},
                                    )
                                elif nullable:
                                    db.session.execute(
                                        text(
                                            f"UPDATE {q_table} SET {q_col} = NULL WHERE {q_col} = :val"
                                        ),
                                        {"val": val},
                                    )
                                else:
                                    db.session.execute(
                                        text(
                                            f"UPDATE {q_table} SET {q_col} = :actor WHERE {q_col} = :val"
                                        ),
            ...
            return True, {
                "success": True,
                "message": "User profile force-purged cleanly",
            }
        except Exception as e:
            db.session.rollback()
            return False, {
                "error": f"Force-purge transaction failed: {str(e)}",
