...
class PolymorphicGradingEngine:
    ...
    def calculate_final_score(
        class_id: int, class_score: float, exam_score: float
    ) -> dict:
        """
        ...
        if grade_level and grade_level.track_id:
            ...
            scale = PolymorphicGradingScale.query.filter_by(
                tenant_id=tenant_id, track_id=grade_level.track_id
            ).first()
        ...
        if not scale:
            raise ValueError(
                f"No polymorphic grading scale configured for tenant {tenant_id}."
            )
