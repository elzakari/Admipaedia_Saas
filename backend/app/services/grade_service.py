...
class GradeService:
    ...
    def get_grade_by_id(grade_id):
        ...
        if grade:
            cache_service.set(
                cache_key, grade_schema.dump(grade), ttl=cache_service.SHORT_TTL
            )
    ...
    @staticmethod
    def get_exam_grades(exam_id, page, per_page, tenant_id=None, branch_id=None):
    ...
    def create_grade(data):
        ...
        existing_grade = Grade.query.filter(
            and_(
                Grade.student_id == data["student_id"], Grade.exam_id == data["exam_id"]
            )
        ).first()
    ...
    def get_class_performance(class_id):
        ...
        # Calculate pass rate
        passing_grades = [
            grade for grade in grades if grade.percentage >= 40
        ]  # Assuming 40% is passing
        pass_rate = (len(passing_grades) / total_grades) * 100

        # Calculate grade distribution
        grade_distribution = {"A+": 0, "A": 0, "B+": 0, "B": 0, "C+": 0, "C": 0, "F": 0}
        ...
        for letter in grade_distribution:
            grade_distribution[letter] = (
                grade_distribution[letter] / total_grades
            ) * 100
    ...
    def calculate_final_grades(
        student_id=None, class_id=None, subject_id=None, term=None, academic_year=None
    ):
        ...
        try:
            ...
            for key, group_grades in grouped_grades.items():
                ...
                weighted_sum = sum(
                    grade.percentage * grade.weight for grade in group_grades
                )
    ...
    def get_student_grade_report(
        student_id, subject_id=None, class_id=None, term=None, academic_year=None
    ):
        ...
        try:
            ...
            for subject_id, subject_grades in subjects.items():
                ...
                if assessment_grades:
                    assessment_average = sum(
                        g.percentage for g in assessment_grades
                    ) / len(assessment_grades)
            ...
            final_average = (
                sum(g.percentage for g in final_grades) / len(final_grades)
                if final_grades
                else 0
            )
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    if existing_grade and update_existing:
                        ...
                        existing_grade.grade_letter = GradeService.get_grade_letter(
                            percentage
                        )
