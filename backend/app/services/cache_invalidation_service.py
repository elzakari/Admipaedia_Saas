...
class CacheInvalidationService:
    ...
    def invalidate_student_cache(
        self, student_id: int, user_id: Optional[int] = None
    ) -> None:
    ...
    def invalidate_teacher_cache(
        self, teacher_id: int, user_id: Optional[int] = None
    ) -> None:
    ...
    def invalidate_parent_cache(
        self, parent_id: int, user_id: Optional[int] = None
    ) -> None:
    ...
    def invalidate_attendance_cache(
        ...
        try:
            ...
            logger.info(
                f"Invalidated attendance cache for attendance_id: {attendance_id}"
            )
        except Exception as e:
    ...
    def invalidate_exam_cache(
        self, exam_id: int, class_id: Optional[int] = None
    ) -> None:
    ...
    def invalidate_upcoming_exams_cache(self, class_id: Optional[int] = None) -> None:
        ...
        try:
            if class_id:
                ...
                self.cache_service.delete_pattern(
                    f"upcoming_exams:dto:class_{class_id}:*"
                )
            else:
                # All upcoming exams
                self.cache_service.delete_pattern("upcoming_exams:*")
    ...
    def invalidate_teacher_analytics_cache(
    ...
    def invalidate_subject_related_cache(self, subject_id: int) -> None:
        ...
        try:
            ...
            logger.info(
                f"Invalidated subject-related cache for subject_id: {subject_id}"
            )
        except Exception as e:
    ...
    def warm_upcoming_exams_cache(
        self, class_id: Optional[int] = None, days: int = 7
    ) -> None:
