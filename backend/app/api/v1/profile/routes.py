...
from app.middleware.security_middleware import (log_security_event, rate_limit,
...
from app.utils.avatar_utils import (get_avatar_directory,
...
def _get_or_create_profile(user: User) -> UserProfile:
    ...
    if profile:
        return profile
    profile = UserProfile(user_id=user.id, display_name=user.username or "User")
...
class UpdatePreferencesSchema(Schema):
    theme_mode = fields.String(
        load_default=None,
        allow_none=True,
        validate=validate.OneOf(["system", "light", "dark", "gradient", "casaos"]),
    )
    language = fields.String(
        load_default=None, allow_none=True, validate=validate.Length(min=2, max=12)
    )
    date_time_format = fields.String(
        load_default=None,
        allow_none=True,
        validate=validate.OneOf(["auto", "12h", "24h"]),
    )
    default_profile_tab = fields.String(
        load_default=None,
        allow_none=True,
        validate=validate.OneOf(["profile", "security", "preferences"]),
    )
...
def get_preferences():
    try:
        ...
        logger.error(
            "get_preferences_error", error=str(err), user_id=get_jwt_identity()
        )
...
def update_preferences():
    try:
        ...
        logger.error(
            "update_preferences_error", error=str(err), user_id=get_jwt_identity()
        )
...
def list_sessions():
    try:
        ...
        data.sort(
            key=lambda s: (
                not s.get("is_current", False),
                s.get("last_used_at") or s.get("issued_at") or "",
            ),
            reverse=False,
        )
...
def revoke_session(session_id: int):
    try:
        ...
        token = SessionToken.query.filter_by(
            id=session_id, user_id=user_id, token_type="access"
        ).first()
        if not token:
...
def revoke_other_sessions():
    try:
        ...
        logger.error(
            "revoke_other_sessions_error", error=str(err), user_id=get_jwt_identity()
        )
        return (
            jsonify({"success": False, "error": "Failed to revoke other sessions"}),
            500,
        )
...
def list_security_events():
    try:
        ...
        logger.error(
            "list_security_events_error", error=str(err), user_id=get_jwt_identity()
        )
        return (
            jsonify({"success": False, "error": "Failed to load security activity"}),
            500,
        )
        ...
        if linked_student and getattr(linked_student, "profile_picture_locked", False):
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "Student profile picture is locked by admin",
                    }
                ),
                403,
            )
        ...
        if old_url and old_url.startswith("/api/v1/profile/avatar/"):
            ...
            try:
                for base_dir in (
                    ...
                    get_avatar_directory(prefer_legacy=True),
                ):
