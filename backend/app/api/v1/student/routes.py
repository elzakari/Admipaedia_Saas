...
def get_student_dashboard_summary():
    ...
    if class_id:
        ...
        all_assignments = Assignment.query.filter(
            Assignment.class_id == class_id, Assignment.status == "active"
        ).all()
        ...
        pending_count = sum(
            1 for a in all_assignments if a.id not in submitted_assignment_ids
        )
    ...
    avg_grade = (
        Grade.query.with_entities(func.avg(Grade.percentage))
        .filter(Grade.student_id == student.id)
        .scalar()
    )
    term_average_grade = (
        round(float(avg_grade or 0.0), 2) if avg_grade is not None else None
    )
    ...
    if class_id:
        ...
        slots_sorted = sorted(
            slots,
            key=lambda s: s.period.start_time if s.period else datetime.min.time(),
        )
        for s in slots_sorted:
            start_str = (
                s.period.start_time.strftime("%H:%M")
                if s.period and s.period.start_time
                else "08:00"
            )
            end_str = (
                s.period.end_time.strftime("%H:%M")
                if s.period and s.period.end_time
                else "09:30"
            )
            todays_classes.append(
                {
                    ...
                    "room": (
                        s.class_.room or f"Room {s.room_id}"
                        if s.room_id
                        else (s.class_.room or "N/A")
                    ),
                }
            )
...
def get_student_courses():
    ...
    for sub in subjects:
        ...
        first_slot = TimetableSlot.query.filter_by(
            class_id=class_id, subject_id=sub.id
        ).first()
        teacher_name = (
            first_slot.teacher.full_name
            if first_slot and first_slot.teacher
            else (cls.class_teacher_name or "TBA")
        )
        courses_data.append(
            {
                "id": sub.id,
    ...
    for slot in slots:
        start_str = (
            slot.period.start_time.strftime("%H:%M")
            if slot.period and slot.period.start_time
            else "08:00"
        )
        end_str = (
            slot.period.end_time.strftime("%H:%M")
            if slot.period and slot.period.end_time
            else "09:30"
        )
        ...
        timetable_data.append(
            {
                ...
                "room": (
                    cls.room or f"Room {slot.room_id}"
                    if slot.room_id
                    else (cls.room or "N/A")
                ),
            }
        )
    ...
    return (
        jsonify(
            {"success": True, "courses": courses_data, "timetable": timetable_data}
        ),
        200,
    )
...
def get_student_assignments():
    ...
    assignment_attachment_map = AssignmentService.get_attachment_map(
        "assignment", [a.id for a in assignments]
    )
    submission_attachment_map = AssignmentService.get_attachment_map(
        "assignment_submission", [sub.id for sub in submissions]
    )
        ...
        assignments_data.append(
            {
                ...
                "submission_date": (
                    ...
                    if sub and sub.submission_date
                    else None
                ),
                "submission_attachments": (
                    submission_attachment_map.get(str(sub.id), []) if sub else []
                ),
...
def get_student_grades():
    ...
    total_students = (
        Student.query.filter_by(class_id=student.class_id).count()
        if student.class_id
        else 1
    )
    ...
    class_students = (
        Student.query.filter_by(class_id=student.class_id).all()
        if student.class_id
        else [student]
    )
    ...
    student_avg = next(
        (avg_val for s_id, avg_val in student_averages if s_id == student.id), 0.0
    )
    ...
    for sub_id, g_list in grades_by_subject.items():
        ...
        avg_ca = (
            sum(ca_percentages) / len(ca_percentages)
            if ca_percentages
            else (g_list[0].percentage if g_list else 0.0)
        )
        avg_exam = (
...
def get_student_attendance_summary():
    ...
    overall_percentage = round(
        (
            ((days_present + days_late + days_excused) / total_days * 100)
            if total_days
            else 100.0
        ),
        2,
...
def get_student_timetable():
    ...
    return jsonify({"success": True, "timetable": timetable_data}), 200
...
def get_student_subject_lessons(subject_id):
    ...
    lessons = (
        Lesson.query.filter_by(class_id=student.class_id)
    ...
    return jsonify({"success": True, "lessons": lesson_payload}), 200
...
def get_student_notifications():
    ...
    notifications, _ = NotificationService.get_user_notifications(
        user_id=int(user_id), page=page, per_page=per_page
    )
        ...
        serialized.append(
            {
                ...
                "createdAt": (
                    ...
                    else (n.time.isoformat() if hasattr(n, "time") and n.time else "")
                ),
...
def mark_all_student_notifications_read():
    ...
    db.session.commit()
    return (
        jsonify({"success": True, "message": "All notifications marked as read"}),
        200,
    )
...
def get_student_conversations():
    ...
    query = query.filter(
        or_(
            ...
            and_(
                Message.sender_id.in_(user_ids), Message.is_deleted_by_sender == False
            ),
        )
    ...
    for msg in messages:
        ...
        sender_name = (
            "You" if msg.sender_id in user_ids else thread_map[other_id]["title"]
        )
...
def send_student_message():
    ...
    if not recipient_id or not content:
        return (
            jsonify({"success": False, "message": "Missing recipient or content"}),
            400,
        )
