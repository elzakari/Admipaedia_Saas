...
def get_exam_analytics(exam_id):
    ...
    try:
        ...
        if "error" in analytics:
            return jsonify({"success": False, "message": analytics["error"]}), (
...
def get_class_exam_schedule(class_id):
    ...
    try:
        ...
        if "error" in schedule:
            return jsonify({"success": False, "message": schedule["error"]}), 500
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to get schedule", "error": str(e)}
            ),
            500,
        )
...
def calculate_optimal_duration():
    ...
    try:
        ...
        return jsonify({"success": True, "duration_info": duration_info}), 200
...
def get_batch_exam_analytics():
    ...
    try:
        ...
        if not exam_ids:
            return jsonify({"success": False, "message": "No exam IDs provided"}), 400
        ...
        if analytics_results:
            total_students = sum(
                result["total_students"] for result in analytics_results
            )
            avg_pass_rate = sum(
...
def get_performance_trends():
    ...
    try:
        ...
        if date_from:
            query = query.filter(
                Exam.exam_date >= datetime.strptime(date_from, "%Y-%m-%d")
            )
        if date_to:
            query = query.filter(
                Exam.exam_date <= datetime.strptime(date_to, "%Y-%m-%d")
            )
