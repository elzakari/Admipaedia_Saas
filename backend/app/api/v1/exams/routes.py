...
def get_exams():
    ...
    try:
        ...
        include_conflicts = (
            request.args.get("include_conflicts", "false").lower() == "true"
        )
        ...
        if date_from:
            try:
                ...
                return (
                    jsonify({"success": False, "message": "Invalid date_from format"}),
                    400,
                )
        ...
        if date_to:
            try:
                ...
                return (
                    jsonify({"success": False, "message": "Invalid date_to format"}),
                    400,
                )
...
def create_exam():
    ...
    try:
        ...
        if check_conflicts:
            ...
            conflicts = EnhancedExamService.detect_exam_conflicts(
                data["class_id"], exam_date, data["duration"]
            )
        ...
        if check_conflicts:
            conflicts = EnhancedExamService.detect_exam_conflicts(
                exam.class_id, exam.exam_date, exam.duration, exam.id
            )
            exam_data["conflicts"] = conflicts
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to create exam", "error": str(e)}
            ),
            500,
        )
...
def update_exam(exam_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to update exam", "error": str(e)}
            ),
            500,
        )
...
def delete_exam(exam_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Failed to delete exam", "error": str(e)}
            ),
            500,
        )
...
def get_exam_statistics(exam_id):
    ...
    try:
        ...
        if "error" in analytics:
            return jsonify({"success": False, "message": analytics["error"]}), 500

        return jsonify({"success": True, "statistics": analytics}), 200
