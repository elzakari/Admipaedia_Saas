...
def _get_plain_token() -> str:
    header = request.headers.get("X-Service-Token") or request.headers.get(
        "X-Api-Token"
    )
    if header:
        return header.strip()
...
def validate_service_token():
    data = request.get_json(silent=True) or {}
    service_type = (
        str(data.get("service_type") or data.get("serviceType") or "").strip().lower()
    )
    ...
    token, err = ServiceTokenService.validate_token(
        token_plain, service_type, tenant_id_hint=tenant_hint
    )
    if err or not token:
...
def consume_service_token():
    ...
    status, err, http_status = ServiceTokenService.consume(
        token_plain, service_type, amount=amount_i, tenant_id_hint=tenant_hint
    )
    if err or not status:
        return (
            jsonify({"success": False, "message": err or "Invalid token"}),
            http_status,
        )
