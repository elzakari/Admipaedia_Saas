...
api_v1_bp.register_blueprint(
    departments_bp, url_prefix="/structures", name="structures"
)  # polymorphic alias
...
from app.api.v1.saas.routes import (complete_setup,
                                    get_tenant_notification_status)
from app.api.v1.settings.routes import get_assessment_categories

api_v1_bp.add_url_rule(
    "/tenant/complete-setup", "complete_setup", complete_setup, methods=["POST"]
)
api_v1_bp.add_url_rule(
...
api_v1_bp.add_url_rule(
    "/admin/users", "admin_list_users", _admin_list_users, methods=["GET"]
)

from app.api.v1.super_admin.routes import super_admin_force_purge_user

api_v1_bp.add_url_rule(
    "/superadmin/users/<int:user_id>/force-purge",
    "super_admin_force_purge_user_non_hyphen",
    super_admin_force_purge_user,
    methods=["POST", "DELETE"],
)
...
def _sanitize_telemetry_payload(data):
    ...
    for k, v in data.items():
        ...
        if isinstance(v, dict):
            ...
            sanitized[k] = [
                _sanitize_telemetry_payload(item) if isinstance(item, dict) else item
                for item in v
            ]
        else:
            sanitized[k] = v
...
def _ingest_errors():
    ...
    error_msg = (
        sanitized_payload.get("message")
        or sanitized_payload.get("error")
        or "Telemetry error captured"
    )
    _logger.error(
        f"Frontend diagnostic log captured: {error_msg} | Payload: {sanitized_payload}"
    )
...
def register_blueprints(app):
    ...
    app.register_blueprint(
        enhanced_dashboard_bp, url_prefix="/api/v1/enhanced-dashboard"
    )
