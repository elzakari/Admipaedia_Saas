...
from app.models import (Attendance, CoreCompetency, EducationalLevel,
                        EnhancedGrade, FinalGrade, Grade, GradingScheme,
                        Student, StudentCompetencyAssessment,
                        StudentProgression, Subject, Term)
...
def _get_report_card_subject_results(student_id, term, academic_year):
    ...
    final_grades = (
        ...
        .join(FinalGrade, Subject.id == FinalGrade.subject_id)
        .filter(
    ...
    enhanced_grades = (
        ...
        .join(EnhancedGrade, Subject.id == EnhancedGrade.subject_id)
        .filter(
...
def _get_term_gpa(student_id, term, academic_year):
    final_gpa = (
        db.session.query(func.avg(FinalGrade.final_grade_points))
        .filter(
            FinalGrade.student_id == student_id,
            FinalGrade.term == term,
            FinalGrade.academic_year == academic_year,
    ...
    enhanced_gpa = (
        db.session.query(func.avg(EnhancedGrade.grade_points))
        .filter(
            EnhancedGrade.student_id == student_id,
            EnhancedGrade.term == term,
            EnhancedGrade.academic_year == academic_year,
    ...
    grade_gpa = (
        db.session.query(func.avg(Grade.percentage / 25))
        .filter(
            Grade.student_id == student_id,
            Grade.term == term,
            Grade.academic_year == academic_year,
...
def _get_transcript_year_subject_results(student_id, academic_year):
    ...
    return (
        ...
        .join(Grade, Subject.id == Grade.subject_id)
        .filter(Grade.student_id == student_id, Grade.academic_year == academic_year)
        .group_by(Subject.id, Subject.name, Subject.code)
        .all()
    )
...
def _get_class_subject_performance(class_id, term, academic_year, tenant_id=None, branch_id=None):
    ...
    enhanced_query = (
        ...
        .join(EnhancedGrade, Subject.id == EnhancedGrade.subject_id)
        .join(Student, EnhancedGrade.student_id == Student.id)
        .filter(
...
def get_academic_years():
    ...
    return jsonify({"success": True, "data": [y.name for y in years]}), 200
...
def send_student_report_email(student_id):
    ...
    try:
        ...
        if not report_data or not recipient_email:
            return (
                jsonify({"success": False, "message": "Missing report data or email"}),
                400,
            )
        ...
        attachments = [
            (f"report_card_{student_id}.pdf", "application/pdf", pdf_buffer.getvalue())
        ]

        success = send_email(
            subject, [recipient_email], text_body, attachments=attachments
        )

        if success:
            return (
                jsonify({"success": True, "message": "Report card sent successfully"}),
                200,
            )
        else:
...
def get_administrative_report():
    ...
    try:
        ...
        current_app.logger.error(
            f"Error generating administrative report: {error_details}"
        )
        return jsonify({"success": False, "message": str(e)}), 500
...
def generate_custom_report():
    ...
    try:
        ...
        if not config:
            return (
                jsonify({"success": False, "message": "No configuration provided"}),
                400,
            )
...
def export_report():
    ...
    try:
        ...
        if not report_data:
            return (
                jsonify({"success": False, "message": "No report data provided"}),
                400,
            )
        ...
        if format_type == "pdf":
            ...
            return (
                jsonify(
                    {"success": False, "message": f"Unsupported format: {format_type}"}
                ),
                400,
            )
...
def generate_student_report_card(student_id):
    ...
    try:
        ...
        if not academic_year:
            latest_progression = (
                StudentProgression.query.filter_by(student_id=student_id)
                .order_by(StudentProgression.academic_year.desc())
                .first()
            )
        ...
        if not grading_scheme and educational_level:
            grading_scheme = GradingScheme.query.filter_by(
                educational_level_id=educational_level.id, is_active=True
            ).first()
        ...
        tenant_term = AcademicTerm.query.filter_by(
            tenant_id=g.tenant_id, name=term
        ).first()
        if tenant_term:
        ...
        attendance_stats = (
            db.session.query(
                ...
                func.sum(func.cast(Attendance.status == "present", db.Integer)).label(
                    "present_days"
                ),
                func.sum(func.cast(Attendance.status == "absent", db.Integer)).label(
                    "absent_days"
                ),
                func.sum(func.cast(Attendance.status == "late", db.Integer)).label(
                    "late_days"
                ),
            )
            .filter(
        ...
        attendance_rate = (
            round(effective_present / total_days * 100, 1) if total_days > 0 else 0.0
        )
        ...
        competencies = (
            db.session.query(
                ...
                func.avg(StudentCompetencyAssessment.level_achieved).label(
                    "average_level"
                ),
            )
            .join(
                StudentCompetencyAssessment,
                CoreCompetency.id == StudentCompetencyAssessment.competency_id,
            )
            .filter(
                StudentCompetencyAssessment.student_id == student_id,
                StudentCompetencyAssessment.academic_year == academic_year,
            )
            .group_by(CoreCompetency.id, CoreCompetency.name)
            .all()
        )
            ...
            report_competencies = [
                {
                    "name": "Attitude",
                    "level": 4.0,
                    "description": "Shows a positive attitude towards learning. Accepts feedback well and strives to improve.",
                },
                {
                    "name": "Conduct",
                    "level": 4.0,
                    "description": "Respects teachers and classmates. Follows school rules and is responsible.",
                },
                {
                    "name": "Interest",
                    "level": 4.0,
                    "description": "Shows keen interest in technology, science projects, and reading.",
                },
            ]
        else:
            report_competencies = [
        ...
        report_data = {
            "student_info": {
                ...
                "class": getattr(student.class_, "display_name", None)
                or (student.class_.name if student.class_ else "N/A"),
                "educational_level": educational_level.level_name,
                "academic_year": academic_year,
                "term": term,
                "profile_picture": EnhancedStudentService.build_profile_picture_url(
                ...
                "subjects": [
                    {
                        ...
                        "score": (
                            round(grade.average_score, 1) if grade.average_score else 0
                        ),
                        "grade": grade.grade or "N/A",
                        "grade_point": (
                            round(grade.grade_point, 2) if grade.grade_point else 0.0
                        ),
                        "remarks": getattr(grade, "remarks", None)
                        or _get_grade_remarks(grade.grade),
                    }
                    for grade in grades
                ],
                "overall_gpa": gpa,
            ...
            "progression_status": {
                ...
                "promotion_status": (
                    "Promoted"
                    if progression.meets_academic_threshold
                    and progression.meets_attendance_threshold
                    else "Repeat"
                ),
                "next_level": (
                    progression.next_level.level_name
            ...
            "grading_scheme": {
                ...
                "scale": (
                    grading_scheme.standard.value
                    if grading_scheme and grading_scheme.standard
                    else "Ghana Education Service"
                ),
            },
        }
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error generating report card: {str(e)}"}
            ),
            500,
        )
...
def generate_student_transcript(student_id):
    ...
    try:
        ...
        progressions = (
            StudentProgression.query.filter_by(student_id=student_id)
            .order_by(StudentProgression.academic_year)
            .all()
        )
        ...
        transcript_data = {
            "student_info": {
                ...
                "graduation_date": (
                    progressions[-1].academic_year if progressions else "In Progress"
                ),
            },
            "academic_record": [],
            "overall_performance": {
        ...
        for progression in progressions:
            ...
            year_grades = _get_transcript_year_subject_results(
                student_id, progression.academic_year
            )

            year_gpa = (
                sum([grade.grade_point or 0 for grade in year_grades])
                / len(year_grades)
                if year_grades
                else 0
            )
            ...
            transcript_data["academic_record"].append(
                {
                    ...
                    "subjects": [
                        {
                            ...
                            "score": (
                                round(grade.average_score, 1)
                                if grade.average_score
                                else 0
                            ),
                            "grade": grade.grade or "N/A",
                            "grade_point": (
                                round(grade.grade_point, 2)
                    ...
                    "promotion_status": (
                        "Promoted" if progression.meets_academic_threshold else "Repeat"
                    ),
                }
            )

        # Calculate cumulative GPA
        transcript_data["overall_performance"]["cumulative_gpa"] = (
            round(total_gpa_points / total_years, 2) if total_years > 0 else 0.0
        )
        transcript_data["overall_performance"]["total_credits"] = sum(
            [len(year["subjects"]) for year in transcript_data["academic_record"]]
        )

        if format_type == "pdf":
            return _generate_pdf_transcript(transcript_data)
        else:
            return jsonify({"success": True, "data": transcript_data}), 200

    except Exception as e:
        return (
            jsonify(
                {"success": False, "message": f"Error generating transcript: {str(e)}"}
            ),
            500,
        )
...
def generate_class_performance_summary(class_id):
    ...
    try:
        ...
        if not students:
            return (
                jsonify(
                    {"success": False, "message": "No students found in this class"}
                ),
                404,
            )
        ...
        summary_data = {
            ...
            "subject_performance": [
                {
                    ...
                    "class_average": (
                        round(grade.class_average, 1) if grade.class_average else 0
                    ),
            ...
            "attendance_summary": {
                "average_attendance_rate": (
                    round(attendance_summary.average_attendance, 1)
                    if attendance_summary.average_attendance
                    else 0
                )
            },
        }
