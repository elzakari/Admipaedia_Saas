    ...
    if request.method == "DELETE":
        ...
        if not success:
            return jsonify({"success": False, "message": error}), (
                404 if error == "Slot not found" else 400
            )
...
def auto_schedule():
    data = request.json
    count, errors = TimetableService.auto_schedule_greedy(
        data.get("term"), data.get("academic_year")
    )
    return jsonify({"success": True, "allocated_slots": count, "errors": errors}), 200
