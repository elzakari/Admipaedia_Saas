...
from app.models.staff_enhanced import LeaveStatus, LeaveType, StaffLeave
...
def process_leave(id):
    ...
    leave, error = StaffService.process_leave(
        id, data.get("status"), approver_id, data.get("reason")
    )
    if error:
        return jsonify({"success": False, "message": error}), 400
        ...
        jsonify(
            {
                "success": True,
                "data": [
                    {"id": t.id, "name": t.name, "days": t.days_allowed} for t in types
                ],
            }
        ),
        200,
