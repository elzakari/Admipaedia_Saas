...
def get_grading_schemes(educational_level_id):
    ...
    try:
        schemes = (
            ...
            .order_by(GradingScheme.is_default.desc(), GradingScheme.name.asc())
            .all()
        )
        ...
        for scheme in schemes:
            boundaries = (
                GradeBoundary.query.filter_by(grading_scheme_id=scheme.id)
                .order_by(GradeBoundary.sequence_order.asc())
                .all()
            )
            result.append(
                {
...
def calculate_final_grade(student_id):
    ...
    try:
        ...
        final_grade, error = EnhancedGradingService.calculate_final_grade(
            ...
            academic_year=data["academic_year"],
            external_exam_score=(
                float(data["external_score"])
                if data.get("external_score") is not None
                else None
            ),
            grading_scheme_id=int(data["grading_scheme_id"]),
