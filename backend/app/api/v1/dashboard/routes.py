                ...
                result.append(
                    {
                        ...
                        "change": (
                            ...
                            if getattr(stat, "change_value", None) is not None
                            else None
                        ),
...
def get_dashboard_data():
    ...
    try:
                    ...
                    formatted_notifications.append(
                        {
                            ...
                            "time": format_time_ago(
                                getattr(n, "time", None) or datetime.utcnow()
                            ),
        ...
        return jsonify(
            {
                ...
                "quick_actions": getattr(
                    DashboardService, "get_quick_actions", lambda r: []
                )(role),
            }
        )
    except Exception as e:
        ...
        return jsonify(
            {"statistics": [], "notifications": [], "events": [], "quick_actions": []}
        )
...
def get_notifications():
    ...
    try:
        ...
        notifications = (
            DashboardService.get_notifications(user_id, limit, start_date, end_date)
            or []
        )
                ...
                result.append(
                    {
                        ...
                        "time": format_time_ago(
                            getattr(notification, "time", None) or datetime.utcnow()
                        ),
    ...
    if timestamp.tzinfo is None:
        try:
            timestamp = timestamp.replace(
                tzinfo=None
            )  # Keep it naive for comparison if now is naive
        except:
            pass
...
def create_notification():
    ...
    for field in required_fields:
        if field not in data or data[field] is None or str(data[field]).strip() == "":
            return (
                jsonify({"error": f"Missing or invalid required field: {field}"}),
                400,
            )
        ...
        if "notifications" in data and isinstance(data["notifications"], list):
            ...
            for notif in data["notifications"]:
                ...
                try:
                    ...
                    logger.error(
                        f"Error in batch bulk notification item: {str(notif_err)}"
                    )
...
def get_admin_dashboard_metrics():
    ...
    try:
        ...
        try:
            ...
            average_grade = (
                round(float(avg_grade_val), 1) if avg_grade_val is not None else 0.0
            )
            ...
            logger.error(
                f"Error calculating grades in get_admin_dashboard_metrics: {str(e)}"
            )
            average_grade = 0.0
            pass_rate = 0.0
            grade_distribution = []
        ...
        try:
            ...
            logger.error(
                f"Error calculating attendance in get_admin_dashboard_metrics: {str(e)}"
            )
            attendance_rate = 0.0
        ...
        try:
            submitted_count = (
                db.session.query(func.count(AssignmentSubmission.id))
            ...
            assignment_completion_rate = (
                round((submitted_count / total_submissions * 100), 1)
                if total_submissions > 0
                else 0.0
            )
        except Exception as e:
            logger.error(
                f"Error calculating homework completion in get_admin_dashboard_metrics: {str(e)}"
            )
        ...
        try:
            ...
            parent_count = (
                db.session.query(func.count(Parent.id))
            ...
            logger.error(
                f"Error calculating system stats in get_admin_dashboard_metrics: {str(e)}"
            )
        ...
        try:
            ...
            for year, month, name in months_list:
                m_grades = (
                    ...
                    .filter(func.extract("year", Grade.created_at) == year)
                    .scalar()
                )
                ...
                m_att_rate = (
                    (m_att_present / m_att_total * 100) if m_att_total > 0 else None
                )
                ...
                m_sub_done = (
                    ...
                    .filter(AssignmentSubmission.status.in_(["submitted", "graded"]))
                    .filter(
                        func.extract("month", AssignmentSubmission.submission_date)
                        == month
                    )
                    .filter(
                        func.extract("year", AssignmentSubmission.submission_date)
                        == year
                    )
                    .scalar()
                    or 0
                ...
                m_sub_rate = (
                    (m_sub_done / m_sub_total * 100) if m_sub_total > 0 else None
                )

                if (
                    m_grades is not None
                    or m_att_rate is not None
                    or m_sub_rate is not None
                ):
                    ...
                    monthly_trends.append(
                        {
                            "month": name,
                            "performance": (
                                round(float(m_grades), 1)
                                if m_grades is not None
                                else 0.0
                            ),
                            "attendance": (
                                round(float(m_att_rate), 1)
                                if m_att_rate is not None
                                else 0.0
                            ),
                            "assignments": (
                                round(float(m_sub_rate), 1)
                                if m_sub_rate is not None
                                else 0.0
                            ),
                        }
                    )
            ...
            logger.error(
                f"Error calculating monthly trends in get_admin_dashboard_metrics: {str(e)}"
            )
            monthly_trends = []
        ...
        try:
            ...
            active_sessions_total = (
                db.session.query(func.count(SessionToken.id))
                .join(
                    TenantMembership, TenantMembership.user_id == SessionToken.user_id
                )
                ...
                .filter(SessionToken.expires_at > datetime.utcnow())
                .scalar()
                or 0
            )
        except Exception as e:
        ...
        try:
            ...
            if depts:
                for dept in depts:
                    ...
                    teachers_count = Teacher.query.filter_by(
                        tenant_id=tenant_id, department_id=dept.id
                    ).count()
                    ...
                    performance = (
                        round(float(dept_avg_grade), 1)
                        if dept_avg_grade is not None
                        else 0.0
                    )
    ...
    try:
        ...
        if subjects:
            for index, subj in enumerate(subjects):
                ...
                average_score = (
                    round(float(avg_score_val), 1) if avg_score_val is not None else 0.0
                )
                ...
                teacher_query = (
                    db.session.query(
                        func.count(func.distinct(teacher_subjects.c.teacher_id))
                    )
                ...
                difficulty = (
                    "Hard"
                    if average_score < 50
                    else "Medium" if average_score < 75 else "Easy"
                )
                improvement = 0.0
