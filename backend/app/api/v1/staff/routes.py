...
from app.schemas.staff import StaffListSchema, StaffSchema
...
def _serialize_staff(staff):
    department_info = (
        StaffService._get_department_lookup(
            [getattr(staff, "user_id", None)], tenant_id=getattr(g, "tenant_id", None)
        )
    ...
    return {
        ...
        "joining_date": (
            ...
            if getattr(staff, "joining_date", None)
            else None
        ),
        "status": staff.status,
        ...
        "created_at": (
            staff.created_at.isoformat() if getattr(staff, "created_at", None) else None
        ),
        "updated_at": (
            staff.updated_at.isoformat() if getattr(staff, "updated_at", None) else None
        ),
    }
...
def assign_department(staff_id: int):
    ...
    ok, error = StaffService.assign_department(
        staff_id=staff_id, department_id=department_id, role=role, tenant_id=g.tenant_id
    )
    if not ok:
    ...
    return (
        jsonify(
            {"success": True, "message": "Staff assigned to department successfully"}
        ),
        200,
    )
...
def list_staff():
    ...
    paginated = StaffService.list_staff(
        page=page, per_page=per_page, search=search, tenant_id=g.tenant_id
    )
                ...
                "summary": {
                    ...
                    "teachers": len(
                        [
                            ...
                            if row.get("entity_type") == "teacher"
                        ]
                    ),
                    "staff": len(
                        [row for row in directory if row.get("entity_type") == "staff"]
                    ),
                    "active": len(
                        [
                            row
...
def get_staff_attendance(staff_id: int):
    ...
    start_date = (
        date.fromisoformat(start_value) if start_value else date.today().replace(day=1)
    )
    end_date = date.fromisoformat(end_value) if end_value else date.today()
    records, error = StaffService.get_staff_attendance(
        staff_id, start_date, end_date, tenant_id=g.tenant_id
    )
    if error:
...
def mark_staff_attendance(staff_id: int):
    ...
    if not attendance_date or not status:
        return (
            jsonify({"success": False, "message": "date and status are required"}),
            400,
        )
