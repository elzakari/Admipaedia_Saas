...
def get_core_competencies():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error fetching competencies: {str(e)}"}
            ),
            500,
        )
...
def create_core_competency():
    ...
    if CoreCompetency.query.filter(
        func.lower(CoreCompetency.name) == name.lower()
    ).first():
        return (
            jsonify(
                {
    ...
    return (
        jsonify(
            {"success": True, "competency": _serialize_core_competency(competency)}
        ),
        201,
    )
...
def update_core_competency(competency_id):
    ...
    if name is not None:
        ...
        exists = CoreCompetency.query.filter(
            func.lower(CoreCompetency.name) == name.lower(),
            CoreCompetency.id != competency.id,
        ).first()
        if exists:
            return (
                jsonify(
    ...
    if category is not None:
        ...
        if not category:
            return (
                jsonify({"success": False, "message": "category cannot be empty"}),
                400,
            )
...
@competencies_bp.route(
    "/core-competencies/<int:competency_id>/indicators", methods=["GET"]
)
@jwt_required()
def get_competency_indicators(competency_id):
    ...
    try:
        ...
        indicators = CompetencyIndicator.query.filter_by(
            competency_id=competency_id, is_active=True
        ).all()
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error fetching indicators: {str(e)}"}
            ),
            500,
        )
...
def get_student_competency_profile(student_id):
    ...
    try:
        ...
        profile_data = {
            ...
            "communication_collaboration_score": profile.communication_collaboration_score
            or 0.0,
            ...
            "overall_competency_level": (
                profile.overall_competency_level.value
...
@competencies_bp.route(
    "/students/<int:student_id>/competency-assessments", methods=["GET"]
)
@jwt_required()
def get_student_competency_assessments(student_id):
    ...
    try:
        ...
        assessments = query.order_by(
            StudentCompetencyAssessment.assessment_date.desc()
        ).all()
            ...
            assessments_data.append(
                {
                    ...
                    "competency_name": (
                        assessment.competency.name if assessment.competency else None
                    ),
                }
            )

        return jsonify({"success": True, "assessments": assessments_data}), 200

    except Exception as e:
        return (
            jsonify(
                {"success": False, "message": f"Error fetching assessments: {str(e)}"}
            ),
            500,
        )
...
def create_competency_assessment():
    try:
        ...
        if (
            not isinstance(competency_id, int)
            or CoreCompetency.query.get(competency_id) is None
        ):
            errors["competency_id"] = "Invalid competency_id"
        if (
            level_val is None
            or not isinstance(level_val, int)
            or level_val < 1
            or level_val > 4
        ):
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error creating assessment: {str(e)}"}
            ),
            500,
        )
...
def get_class_competency_dashboard(class_id):
    ...
    try:
        ...
        if not student_ids:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Class not found or has no active students",
                    }
                ),
                404,
            )
            ...
            student_progress.append(
                {
                    ...
                    "communication_collaboration_score": (
                        profile.communication_collaboration_score if profile else 0.0
                    )
                    or 0.0,
                    "critical_thinking_score": (
                        profile.critical_thinking_score if profile else 0.0
                    )
                    or 0.0,
                    "creativity_innovation_score": (
                        profile.creativity_innovation_score if profile else 0.0
                    )
                    or 0.0,
                    "cultural_identity_score": (
                        profile.cultural_identity_score if profile else 0.0
                    )
                    or 0.0,
                    "personal_development_score": (
                        profile.personal_development_score if profile else 0.0
                    )
                    or 0.0,
                    "digital_literacy_score": (
                        profile.digital_literacy_score if profile else 0.0
                    )
                    or 0.0,
        ...
        sorted_students = sorted(
            student_progress, key=lambda x: x["overall_score"], reverse=True
        )
        top_performers = sorted_students[:5]  # Top 5 performers
        improvement_needed = [
            s for s in sorted_students if s["overall_score"] < 2.0
        ]  # Below developing level
...
def update_student_competency_profile(student_id, academic_year):
    ...
    try:
        ...
        all_scores = [
            getattr(profile, f"{domain}_score") or 0.0
