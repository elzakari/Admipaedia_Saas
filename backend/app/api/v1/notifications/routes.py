...
def _build_notification_payload(notification, user_id, state):
    ...
    target_scope = (
        getattr(notification, "scope", None)
        or getattr(notification, "target_role", None)
        or "all"
    )
    created_at = getattr(notification, "created_at", None) or getattr(
    ...
    is_read = (
        ...
        else (bool(getattr(notification, "read", False)) if is_direct else False)
    )
    read_at = state.read_at if state is not None else None
    if is_read and read_at is None:
        read_at = created_at

    db_attachments = Attachment.query.filter_by(
        entity_type="notification", entity_id=str(notification.id)
    ).all()
    ...
    return {
        ...
        "time": (
            ...
            if getattr(notification, "time", None)
            else None
        ),
...
def _get_visible_notifications_by_ids(
    notification_model, notification_ids, user, user_role
):
    if not notification_ids:
        return []

    visibility_conditions = _build_notification_visibility_conditions(
        notification_model, user, user_role
    )
    return notification_model.query.filter(
        notification_model.id.in_(notification_ids), or_(*visibility_conditions)
    ).all()
...
def _update_notification_states(db, user, notification_ids, updater):
    ...
    visible_notifications = _get_visible_notifications_by_ids(
        Notification, notification_ids, user, user_role
    )
    if not visible_notifications:
        return 0

    state_map = _get_or_create_notification_states(
        db, [notification.id for notification in visible_notifications], user.id
    )
    now = datetime.utcnow()
...
def get_notifications():
    try:
        ...
        try:
            or_conditions = _build_notification_visibility_conditions(
                Notification, user, user_role
            )
        ...
        state_map = _get_notification_states(
            user.id, [notification.id for notification in notifications]
        )
        serialized = []
        for n in notifications:
            n_recipient_id = getattr(n, "recipient_id", None) or getattr(
                n, "user_id", None
            )
            n_target_role = (
                getattr(n, "scope", None) or getattr(n, "target_role", None) or "all"
            )

            matches_recipient = n_recipient_id == user.id
            matches_role = n_recipient_id is None and (
                n_target_role == user_role
                or n_target_role == f"{user_role}s"
                or n_target_role == "all"
            )
        ...
        return (
            jsonify({"success": True, "data": serialized[offset : offset + limit]}),
            200,
        )
...
def mark_read():
    ...
    if not notification_ids:
        return (
            jsonify({"success": False, "message": "No notification_ids provided"}),
            400,
        )
