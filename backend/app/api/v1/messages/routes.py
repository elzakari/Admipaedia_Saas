...
from app.schemas.message import (MessageCreateSchema, MessageSchema,
                                 MessageUpdateSchema)
...
def get_recipients():
    ...
    try:
        ...
        type_filter = request.args.get(
            "type"
        )  # parent | student | teacher | admin | class
        ...
        if not type_filter:
            return error_response(
                message="Missing required parameter: type", status_code=400
            )
        ...
        if type_filter == "class":
            for c in results:
                audiences_to_add = (
                    [audience] if audience else ["all", "students", "parents"]
                )
                ...
                recipients.append(
                    {
                        ...
                        "user_id": s.user_id,
                        "participant_avatar": EnhancedStudentService.build_profile_picture_url(
                            s.profile_picture
                        ),
                        "avatar_initials": f"{s.first_name[0] if s.first_name else ''}{s.last_name[0] if s.last_name else ''}".upper()
                        or "S",
                    }
                )
            ...
            for p in results:
                ...
                subtitle = (
                    f"Parent of {', '.join(child_names)}" if child_names else "Parent"
                )
                recipients.append(
                    {
                        "ref": f"parent:{p.id}",
                        ...
                        "participant_avatar": normalize_avatar_url_for_response(
                            getattr(
                                getattr(getattr(p, "user", None), "profile", None),
                                "avatar_url",
                                None,
                            )
                        ),
                        "avatar_initials": f"{display_name[0] if display_name else ''}".upper()
                        or "P",
                    }
                )
                ...
                recipients.append(
                    {
                        ...
                        "participant_avatar": normalize_avatar_url_for_response(
                            getattr(
                                getattr(getattr(t, "user", None), "profile", None),
                                "avatar_url",
                                None,
                            )
                        ),
                        "avatar_initials": f"{t.first_name[0] if t.first_name else ''}{t.last_name[0] if t.last_name else ''}".upper()
                        or "T",
                    }
                )
        ...
        return success_response(
            data={"recipients": recipients}, message="Recipients retrieved successfully"
        )
...
def get_message(message_id):
    ...
    try:
        ...
        message = MessageService.get_message_by_id(
            message_id, current_user_id, tenant_id=g.tenant_id
        )
        if not message:
        ...
        return success_response(
            data=message_schema.dump(message), message="Message retrieved successfully"
        )
    except Exception as e:
...
def create_message():
    ...
    try:
        ...
        try:
            ...
            return error_response(
                message="Validation error", errors=err.messages, status_code=400
            )
...
def update_message(message_id):
    ...
    try:
        ...
        message = MessageService.update_message(
            message_id, current_user_id, validated_data, tenant_id=g.tenant_id
        )
        if not message:
            return error_response(
                message="Message not found or access denied", status_code=404
            )

        return success_response(
            data=message_schema.dump(message), message="Message updated successfully"
        )
    except Exception as e:
...
def delete_message(message_id):
    ...
    try:
        ...
        success = MessageService.delete_message(
            message_id, current_user_id, permanent, tenant_id=g.tenant_id
        )
        if not success:
            return error_response(
                message="Message not found or access denied", status_code=404
