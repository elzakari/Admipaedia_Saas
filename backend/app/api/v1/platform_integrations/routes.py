...
def _merge_secrets(existing: dict | None, incoming: dict | None) -> dict | None:
    ...
    for k, v in incoming.items():
        ...
        if (
            k in ("smtpPassword", "smtp_password")
            and isinstance(merged.get(k), str)
...
def _validate_provider_config(
    service_type: str, provider_key: str, config: dict | None
) -> str | None:
    cfg = dict(config or {})
    ...
    if st == "email":
        ...
        if pk in ("smtp", "ses_smtp"):
            host = _sanitize_smtp_host(
                str(cfg.get("smtpHost") or cfg.get("smtp_host") or "")
            )
            if not host:
            ...
            username = str(
                cfg.get("smtpUsername") or cfg.get("smtp_username") or ""
            ).strip()
            if not username:
            ...
            if not _has_secret(
                cfg.get("awsSecretAccessKey") or cfg.get("aws_secret_key")
            ):
...
def _test_smtp_email(cfg: dict, params: dict) -> tuple[bool, str]:
    ...
    password = cfg.get("smtpPassword") or cfg.get("smtp_password")
    enc = (
        str(cfg.get("smtpEncryption") or cfg.get("smtp_encryption") or "tls")
        .strip()
        .lower()
    )
    from_email = str(cfg.get("fromEmail") or cfg.get("from_email") or "").strip()
    ...
    to_email = str(
        params.get("to_email")
        or params.get("toEmail")
        or params.get("test_email")
        or params.get("testEmail")
        or ""
    ).strip()
    subject = str(params.get("subject") or "ADMIPAEDIA test email").strip()
    message = str(
        params.get("message") or "This is a test message from ADMIPAEDIA."
    ).strip()
    ...
    try:
        ...
        return (
            False,
            f"SMTP authentication failed for user {masked_user}. Please check your username and password.",
        )
        ...
        return (
            False,
            "SMTP connection timed out. Please verify host, port, and security settings.",
        )
    except socket.gaierror:
        used_host = cfg.get("smtpHost") or cfg.get("smtp_host") or ""
        return (
            False,
            f"DNS resolution failed for host {used_host}. Please verify the SMTP host.",
        )
...
def _test_ses_api_email(cfg: dict, params: dict) -> tuple[bool, str, str]:
    try:
        ...
        to_email = (
            params.get("to_email")
            or params.get("toEmail")
            or "support@admipaedia.easymsdigit.com"
        )
...
def list_provider_configs():
    ...
    include_overrides = str(request.args.get("include_overrides") or "").lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    providers = providers_q.order_by(
        PlatformServiceProviderConfig.service_type.asc(),
        PlatformServiceProviderConfig.priority.asc(),
    ).all()
    ...
    if include_overrides and tenant_id:
        ...
        overrides = ov_q.order_by(
            TenantServiceProviderOverride.service_type.asc(),
            TenantServiceProviderOverride.priority.asc(),
        ).all()
    ...
    for p in providers:
        ...
        try:
            ...
            if (
                p_key == "smtp"
                and p_cfg.get("smtpHost")
                and "amazonaws.com" in str(p_cfg.get("smtpHost")).lower()
            ):
                p_key = "ses_smtp"
            ...
            warning = (
                "Provider settings cannot be decrypted. Please re-save this provider."
            )
            redacted_config = {}
    ...
    for o in overrides:
        ...
        try:
            ...
            if (
                o_key == "smtp"
                and o_cfg.get("smtpHost")
                and "amazonaws.com" in str(o_cfg.get("smtpHost")).lower()
            ):
                o_key = "ses_smtp"
    ...
    return (
        jsonify(
            {"success": True, "providers": providers_list, "overrides": overrides_list}
        ),
        200,
    )
...
def upsert_provider_config():
    ...
    normalized_config = _normalize_provider_config(
        service_type, provider_key, config if isinstance(config, dict) else {}
    )
    validation_error = _validate_provider_config(
        service_type, provider_key, normalized_config
    )
    if validation_error:
        ...
        if not tenant_id:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "tenant_id is required for tenant scope",
                    }
                ),
                400,
            )
        row = TenantServiceProviderOverride.query.filter_by(
            tenant_id=tenant_id, service_type=service_type, provider_key=provider_key
        ).first()
        if not row:
    ...
    row = PlatformServiceProviderConfig.query.filter_by(
        service_type=service_type, provider_key=provider_key
    ).first()
    if not row:
        row = PlatformServiceProviderConfig(
            service_type=service_type, provider_key=provider_key
        )
        db.session.add(row)
    row.display_name = display_name
...
def list_plan_token_limits():
    ...
    limit_keys = [
        "tokens.email.monthly",
        "tokens.sms.monthly",
        "tokens.whatsapp.monthly",
        "tokens.ai.monthly",
    ]
    limits = PlanLimit.query.filter(PlanLimit.limit_key.in_(limit_keys)).all()
    by_plan = {}
    for l in limits:
        by_plan.setdefault(l.plan_id, {})[l.limit_key] = {
            "value": l.limit_value,
...
def update_plan_token_limits(plan_id: int):
    ...
    for key, value in limits.items():
        ...
        row.value_type = (
            "string" if str(value).lower() in ("unlimited", "contracted") else "number"
        )
...
def test_provider_config():
    ...
    if decryption_failed:
        ...
        current_app.logger.error(
            f"Decryption failed for email provider integration configuration test. Key: {actual_provider_key}"
        )
        return (
            jsonify(
                {
    ...
    test_unsaved_config = str(
        data.get("test_unsaved_config") or data.get("testUnsavedConfig") or ""
    ).lower() in ("1", "true", "yes")
    ...
    if service_type == "email":
        ...
        if service_type == "sms":
            if not _has_secret(
                cfg.get("smsApiKey") or cfg.get("apiKey") or cfg.get("api_key")
            ):
