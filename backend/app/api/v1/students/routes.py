...
def get_student_dashboard():
    ...
    total_att = Attendance.query.filter(
        Attendance.student_id == student.id, Attendance.date >= since
    ).count()
    ...
    avg_grade = (
        ...
        .filter(Grade.student_id == student.id, Grade.created_at >= since)
        .scalar()
    )
    ...
    if getattr(student, "class_id", None):
        exams = (
            Exam.query.filter(Exam.class_id == student.class_id, Exam.exam_date >= now)
    ...
    unread_notifications = sum(
        1 for n in notifications if not getattr(n, "read", False)
    )
...
@students_bp.route("/", methods=["GET"])  # This will match /students/
@jwt_required()
@require_role(["admin", "school_admin", "teacher", "super_admin", "super_manager"])
@tenant_required
def get_students():
    ...
    try:
            ...
            jsonify(
                {
                    "success": True,
                    "students": (
                        students_schema.dump(paginated_students.items)
                    ...
                    "pagination": {
                        ...
                        "page": page_val,
                        "total_pages": (
                            paginated_students.pages if paginated_students else 0
                        ),
                        "current_page": page_val,
                        "per_page": (
                            paginated_students.per_page
                            if paginated_students
                            else per_page
                        ),
                        "next": (
                            paginated_students.next_num if paginated_students else None
                        ),
                        "prev": (
                            paginated_students.prev_num if paginated_students else None
                        ),
                    },
                }
            ),
        ...
        return jsonify({"success": True, "students": [], "total": 0, "page": 1}), 200
...
def create_student():
    ...
    try:
        ...
        if "user_id" not in payload:
            ...
            email = (
                payload.get("email")
                or f"student_{datetime.now().year}_{get_jwt_identity()}@example.com"
            )
        ...
        if existing_student:
            current_app.logger.info(
                f"Attempted to create duplicate student profile",
                extra={
        ...
        if data.get("admission_number"):
            ...
            if existing_adm:
                current_app.logger.info(
                    f"Attempted to create student with existing admission number {data['admission_number']}"
                )
                data["admission_number"] = (
                    f"{data['admission_number']}-{datetime.now().strftime('%H%M%S')}"
                )
        try:
    ...
    try:
        ...
        if error:
            current_app.logger.warning(
                f"Failed to update student {student_id}: {error}"
            )
            return jsonify({"success": False, "message": error}), 400
        ...
        current_app.logger.error(
            f"Unexpected error updating student {student_id}: {str(e)}"
        )
        return (
            jsonify(
                {
...
def delete_student(student_id):
    ...
    try:
        ...
        if error:
            current_app.logger.warning(
                f"Failed to delete student {student_id}: {error}"
            )
            return jsonify({"success": False, "message": error}), 400

        return (
            jsonify({"success": True, "message": "Student deleted successfully"}),
            200,
        )
    except Exception as e:
        current_app.logger.error(
            f"Unexpected error deleting student {student_id}: {str(e)}"
        )
        return (
            jsonify(
                {
...
def assign_class(student_id):
    ...
    try:
        ...
        current_app.logger.debug(
            f"Assign class request for student {student_id}: {request.json}"
        )
        ...
        if class_id is None:
            current_app.logger.warning(
                f"Missing class_id in assign class request for student {student_id}"
            )
        ...
        if error:
            current_app.logger.warning(
                f"Failed to assign student {student_id} to class {class_id}: {error}"
            )
            return jsonify({"success": False, "message": error}), 400
        ...
        current_app.logger.warning(
            f"Validation error assigning student {student_id} to class: {err.messages}"
        )
        return jsonify({"success": False, "errors": err.messages}), 400
    except Exception as e:
        current_app.logger.error(
            f"Unexpected error assigning student {student_id} to class: {str(e)}"
        )
        return (
            jsonify(
                {
...
def link_student_to_parent(student_id):
    ...
    try:
        # Log the incoming request data
        current_app.logger.debug(
            f"Link parent request for student {student_id}: {request.json}"
        )
        ...
        if parent_id is None:
            current_app.logger.warning(
                f"Missing parent_id in link parent request for student {student_id}"
            )
        ...
        student, error = EnhancedStudentService.link_student_to_parent(
            student_id, parent_id
        )

        if error:
            current_app.logger.warning(
                f"Failed to link student {student_id} to parent {parent_id}: {error}"
            )
            return jsonify({"success": False, "message": error}), 400
        ...
        current_app.logger.error(
            f"Unexpected error linking student {student_id} to parent: {str(e)}"
        )
        return (
            jsonify(
                {
...
def get_students_by_parent(parent_id):
    ...
    try:
        ...
        if error:
            current_app.logger.warning(
                f"Failed to get students for parent {parent_id}: {error}"
            )
            return jsonify({"success": False, "message": error}), 400

        return (
            jsonify({"success": True, "students": students_schema.dump(students)}),
            200,
        )
    except Exception as e:
        current_app.logger.error(
            f"Unexpected error getting students for parent {parent_id}: {str(e)}"
        )
        return (
            jsonify(
                {
...
def generate_activation_link(student_id):
    ...
    try:
        ...
        if not user:
            return (
                jsonify(
                    {"success": False, "message": "Associated user account not found"}
                ),
                404,
            )
