...
def update_setting():
    ...
    if setting_type == "int":
        ...
        store[key] = (
            ...
            else str(value).lower() in ("true", "1", "t", "y", "yes")
        )
        ...
        if isinstance(store, dict):
            ...
            try:
                if v is not None and str(v).strip() != "":
                    return (
                        jsonify(
                            {"success": True, "price": float(v), "currency": currency}
                        ),
                        200,
                    )
            except Exception:
...
def _set_setting(
    key: str, value, setting_type: str = "string", description: str | None = None
):
    if setting_type == "json":
...
def _get_scoped_setting(
    tenant_id, key: str, default=None, setting_type: str = "string"
):
    if tenant_id:
...
def _set_scoped_setting(tenant_id, key: str, value, setting_type: str = "string"):
    if tenant_id:
        ...
        if not tenant:
            return None
        store[key] = _coerce_setting(
            value, setting_type, "" if setting_type == "string" else value
        )
        tenant.settings = store
        db.session.commit()
        return True
...
def get_general_settings_v2():
    if Tenant.query.first() is None:
        return (
            jsonify(
                {"theme": "light", "language": "en", "school_name": "ADMIPAEDIA System"}
            ),
            200,
        )
    ...
    school_code = _get_from_settings(
        "schoolCode", "school_code", "code", default=(getattr(tenant, "slug", "") or "")
    )
    ...
    result = {
        ...
        "schoolEmail": _get_from_settings(
            "schoolEmail", "school_email", "email", default=""
        ),
        "schoolPhone": _get_from_settings(
            "schoolPhone", "school_phone", "phone", default=""
        ),
        "timezone": getattr(tenant, "timezone", None)
        or _get_from_settings("timezone", default="Africa/Accra"),
        "language": getattr(tenant, "default_language", None)
        or _get_from_settings("language", default="en"),
        "currency": getattr(tenant, "currency", None)
        or _get_from_settings("currency", default="GHS"),
        "dateFormat": _get_from_settings(
            "dateFormat", "date_format", default="dd/mm/yyyy"
        ),
...
def update_general_settings_v2():
    ...
    if (
        ...
        if not membership or membership.role != "school_admin":
            return (
                jsonify(
                    {"success": False, "message": "School Admin privileges required"}
                ),
                403,
            )
    ...
    if (
        "schoolName" in payload
        and isinstance(payload.get("schoolName"), str)
        and payload.get("schoolName").strip()
    ):
...
def get_notification_settings_v2():
    ...
    if tenant_id is not None:
        ...
        if tenant:
            ...
            if isinstance(store, dict):
                defaults["fromEmail"] = (
                    store.get("schoolEmail")
                    or store.get("school_email")
                    or defaults["fromEmail"]
                )
    ...
    for k, d in defaults.items():
        if k in ints:
            ...
            result[k] = _get_scoped_setting(
                tenant_id, f"notifications.{k}", d, "boolean"
            )
        else:
            result[k] = _get_scoped_setting(
                tenant_id, f"notifications.{k}", d, "string"
            )
    return jsonify(result), 200
    ...
    if getattr(actor, "role", None) not in ("super_admin", "super_manager"):
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Email provider configuration is managed by the platform.",
                }
            ),
            403,
        )
    ...
    if getattr(actor, "role", None) not in ("super_admin", "super_manager"):
        return (
            jsonify(
                {
                    "success": False,
                    "message": "SMS provider configuration is managed by the platform.",
                }
            ),
            403,
        )
...
def get_backup_settings_v2():
    ...
    result = {
        "backupEnabled": _get_scoped_setting(
            tenant_id, "backup.backupEnabled", defaults["backupEnabled"], "boolean"
        ),
        "backupFrequency": _get_scoped_setting(
            tenant_id, "backup.backupFrequency", defaults["backupFrequency"], "string"
        ),
        "backupTime": _get_scoped_setting(
            tenant_id, "backup.backupTime", defaults["backupTime"], "string"
        ),
        "retainDays": _get_scoped_setting(
            tenant_id, "backup.retainDays", defaults["retainDays"], "int"
        ),
        "includeUploads": _get_scoped_setting(
            tenant_id, "backup.includeUploads", defaults["includeUploads"], "boolean"
        ),
    }
    return jsonify(result), 200
    ...
    if getattr(user, "role", None) not in ("super_admin", "super_manager"):
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Integration configuration is managed by the platform.",
                }
            ),
            403,
        )
...
def get_theme_settings_v2():
    ...
    defaults = {"theme": "system", "primaryColor": "#3B82F6", "reducedMotion": False}
    result = {
        "theme": _get_setting(
            f"user.{user_id}.theme.theme", defaults["theme"], "string"
        ),
        "primaryColor": _get_setting(
            f"user.{user_id}.theme.primaryColor", defaults["primaryColor"], "string"
        ),
        "reducedMotion": _get_setting(
            f"user.{user_id}.theme.reducedMotion", defaults["reducedMotion"], "boolean"
        ),
    }
    return jsonify(result), 200
    ...
    if getattr(user, "role", None) not in ("super_admin", "super_manager"):
        return (
            jsonify(
                {
                    "success": False,
                    "message": "AI provider configuration is managed by the platform.",
                }
            ),
            403,
        )
...
def get_audit_filter_options_v2():
    return jsonify({"categories": [], "actions": [], "resources": []}), 200
...
def get_assessment_categories():
    ...
    active_types = [
        t for t in assessment_types if isinstance(t, dict) and t.get("isActive", True)
    ]
