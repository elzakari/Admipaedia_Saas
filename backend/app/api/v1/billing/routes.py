...
def list_payment_gateways():
    ...
    return (
        jsonify(
            {
                "success": True,
                "gateways": [PaymentService.serialize_gateway(g) for g in gateways],
            }
        ),
        200,
    )
...
def create_payment_gateway():
    ...
    _audit_gateway_change(
        "super_admin.payment_gateway_created",
        gw,
        {
    ...
    return (
        jsonify({"success": True, "gateway": PaymentService.serialize_gateway(gw)}),
        201,
    )
...
def update_payment_gateway(gateway_id: int):
    ...
    _audit_gateway_change(
        "super_admin.payment_gateway_updated",
        gw,
        {
            "gateway_id": int(gw.id),
            "gateway_name": gw.name,
...
def platform_list_payments():
    ...
    return (
        jsonify(
            {
                "success": True,
                "payments": [PaymentService.serialize_payment(p) for p in items],
            }
        ),
        200,
    )
...
def platform_verify_payment(payment_id: int):
    ...
    if err or not p:
        ...
        return jsonify({"success": False, "message": err or "Failed"}), status
    return (
        jsonify({"success": True, "payment": PaymentService.serialize_payment(p)}),
        200,
    )
...
def platform_review_manual_payment(payment_id: int):
    ...
    p, err = PaymentService.review_manual_payment(
...
def platform_list_invoices_for_tenant(tenant_id: str):
    ...
    return (
        jsonify(
            {
                "success": True,
                "invoices": [PaymentService.serialize_invoice(i) for i in items],
            }
        ),
        200,
    )
...
def platform_generate_invoice(tenant_id: str):
    ...
    if not academic_term_id:
        return (
            jsonify({"success": False, "message": "academic_term_id is required"}),
            400,
        )
    ...
    return (
        jsonify(
            {"success": True, "invoice": PaymentService.serialize_invoice(invoice)}
        ),
        201,
    )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": str(ex) or "Failed to load invoices",
                    "invoices": [],
                }
            ),
            200,
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": str(ex) or "Failed to load payments",
                    "payments": [],
                }
            ),
            200,
...
def school_payment_options():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": str(ex) or "Gateway lookup failed"}),
            400,
        )
    if err or not selected:
        return (
            jsonify(
                {
                    "success": False,
                    "message": err or "No gateway configured for this school",
                }
            ),
            400,
        )
...
@billing_bp.route(
    "/school/invoices/<int:invoice_id>/initialize-payment", methods=["POST"]
)
@tenant_required
@school_admin_required
def school_initialize_payment(invoice_id: int):
    ...
    inv = BillingInvoice.query.filter_by(
        id=int(invoice_id), tenant_id=tenant_id
    ).first()
    if not inv:
        ...
        jsonify(
            {
                ...
                "invoice": PaymentService.serialize_invoice(inv),
            }
        ),
        200,
    )
...
def school_verify_payment(payment_id: int):
    ...
    if err or not p:
        return jsonify({"success": False, "message": err or "Failed"}), 400
    inv = BillingInvoice.query.filter_by(
        id=int(p.invoice_id), tenant_id=tenant_id
    ).first()
    return (
        jsonify(
            {
...
def school_submit_manual_payment(invoice_id: int):
    ...
    if not currency:
        try:
            ...
            if _inv and _inv.currency:
                ...
                if _tenant:
                    _plan_ctx, _ = EntitlementService.getSchoolActivePlan(
                        str(tenant_id)
                    )
    ...
    if "proof" in request.files:
        proof_path, err = FileUtils.upload_payment_proof(
            request.files["proof"], payment_reference=reference
        )
        if err:
...
def platform_list_plans():
    ...
    return (
        jsonify(
            {
                "success": True,
                "plans": [subscription_change_ops.serialize_plan(p) for p in plans],
            }
        ),
        200,
    )
...
def platform_update_plan_billing_settings(plan_id: int):
    ...
    if min_months is None:
        return (
            jsonify({"success": False, "message": "billing_min_months is required"}),
            400,
        )
    plan, err = plan_pricing_ops.update_plan_billing_min_months(
        plan_id=int(plan_id), min_months=int(min_months)
    )
    if err or not plan:
    ...
    return (
        jsonify(
            {"success": True, "plan": subscription_change_ops.serialize_plan(plan)}
        ),
        200,
    )
...
def platform_create_plan_pricing_tier(plan_id: int):
    ...
    if err or not tier:
        return jsonify({"success": False, "message": err or "Failed"}), 400
    return (
        jsonify({"success": True, "tier": plan_pricing_ops.serialize_tier(tier)}),
        201,
    )
        ...
        jsonify(
            {
                "success": True,
                "requests": [
                    subscription_change_ops.serialize_change_request(r) for r in items
                ],
            }
        ),
        200,
...
@billing_bp.route(
    "/subscription-change-requests/<int:request_id>/approve", methods=["POST"]
)
@super_admin_required
def platform_approve_plan_change_request(request_id: int):
    reviewer_id = getattr(g, "current_user", None).id
    req, err = subscription_change_ops.approve_request(
        request_id=int(request_id), reviewer_user_id=int(reviewer_id)
    )
    if err or not req:
    ...
    return (
        jsonify(
            {
                "success": True,
                "request": subscription_change_ops.serialize_change_request(req),
            }
        ),
        200,
    )
...
@billing_bp.route(
    "/subscription-change-requests/<int:request_id>/reject", methods=["POST"]
)
@super_admin_required
...
def school_list_plans():
    ...
    if not active_plan:
        ...
        from app.models.billing import Plan as _Plan
    ...
    for p in plans:
        ...
        sp["is_current_plan"] = str(p.slug or "").strip().lower() == current_plan_slug
...
def school_list_academic_terms():
    ...
    return (
        jsonify(
            {
                "success": True,
                "terms": [subscription_change_ops.serialize_term(t) for t in terms],
            }
        ),
        200,
    )
    ...
    if not academic_term_id:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "academic_term_id could not be resolved or is missing",
                }
            ),
            400,
        )
...
def school_request_downgrade():
    ...
    plan_slug = data.get("plan_slug") or data.get("plan")
    raw_term = (
        data.get("effective_academic_term_id")
        or data.get("effective_term_id")
        or data.get("term_id")
    )
    ...
    if not effective_term_id:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "effective_academic_term_id could not be resolved or is missing",
                }
            ),
            400,
        )
