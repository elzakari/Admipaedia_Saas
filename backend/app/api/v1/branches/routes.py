...
def list_branches():
    ...
    try:
        ...
        branches = (
            Branch.query.filter_by(tenant_id=g.tenant_id)
        ...
        return (
            jsonify({"success": False, "message": "Failed to retrieve branches"}),
            500,
        )
...
def create_branch():
    ...
    try:
            ...
            if existing:
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": f"Branch code '{code}' is already in use",
                        }
                    ),
                    400,
                )
        ...
        branch = Branch(
            tenant_id=g.tenant_id, name=name, code=code, address=address, is_active=True
        )
    ...
    try:
        ...
        if name is not None:
            if not name.strip():
                return (
                    jsonify(
                        {"success": False, "message": "Branch name cannot be empty"}
                    ),
                    400,
                )
        ...
        if code is not None:
            if code.strip():
                ...
                existing = Branch.query.filter_by(
                    tenant_id=g.tenant_id, code=code.strip()
                ).first()
...
def delete_branch(branch_id):
    ...
    try:
        ...
        return (
            jsonify({"success": True, "message": "Branch deactivated successfully"}),
            200,
        )
        ...
        return (
            jsonify({"success": False, "message": "Failed to deactivate branch"}),
            500,
        )
