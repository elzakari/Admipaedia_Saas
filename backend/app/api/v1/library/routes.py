            ...
            jsonify(
                {
                    ...
                    "book": {
                        ...
                        "category": (
                            book.category.value
                            if hasattr(book.category, "value")
                            else book.category
                        ),
        ...
        jsonify(
            {
                ...
                "book": {
                    ...
                    "created_at": (
                        ...
                        if getattr(book, "created_at", None)
                        else None
                    ),
                },
            }
...
def get_borrow_records():
    ...
    paginated = LibraryService.get_borrow_records(
        page=page,
        per_page=per_page,
        status=status,
        search=search,
                ...
                "borrow_records": [
                    {
                        ...
                        "member_type": (
                            r.member.member_type.value
                            if hasattr(r.member.member_type, "value")
                            else r.member.member_type
                        ),
                        "member_name": f"{getattr(r.member.user, 'first_name', '')} {getattr(r.member.user, 'last_name', '')}".strip(),
                        "borrow_date": (
                            r.borrow_date.isoformat() if r.borrow_date else None
                        ),
                        ...
                        "status": (
                            "overdue"
                            if (
                                getattr(r, "return_date", None) is None
                                and getattr(r, "due_date", None)
...
def get_members():
    ...
    paginated = LibraryService.get_members(
        page=page,
        per_page=per_page,
        search=search,
        member_type=member_type,
                ...
                "members": [
                    {
                        ...
                        "member_type": (
                            m.member_type.value
                            if hasattr(m.member_type, "value")
                            else m.member_type
                        ),
...
def create_member():
    ...
    if not user_id or not member_type:
        return (
            jsonify(
                {"success": False, "message": "user_id and member_type are required"}
            ),
            400,
        )

    member = LibraryService.create_library_member(
        user_id=user_id, member_type=member_type, max_books=max_books, max_days=max_days
    )
    return (
        jsonify(
            {
                ...
                "member": {
                    ...
                    "member_type": (
                        ...
                        if hasattr(member.member_type, "value")
                        else member.member_type
                    ),
...
def create_digital_resource():
    ...
    resource = LibraryService.create_digital_resource(
        payload, created_by_id=get_jwt_identity()
    )
...
def export_library_report():
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=library-{report_type}.csv"
    )
    return response
