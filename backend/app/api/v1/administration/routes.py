...
from app.models.administration import (Budget, BudgetCategory, Transaction,
                                       TransactionType)
...
from app.schemas.administration import (  # FeeRecordSchema, FeeRecordCreateSchema,; FeePaymentSchema, FeePaymentCreateSchema,; Infrastructure schemas
    AssetSchema, BudgetCreateSchema, BudgetSchema, BudgetUpdateSchema,
    FacilitySchema, FeeStructureCreateSchema, FeeStructureSchema,
    FeeStructureUpdateSchema, FinancialSummarySchema,
    InfrastructureSummarySchema, MaintenanceRequestSchema, SystemSettingSchema,
    TransactionCreateSchema, TransactionSchema, TransactionUpdateSchema)
...
def _serialize_transaction(transaction):
    return {
        "id": transaction.id,
        "type": getattr(
            transaction.transaction_type, "value", transaction.transaction_type
        ),
...
def _serialize_fee_template_group(group_rows, category_by_id):
    ...
    return {
        ...
        "items": [
            ...
            for r in sorted(
                group_rows, key=lambda x: category_by_id.get(x.fee_category_id, "")
            )
        ],
        "total_amount": total,
        "created_at": (
            first.created_at.isoformat() if getattr(first, "created_at", None) else None
        ),
        "usage": usage,
    }
...
def _student_fees_have_payment_activity(student_fees):
    ...
    return (
        ...
        .filter(PaymentAllocation.student_fee_id.in_(fee_ids))
        .first()
        is not None
    )
...
def _summarize_fee_template_usage(structures):
    ...
    paid_count = sum(
        1 for fee in student_fees if float(getattr(fee, "paid_amount", 0) or 0) > 0
    )
...
def _persist_fee_structure_group(data, existing_rows=None):
    ...
    if not academic_year or not term or not isinstance(items, list) or len(items) == 0:
        return False, (
            {
                "success": False,
                "message": "academic_year, term, and items are required",
            },
            400,
        )
    ...
    for item in items:
        ...
        if category_key in seen_categories:
            return False, (
                {"success": False, "message": f"Duplicate fee category: {name}"},
                400,
            )
        try:
            ...
            return False, (
                {"success": False, "message": f"Invalid amount for {name}"},
                400,
            )
        if amount_val <= 0:
            return False, (
                {
                    "success": False,
                    "message": f"Amount for {name} must be greater than zero",
                },
                400,
            )
    ...
    if not normalized_items:
        return False, (
            {"success": False, "message": "At least one fee item is required"},
            400,
        )
    ...
    existing_rows = (
        ...
        else _get_fee_structure_group_rows(academic_year, term, class_id)
    )
    if (
        existing_rows
        and _summarize_fee_template_usage(existing_rows)["assigned_fee_records"] > 0
    ):
        return False, (
            {
                "success": False,
...
def get_fee_structure_groups():
    try:
        ...
        groups_list = [
            _serialize_fee_template_group(v, category_by_id) for v in grouped.values()
        ]
        groups_list.sort(
            key=lambda g: (
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "An error occurred while retrieving fee structures",
                }
            ),
            500,
        )
...
def update_fee_structure_group(group_id):
    ...
    if not parsed:
        return (
            jsonify({"success": False, "message": "Invalid fee structure group id"}),
            400,
        )
    ...
    try:
        success, result = _persist_fee_structure_group(
            data,
            existing_rows=_get_fee_structure_group_rows(academic_year, term, class_id),
        )
        if not success:
            payload, status_code = result
            return jsonify(payload), status_code
        ...
        return (
            jsonify({"success": False, "message": "Failed to update fee structure"}),
            500,
        )
...
def delete_fee_structure_group(group_id):
    ...
    try:
        ...
        if not rows:
            return (
                jsonify({"success": False, "message": "Fee structure group not found"}),
                404,
            )
        ...
        return (
            jsonify({"success": False, "message": "Failed to delete fee structure"}),
            500,
        )
...
def assign_fee_structure_group(group_id):
    ...
    try:
        ...
        if not structures:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "No fee structures found for this group",
                    }
                ),
                404,
            )
        ...
        created_count = FeeService.assign_fee_structures_to_students(
            structures, students, commit=True
        )
        ...
        return (
            jsonify({"success": False, "message": "Failed to assign fee structure"}),
            500,
        )
            ...
            out.append(
                {
                    ...
                    "currency": _get_school_currency(
                        getattr(p, "currency", None) or "USD"
                    ),
...
def list_fee_payments():
    try:
        ...
        paginated = query.order_by(Payment.paid_at.desc()).paginate(
...
def create_fee_payment_v2():
    try:
        ...
        reference_number = (data.get("reference_number") or "").strip() or str(
            uuid.uuid4()
        )
        ...
        if not fee_record_id or amount is None or not payment_method:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "fee_record_id, amount and payment_method are required",
                    }
                ),
                400,
            )
        ...
        if payment_method not in allowed_payment_methods:
            return (
                jsonify({"success": False, "message": "Unsupported payment method"}),
                400,
            )
        ...
        if fee_balance <= 0:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "This fee record is already fully paid",
                    }
                ),
                400,
            )
        ...
        if payment_date_raw:
            try:
                paid_at = datetime.fromisoformat(
                    str(payment_date_raw).replace("Z", "+00:00")
                ).replace(tzinfo=None)
            except Exception:
                paid_at = datetime.utcnow()
...
def get_overdue_fees_v2():
    try:
        ...
        paginated = query.order_by(
            FeeStructure.due_date.asc(), StudentFee.id.desc()
        ...
        return (
            jsonify({"success": False, "message": "Failed to fetch overdue fees"}),
            500,
        )
            ...
            recipient = recipients.setdefault(
                record.student_id,
                {
                    ...
                    "student_name": (
                        ...
                        if student
                        else f"Student {record.student_id}"
                    ),
                    "class_name": (
                        (
                            getattr(
                                getattr(student, "class_", None), "display_name", None
                            )
            ...
            jsonify(
                {
                    ...
                    "total_balance": round(
                        sum(float(record.balance or 0) for record in overdue_records), 2
                    ),
...
def get_budgets():
    ...
    try:
        ...
        paginated_budgets = administration_service.get_all_budgets(
            page, per_page, academic_year
        )
            ...
            jsonify(
                {
                    "success": True,
                    "budgets": [
                        _serialize_budget(budget) for budget in paginated_budgets.items
                    ],
                    "pagination": {
...
def get_budget(budget_id):
    ...
    try:
        ...
        return jsonify({"success": True, "budget": _serialize_budget(budget)}), 200
    except Exception as e:
...
def create_budget():
    ...
    try:
        ...
        academic_year = str(
            ...
            or data.get("fiscal_year")
            or data.get("budget_year")
            or datetime.utcnow().year
        )
        allocated_amount = Decimal(
            str(data.get("allocated_amount") or data.get("total_amount") or 0)
        )
        description = (
            data.get("description") or str(category.value).replace("_", " ").title()
        ).strip()
        ...
        existing_budget = Budget.query.filter_by(
            category=category, fiscal_year=academic_year
        ).first()
        if existing_budget:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": f"Budget for {category.value} in {academic_year} already exists",
                    }
                ),
                400,
            )
...
def update_budget(budget_id):
    ...
    try:
        ...
        if "category" in data or "name" in data:
            budget.category = _parse_budget_category(
                data.get("category") or data.get("name")
            )
        if "description" in data:
            budget.description = (
                data.get("description") or ""
            ).strip() or budget.description
        if "academic_year" in data or "fiscal_year" in data or "budget_year" in data:
            budget.fiscal_year = str(
                data.get("academic_year")
                or data.get("fiscal_year")
                or data.get("budget_year")
            )
        if "allocated_amount" in data or "total_amount" in data:
            budget.allocated_amount = Decimal(
                str(
                    data.get("allocated_amount")
                    or data.get("total_amount")
                    or budget.allocated_amount
                )
            )
            budget.remaining_amount = max(
                Decimal("0"),
                budget.allocated_amount - Decimal(str(budget.spent_amount or 0)),
            )
        if "quarter" in data:
...
def get_transactions():
    ...
    try:
        ...
        start_date_str = request.args.get("start_date", type=str) or request.args.get(
            "date_from", type=str
        )
        end_date_str = request.args.get("end_date", type=str) or request.args.get(
            "date_to", type=str
        )
...
def get_transaction(transaction_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "transaction": _serialize_transaction(transaction)}
            ),
            200,
        )
    except Exception as e:
        current_app.logger.error(
            f"Error getting transaction {transaction_id}: {str(e)}"
        )
        return (
            jsonify(
                {
...
def create_transaction():
    ...
    try:
        ...
        transaction_type = _parse_transaction_type(
        ...
        if transaction_date_raw:
            try:
                transaction_date = datetime.fromisoformat(
                    str(transaction_date_raw).replace("Z", "+00:00")
                ).date()
            except Exception:
                transaction_date = date.today()
        ...
        transaction = Transaction(
            ...
            reference_number=str(
                data.get("reference_number") or f"TXN-{uuid.uuid4().hex[:10].upper()}"
            ),
...
def get_fee_records():
    ...
    try:
        ...
        paginated = administration_service.get_all_fee_records(
            page, per_page, academic_year, class_id
        )
            ...
            items.append(
                {
                    ...
                    "academic_year": (
                        getattr(structure, "academic_year", None) if structure else None
                    ),
                    "total_amount": (
                        float(r.final_amount) if r.final_amount is not None else 0.0
                    ),
                    "original_amount": (
                    ...
                    "discount_amount": (
                        ...
                        if r.discount_amount is not None
                        else 0.0
                    ),
                    "final_amount": (
                        float(r.final_amount) if r.final_amount is not None else 0.0
                    ...
                    "status": r.status,
                    "due_date": (
                        structure.due_date.isoformat()
                        if getattr(structure, "due_date", None)
                        else None
                    ),
...
def get_student_fee_records(student_id):
    ...
    try:
        ...
        fee_records = administration_service.get_student_fee_records(
        ...
        current_app.logger.error(
            f"Error getting fee records for student {student_id}: {str(e)}"
        )
        return (
            jsonify(
                {
        ...
        if not student_id or not fee_structure_id:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "student_id and fee_structure_id are required",
                    }
                ),
                400,
            )
            ...
            jsonify(
                {
                    ...
                    "fee_record": {
                        ...
                        "status": fee.status,
                        "created_at": (
                            fee.created_at.isoformat() if fee.created_at else None
                        ),
                    },
                }
            ),
...
def get_facility(facility_id):
    ...
    try:
        ...
        return (
            jsonify({"success": True, "facility": facility_schema.dump(facility)}),
            200,
        )
    except Exception as e:
...
def create_facility():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Validation error", "errors": e.messages}
            ),
            400,
        )
    except Exception as e:
...
def delete_facility(facility_id):
    ...
    try:
        ...
        return (
            jsonify({"success": True, "message": "Facility deleted successfully"}),
            200,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "maintenance_requests": maintenance_requests_schema.dump(
                        paginated_requests.items
                    ),
                    "pagination": {
...
def get_maintenance_request(request_id):
    ...
    try:
        ...
        if not maintenance_request:
            return (
                jsonify({"success": False, "message": "Maintenance request not found"}),
                404,
            )
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "maintenance_request": maintenance_request_schema.dump(
                        maintenance_request
                    ),
                }
            ),
        ...
        current_app.logger.error(
            f"Error getting maintenance request {request_id}: {str(e)}"
        )
        return (
            jsonify(
                {
...
def update_maintenance_request(request_id):
    ...
    try:
        ...
        success, result = administration_service.update_maintenance_request(
            request_id, data
        )
        ...
        current_app.logger.error(
            f"Error updating maintenance request {request_id}: {str(e)}"
        )
        return (
            jsonify(
                {
...
def delete_maintenance_request(request_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "message": "Maintenance request deleted successfully"}
            ),
            200,
        )
        ...
        current_app.logger.error(
            f"Error deleting maintenance request {request_id}: {str(e)}"
        )
        return (
            jsonify(
                {
...
def get_asset(asset_id):
    ...
    try:
        ...
        return jsonify({"success": True, "asset": asset_schema.dump(asset)}), 200
    except Exception as e:
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "infrastructure_summary": infrastructure_summary_schema.dump(
                        summary
                    ),
                }
            ),
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "overdue_maintenance_requests": maintenance_requests_schema.dump(
                        overdue_requests
                    ),
                }
            ),
        ...
        current_app.logger.error(
            f"Error getting overdue maintenance requests: {str(e)}"
        )
        return (
            jsonify(
                {
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "assets_needing_service": assets_schema.dump(
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "expired_warranty_assets": assets_schema.dump(
                        expired_warranty_assets
                    ),
                }
            ),
        ...
        current_app.logger.error(
            f"Error getting assets with expired warranty: {str(e)}"
        )
        return (
            jsonify(
                {
...
def get_system_settings():
    ...
    try:
        ...
        return (
            jsonify({"success": True, "settings": {s.key: s.value for s in settings}}),
            200,
        )
    except Exception as e:
...
def update_system_settings():
    ...
    try:
        ...
        return (
            jsonify({"success": True, "message": "Settings updated successfully"}),
            200,
        )
    except Exception as e:
