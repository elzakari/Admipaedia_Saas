...
def get_plan_context():
    ...
    usage = {}
    for service_type, st in ServiceTokenService.get_status_map(
        tenant_id, SERVICE_TYPES
    ).items():
        usage[service_type] = {
    ...
    resp = {
        ...
        "data": {
            ...
            "subscription": {
                ...
                "starts_at": (
                    active.subscription.starts_at.isoformat()
                    if active.subscription.starts_at
                    else None
                ),
                "ends_at": (
                    active.subscription.ends_at.isoformat()
                    if active.subscription.ends_at
                    else None
                ),
            },
