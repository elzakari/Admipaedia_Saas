...
from app.schemas.parent import (ParentCreateSchema, ParentSchema,
                                ParentUpdateSchema)
...
def get_parents():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "parents": [], "total": 0, "page": 1, "per_page": 10}
            ),
            200,
        )
...
def get_parent(parent_id):
    ...
    try:
        ...
        if parent and getattr(parent, "tenant_id", None) != getattr(
            g, "tenant_id", None
        ):
            parent = None
        if not parent:
        ...
        return success_response(
            data=parent_schema.dump(parent), message="Parent retrieved successfully"
        )
    except Exception as e:
...
def update_parent(parent_id):
    ...
    try:
        ...
        return success_response(
            data=parent_schema.dump(parent), message="Parent updated successfully"
        )
    except ValidationError as e:
        return error_response(
            message="Validation error", errors=e.messages, status_code=400
...
def delete_parent(parent_id):
    ...
    try:
        parent = ParentService.get_parent_by_id(parent_id)
        if not parent or getattr(parent, "tenant_id", None) != getattr(
            g, "tenant_id", None
        ):
            return error_response(message="Parent not found", status_code=404)
...
def get_parent_children(parent_id):
    ...
    try:
        ...
        if not parent or getattr(parent, "tenant_id", None) != getattr(
            ...
            return error_response(message="Parent not found", status_code=404)
        children, total = ParentService.get_children(
            parent_id, page=page, per_page=per_page
        )
...
def get_parent_dashboard():
    ...
    try:
        ...
        return error_response(
            message="Failed to retrieve dashboard data", status_code=500
        )
...
def get_parent_profile():
    ...
    try:
        ...
        return success_response(
            data=parent_schema.dump(parent), message="Profile retrieved successfully"
        )
    except Exception as e:
...
def get_my_children():
    ...
    try:
        ...
        children, total = ParentService.get_children(
            parent.id, page=page, per_page=per_page
        )
...
def get_child_attendance(child_id):
    ...
    try:
        ...
        has_records = (
            Attendance.query.filter_by(student_id=child_id).first() is not None
        )
        page = request.args.get("page", 1, type=int)
        ...
        if start_date:
            try:
                ...
                return error_response(
                    message="Invalid start_date format. Use YYYY-MM-DD", status_code=400
                )
        if end_date:
            try:
                ...
                return error_response(
                    message="Invalid end_date format. Use YYYY-MM-DD", status_code=400
                )
        ...
        return error_response(
            message="Failed to retrieve attendance records", status_code=500
        )
...
def get_child_grades(child_id):
    ...
    try:
        ...
        return error_response(
            message="Failed to retrieve grade records", status_code=500
        )
...
def get_parent_notifications():
    ...
    try:
        ...
        return error_response(
            message="Failed to retrieve notifications", status_code=500
        )
...
def mark_notification_read(notification_id):
    ...
    try:
        ...
        if not success:
            return error_response(
                message="Notification not found or access denied", status_code=404
            )
        ...
        return error_response(
            message="Failed to mark notification as read", status_code=500
        )
...
def get_child_homework(child_id):
    ...
    try:
        ...
        if not child or not child.class_id:
            ...
            total = query.count()
            items = (
                query.order_by(Assignment.due_date.desc())
            ...
            assignment_attachment_map = AssignmentService.get_attachment_map(
                "assignment", [assignment.id for assignment in items]
            )
            submission_attachment_map = AssignmentService.get_attachment_map(
            ...
            for a in items:
                ...
                if submission:
                    status = (
                        "graded"
                        if submission.status == "graded" or submission.score is not None
                        else "submitted"
                    )
                homework.append(
                    {
                        ...
                        "submission_attachments": (
                            submission_attachment_map.get(str(submission.id), [])
                            if submission
                            else []
                        ),
                    }
        ...
        return error_response(
            message="Failed to retrieve homework assignments", status_code=500
        )
...
def get_child_lessons(child_id):
    ...
    try:
        ...
        if not child or not child.class_id:
            return success_response(
                data={"lessons": []}, message="Lessons retrieved successfully"
            )

        limit = min(request.args.get("limit", 30, type=int), 90)
        lessons = (
            Lesson.query.filter_by(class_id=child.class_id)
            .order_by(Lesson.date.desc(), Lesson.created_at.desc())
            .limit(limit)
            .all()
        )
        absent_dates = {
        ...
        lesson_payload = [
            {
                **LessonService.serialize_lesson(lesson),
                "child_was_absent": LessonService.serialize_lesson(lesson).get("date")
                in absent_dates,
            }
        ...
        return success_response(
            data={"lessons": lesson_payload}, message="Lessons retrieved successfully"
        )
    except Exception as e:
...
def get_child_fees(child_id):
    ...
    try:
        ...
        from app.models.finance import Payment, StudentFee
        ...
        def serialize_fee_record(record):
            ...
            category = (
                getattr(getattr(structure, "category", None), "name", None) or "Fee"
            )
            ...
            return {
                ...
                "template_group_id": (
                    f"{getattr(structure, 'academic_year', '')}__{getattr(structure, 'term', '')}__{getattr(structure, 'class_id', None) or 0}"
                    if structure
                    else None
                ),
                "category": category,
                ...
                "class_name": (
                    (
                        getattr(structure_class, "display_name", None)
                        or getattr(structure_class, "name", None)
                    )
                    if structure_class
                    else None
                ),
        ...
        payments = (
            Payment.query.filter_by(student_id=child_id)
...
def get_child_summary_detailed(child_id):
    ...
    try:
        ...
        return success_response(
            data=summary_data, message="Child detailed summary retrieved successfully"
        )
    except Exception as e:
        logger.error(f"Error retrieving child summary detailed: {str(e)}")
        return error_response(
            message="Failed to retrieve child detailed summary", status_code=500
        )
...
def get_child_academic(child_id):
    ...
    try:
        ...
        return error_response(
            message="Failed to retrieve academic records", status_code=500
        )
...
def get_parent_messages(parent_id):
    ...
    try:
        ...
        if not parent or (
            parent.id != parent_id and getattr(parent, "user_id", None) != parent_id
        ):
            return error_response(
                message="Parent not found or access denied", status_code=403
            )
