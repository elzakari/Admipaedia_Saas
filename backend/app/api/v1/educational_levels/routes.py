...
def get_educational_levels():
    ...
    try:
        levels = (
            EducationalLevel.query.filter_by(is_active=True)
            .order_by(EducationalLevel.id)
            .all()
        )
                    ...
                    "data": [
                        {
                            ...
                            "key_phase": (
                                level.key_phase
                                if isinstance(level.key_phase, str)
                                else getattr(
                                    level.key_phase, "value", str(level.key_phase)
                                )
                            ),
                            ...
                            "sequence_order": (
                                (order_codes.index(level.level_code) + 1)
                                if level.level_code in order_codes
                                else None
                            ),
...
@educational_levels_bp.route(
    "/student/<int:student_id>/competency-assessment", methods=["POST"]
)
@jwt_required()
