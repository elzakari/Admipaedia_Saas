...
def create_announcement():
    ...
    try:
        ...
        if scope == "class_bound" and not class_id:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "class_id is required when scope is class_bound",
                    }
                ),
                400,
            )
        ...
        if scheduled_date_raw:
            try:
                scheduled_date = datetime.fromisoformat(
                    str(scheduled_date_raw).replace("Z", "+00:00")
                ).replace(tzinfo=None)
            except Exception:
                scheduled_date = None
        ...
        if isinstance(target_roles_raw, list):
            target_roles = [
                str(x).strip().lower() for x in target_roles_raw if str(x).strip()
            ]
        elif isinstance(target_roles_raw, str) and target_roles_raw.strip():
            target_roles = [
                item.strip().lower()
                for item in target_roles_raw.split(",")
                if item.strip()
            ]
        else:
            target_roles = ["all"]

        tenant_id, request_user, tenant_error = resolve_tenant_for_request(
        ...
        if scope == "class_bound" and class_id:
            ...
            class_tenant_id = AnnouncementService.normalize_tenant_id(
            ...
            if tenant_id and class_tenant_id and tenant_id != class_tenant_id:
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Selected class is outside the active tenant context",
                        }
                    ),
                    403,
                )
        ...
        if user.role == "teacher":
            ...
            if scope == "class_bound":
                ...
                current_app.logger.info(
                    f"Announcement permission check: user_id={user_id}, teacher_id={teacher.id}, class_id={class_id}, is_assigned={is_assigned}"
                )
                if not is_assigned:
                    return (
                        jsonify(
        ...
        return (
            jsonify({"success": False, "message": "Failed to create announcement"}),
            500,
        )
...
def get_announcements():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": "Failed to fetch announcements"}),
            500,
        )
    ...
    try:
        ...
        if "title" in updates:
            ...
            if not updates["title"]:
                return (
                    jsonify({"success": False, "message": "title cannot be empty"}),
                    400,
                )
        ...
        if "scheduled_date" in updates:
            ...
            if raw:
                try:
                    updates["scheduled_date"] = datetime.fromisoformat(
                        str(raw).replace("Z", "+00:00")
                    ).replace(tzinfo=None)
                except Exception:
                    return (
                        jsonify(
                            {"success": False, "message": "Invalid scheduled_date"}
                        ),
                        400,
                    )
            else:
        ...
        if "target_roles" in updates:
            ...
            if isinstance(tr, list):
                ...
                updates["target_roles"] = [
                    item.strip().lower() for item in tr.split(",") if item.strip()
                ]
            else:
        ...
        if user.role in ("admin", "super_admin", "superadmin", "super_manager"):
            announcement, err = AnnouncementService.update_announcement_admin(
                announcement_id, updates
            )
        else:
            announcement = AnnouncementService.get_announcement_by_id(announcement_id)
            if not announcement:
                return (
                    jsonify({"success": False, "message": "Announcement not found"}),
                    404,
                )

            from app.services.identity_resolver import IdentityResolver

            is_assigned = IdentityResolver.can_user_access_class(
                user_id, announcement.class_id
            )
            if not is_assigned:
                return (
                    jsonify(
            ...
            announcement, err = AnnouncementService.update_announcement(
                announcement_id, updates, announcement.class_id, teacher.id
            )
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "announcement": AnnouncementSchema().dump(announcement),
                }
            ),
            200,
        )
        ...
        return (
            jsonify({"success": False, "message": "Failed to update announcement"}),
            500,
        )
...
def delete_announcement(announcement_id):
    try:
        ...
        if user.role in ("admin", "super_admin", "superadmin", "super_manager"):
            ...
            ok, err = AnnouncementService.delete_announcement(
                announcement_id, announcement.class_id, teacher.id
            )
