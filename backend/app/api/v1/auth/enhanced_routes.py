...
from app.models.enhanced_auth import (AuthenticationAttempt, MFADevice,
                                      TrustedDevice, UserSecuritySettings)
...
class MFASetupSchema(Schema):
    method = fields.Str(
        required=True, validate=validate.OneOf(["totp", "sms", "email"])
    )
...
def enhanced_login():
    ...
    try:
        ...
        if not identifier:
            return (
                jsonify({"success": False, "error": "Email or username is required"}),
                400,
            )
        ...
        if result.get("mfa_required"):
            ...
            if data.get("mfa_code"):
                mfa_result = EnhancedAuthService.verify_mfa(
                    mfa_token=result["mfa_token"], code=data["mfa_code"]
                )
        ...
        if data.get("trust_device") and result.get("user"):
            ...
            if (
                user
                and user.security_settings
                and user.security_settings.trust_new_devices
            ):
                # Mark device as trusted
                trusted_device = TrustedDevice.query.filter_by(
                    user_id=user.id, device_fingerprint=device_info["fingerprint"]
                ).first()
                ...
                if not trusted_device:
                    trusted_device = TrustedDevice(
                        ...
                        is_trusted=True,
                        trust_expires_at=datetime.utcnow()
                        + timedelta(
...
def get_trusted_devices():
    ...
    try:
        ...
        devices = (
            ...
            .filter(TrustedDevice.trust_expires_at > datetime.utcnow())
            .all()
        )
                    ...
                    "devices": [
                        {
                            ...
                            "trust_expires_at": (
                                device.trust_expires_at.isoformat()
        ...
        logger.error(
            "get_trusted_devices_error", error=str(err), user_id=get_jwt_identity()
        )
        return (
            jsonify({"success": False, "error": "Failed to retrieve trusted devices"}),
            500,
        )
...
def revoke_trusted_device(device_id):
    ...
    try:
        ...
        log_security_event(
            "trusted_device_revoked", {"user_id": user_id, "device_id": device_id}
        )
        ...
        logger.error(
            "revoke_trusted_device_error", error=str(err), user_id=get_jwt_identity()
        )
        return (
            jsonify({"success": False, "error": "Failed to revoke device trust"}),
            500,
        )
...
def get_security_settings():
    ...
    try:
            ...
            jsonify(
                {
                    ...
                    "settings": {
                        ...
                        "last_password_change": (
                            settings.last_password_change.isoformat()
        ...
        logger.error(
            "get_security_settings_error", error=str(err), user_id=get_jwt_identity()
        )
        return (
            jsonify(
                {"success": False, "error": "Failed to retrieve security settings"}
            ),
            500,
        )
    ...
    try:
        ...
        if "session_timeout_minutes" in data:
            settings.session_timeout_minutes = min(
                max(data["session_timeout_minutes"], 30), 1440
            )  # 30 min to 24 hours
        ...
        if "suspicious_activity_alerts" in data:
            settings.suspicious_activity_alerts = bool(
                data["suspicious_activity_alerts"]
            )
        ...
        if "device_trust_duration_days" in data:
            settings.device_trust_duration_days = min(
                max(data["device_trust_duration_days"], 1), 90
            )  # 1 to 90 days
        ...
        logger.error(
            "update_security_settings_error", error=str(err), user_id=get_jwt_identity()
        )
        return (
            jsonify({"success": False, "error": "Failed to update security settings"}),
            500,
        )
...
def get_user_sessions():
    ...
    try:
        ...
        logger.error(
            "get_user_sessions_error", error=str(err), user_id=get_jwt_identity()
        )
...
def revoke_session(session_id):
    ...
    try:
        ...
        if session_token.jti == current_jti:
            return (
                jsonify({"success": False, "error": "Cannot revoke current session"}),
                400,
            )
