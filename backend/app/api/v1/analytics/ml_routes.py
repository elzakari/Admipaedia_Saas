    ...
    try:
        ...
        if result["status"] == "success":
            return success_response(
                data=result, message="Performance prediction model training completed"
            )
        else:
            return error_response(
                message=result.get("message", "Training failed"), errors=result
            )

    except Exception as e:
        logger.error(f"Error training performance model: {str(e)}")
        return error_response(
            message="Failed to train performance model", errors=str(e)
        )
...
def train_risk_model():
    ...
    try:
        ...
        if result["status"] == "success":
            return success_response(
                data=result, message="Risk assessment model training completed"
            )
        else:
            return error_response(
                message=result.get("message", "Training failed"), errors=result
        ...
        return error_response(message="Failed to train risk model", errors=str(e))
...
def retrain_all_models():
    ...
    try:
        ...
        if result["status"] == "success":
            return success_response(
                data=result, message="All models retrained successfully"
            )
        else:
            return error_response(
                message=result.get("message", "Retraining failed"), errors=result
            )

    except Exception as e:
        logger.error(f"Error retraining models: {str(e)}")
        return error_response(message="Failed to retrain models", errors=str(e))
...
def get_model_status():
    ...
    try:
        ...
        return success_response(
            data=result, message="Model status retrieved successfully"
        )

    except Exception as e:
        logger.error(f"Error getting model status: {str(e)}")
        return error_response(message="Failed to get model status", errors=str(e))
...
def evaluate_models():
    ...
    try:
        ...
        return success_response(data=result, message="Model evaluation completed")

    except Exception as e:
        logger.error(f"Error evaluating models: {str(e)}")
        return error_response(message="Failed to evaluate models", errors=str(e))
