...
def _subject_performance_rows(academic_year=None, term=None):
    ...
    final_rows = (
        final_query.group_by(Subject.id, Subject.name).order_by(Subject.name).all()
    )
    if final_rows:
        return final_rows
    ...
    enhanced_rows = (
        enhanced_query.group_by(Subject.id, Subject.name).order_by(Subject.name).all()
    )
    if enhanced_rows:
...
def _class_trend_rows(class_id):
    ...
    final_rows = (
        final_query.group_by(FinalGrade.academic_year, FinalGrade.term)
        .order_by(FinalGrade.academic_year, FinalGrade.term)
        .all()
    )
    if final_rows:
        return final_rows
    ...
    enhanced_rows = (
        enhanced_query.group_by(EnhancedGrade.academic_year, EnhancedGrade.term)
        .order_by(EnhancedGrade.academic_year, EnhancedGrade.term)
        .all()
    )
    if enhanced_rows:
        return enhanced_rows
    ...
    return (
        ...
        .group_by(Grade.academic_year, Grade.term)
        .order_by(Grade.academic_year, Grade.term)
        .all()
    )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Error retrieving subject analytics: {str(e)}",
                }
            ),
            500,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Error retrieving class trends: {str(e)}",
                }
            ),
            500,
        )
