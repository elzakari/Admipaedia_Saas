    ...
    try:
        ...
        try:
            result, error = EnhancedStudentService.bulk_import_students(
                temp_path, create_users=create_users, update_existing=update_existing
            )
...
def invite_parent(student_id):
    ...
    try:
        ...
        if not email and not phone:
            return (
                jsonify({"success": False, "message": "Email or phone required"}),
                400,
            )
        ...
        return (
            jsonify(
                {"success": True, "message": f"Invitation sent to {email or phone}"}
            ),
            200,
        )
