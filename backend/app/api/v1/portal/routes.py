                ...
                "children": [
                    {
                        ...
                        "class": getattr(c.class_, "display_name", None)
                        or (c.class_.name if c.class_ else None),
...
def get_child_dashboard(student_id):
    user_id = get_jwt_identity()
    data, error, status_code = ParentPortalService.get_authorized_child_dashboard(
        user_id, student_id, g.tenant_id
    )
...
def complete_child_setup():
    ...
    if not task_id:
        return (
            jsonify({"success": False, "message": "task_id parameter is required"}),
            400,
        )

    task, error = ParentPortalService.complete_child_setup_task(
        user_id, task_id, g.tenant_id
    )
