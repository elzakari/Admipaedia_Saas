...
def admin_communicate():
    ...
    recipient_user_ids = resolve_audience(
        tenant_id, recipient_type, recipient_id, audience_scope
    )
    ...
    try:
        ...
        if comm_type == "direct_message":
            for r_id in recipient_user_ids:
                try:
                    ...
                    msg = Message(
                        ...
                        recipient_id=r_id,
                        recipient_type=MessageService._get_user_type(
                            User.query.get(r_id)
                        ),
                        subject=subject,
                        content=content,
                    ...
                    logger.error(
                        f"Failed to create DM for user {r_id}: {str(inner_err)}"
                    )
                    failed_count += 1
            ...
            for r_id in recipient_user_ids:
                try:
                    ...
                    logger.error(
                        f"Failed to create notification for user {r_id}: {str(inner_err)}"
                    )
                    failed_count += 1
            ...
            if not classes_to_announce:
                ...
                classes_to_announce = [
                    c.id for c in Class.query.filter_by(tenant_id=tenant_id).all()
                ]
            ...
            for c_id in classes_to_announce:
                try:
                    ...
                    logger.error(
                        f"Failed to create announcement for class {c_id}: {str(inner_err)}"
                    )
                    failed_count += 1
        ...
        return (
            jsonify(
                {"success": False, "message": f"Failed to send communication: {str(e)}"}
            ),
            500,
        )
