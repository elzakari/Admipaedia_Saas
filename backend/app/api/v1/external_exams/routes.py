...
from app.models import (EnhancedGrade, ExamSession, ExternalExamImportLog,
                        ExternalExamination, ExternalExamRegistration,
                        ExternalExamResult, ExternalExamType, GradeBoundary,
                        GradingScheme, ResultStatus, Student, Subject)
            ...
            result.append(
                {
                    ...
                    "exam_start_date": (
                        exam.exam_start_date.isoformat()
                        if exam.exam_start_date
                        else None
                    ),
                    "exam_end_date": (
                        exam.exam_end_date.isoformat() if exam.exam_end_date else None
                    ),
                    "result_status": exam.result_status.value,
                    "result_release_date": (
                        exam.result_release_date.isoformat()
                        if exam.result_release_date
                        else None
                    ),
                    ...
                    "last_import_date": (
                        exam.last_import_date.isoformat()
                        if exam.last_import_date
                        else None
                    ),
                }
            )
    ...
    try:
        ...
        examination = ExternalExamination(
            ...
            registration_start_date=(
                datetime.strptime(
                    data.get("registration_start_date"), "%Y-%m-%d"
                ).date()
                if data.get("registration_start_date")
                else None
            ),
            registration_end_date=(
                datetime.strptime(data.get("registration_end_date"), "%Y-%m-%d").date()
                if data.get("registration_end_date")
                else None
            ),
            exam_start_date=datetime.strptime(
                data["exam_start_date"], "%Y-%m-%d"
            ).date(),
            exam_end_date=datetime.strptime(data["exam_end_date"], "%Y-%m-%d").date(),
            result_release_date=(
                datetime.strptime(data.get("result_release_date"), "%Y-%m-%d").date()
                if data.get("result_release_date")
                else None
            ),
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error creating examination: {str(e)}"}
            ),
            500,
        )


@external_exams_bp.route(
    "/examinations/<int:exam_id>/register-student", methods=["POST"]
)
@jwt_required()
def register_student_for_exam(exam_id):
    ...
    try:
        ...
        existing_registration = ExternalExamRegistration.query.filter_by(
            examination_id=exam_id, student_id=data["student_id"]
        ).first()
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error registering student: {str(e)}"}
            ),
            500,
        )
...
def import_exam_results(exam_id):
    ...
    try:
        ...
        for row in csv_input:
            ...
            try:
                ...
                registration = ExternalExamRegistration.query.filter_by(
                    examination_id=exam_id, index_number=row["index_number"]
                ).first()

                if not registration:
                    raise ValueError(
                        f"No registration found for index number: {row['index_number']}"
                    )
                ...
                result = ExternalExamResult(
                    ...
                    raw_score=(
                        float(row.get("raw_score", 0)) if row.get("raw_score") else None
                    ),
                    percentage_score=(
                        float(row.get("percentage_score", 0))
                        if row.get("percentage_score")
                        else None
                    ),
                    grade_symbol=row["grade_symbol"],
                    grade_points=(
                        float(row.get("grade_points", 0))
                        if row.get("grade_points")
                        else None
                    ),
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error importing results: {str(e)}"}
            ),
            500,
        )
...
def integrate_external_result(result_id):
    ...
    try:
        ...
        if external_result.is_integrated:
            return (
                jsonify({"success": False, "message": "Result is already integrated"}),
                400,
            )
        ...
        if examination.exam_type == ExternalExamType.BECE:
            grading_scheme = GradingScheme.query.filter_by(
                standard=GradingStandard.BECE, is_active=True
            ).first()
        elif examination.exam_type == ExternalExamType.WASSCE:
            grading_scheme = GradingScheme.query.filter_by(
                standard=GradingStandard.WASSCE, is_active=True
            ).first()
        ...
        enhanced_grade = EnhancedGrade(
            ...
            is_passing=(
                external_result.grade_points >= 5.0
                if external_result.grade_points
                else False
            ),
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error integrating result: {str(e)}"}
            ),
            500,
        )
...
def get_performance_comparison():
    ...
    try:
        ...
        comparison_data = {
            "total_students": len(
                set(result.student_id for result in external_results)
            ),
            "subjects_analyzed": len(
                set(result.subject_id for result in external_results)
            ),
            "average_external_performance": 0,
            "average_internal_performance": 0,
            "performance_correlation": 0,
            "subject_breakdown": [],
            "grade_distribution": {"external": {}, "internal": {}},
        }

        # Calculate averages and correlations
        if external_results:
            external_scores = [
                r.percentage_score for r in external_results if r.percentage_score
            ]
            comparison_data["average_external_performance"] = (
                sum(external_scores) / len(external_scores) if external_scores else 0
            )
            ...
            if internal_grades:
                ...
                comparison_data["average_internal_performance"] = sum(
                    internal_scores
                ) / len(internal_scores)
