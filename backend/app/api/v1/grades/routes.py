...
def bulk_enter_exam_grades():
    ...
    if not exam_id or not isinstance(grades, list):
        return (
            jsonify({"success": False, "message": "exam_id and grades[] are required"}),
            400,
        )
    ...
    for item in grades:
        ...
        percentage = (
            round((marks_obtained / float(exam.total_marks)) * 100, 2)
            if float(exam.total_marks or 0) > 0
            else 0
        )
    ...
    return (
        jsonify(
            {
                "success": True,
                "grades": grades_schema.dump(saved),
...
def enter_grades():
    ...
    if not class_ids:
        return (
            jsonify(
                {"success": False, "message": "class_id is required for grade entry"}
            ),
            400,
        )
    if len(class_ids) > 1:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "All grades in a single request must belong to one class",
                }
            ),
            400,
        )
...
def get_class_grade_analytics(class_id):
    ...
    final_grades = final_query.order_by(
        FinalGrade.computed_at.asc(), FinalGrade.id.asc()
    ).all()
    if final_grades:
        distribution = {}
        ...
        analytics = {
            ...
            "academic_year": academic_year,
            "class_average": round(
                sum(float(grade.final_percentage or 0) for grade in final_grades)
                / len(final_grades),
                2,
            ),
    ...
    enhanced_grades = enhanced_query.order_by(
        EnhancedGrade.assessment_date.asc(), EnhancedGrade.id.asc()
    ).all()
    if enhanced_grades:
        distribution = {}
        ...
        analytics = {
            ...
            "academic_year": academic_year,
            "class_average": round(
                sum(float(grade.raw_score or 0) for grade in enhanced_grades)
                / len(enhanced_grades),
                2,
            ),
            "grade_distribution": distribution,
    ...
    analytics = {
        ...
        "academic_year": academic_year,
        "class_average": round(
            sum(float(grade.percentage or 0) for grade in grades) / len(grades), 2
        ),
        "grade_distribution": distribution,
        "performance_trends": trends,
