...
from app.models.security import SchoolRegistrationToken, SecurityEvent
...
def _membership_map_for_user_ids(user_ids):
    ...
    for membership, tenant in rows:
        membership_map.setdefault(int(membership.user_id), []).append(
            (membership, tenant)
        )
...
def _serialize_user(u: User, memberships=None):
    ...
    active_school_memberships = [
        membership
        for membership in school_memberships
        if membership.get("status") == "active"
    ]
    primary_school = (
        active_school_memberships[0]
        if active_school_memberships
        else (school_memberships[0] if school_memberships else None)
    )
    return {
        "id": u.id,
        "username": u.username,
...
def super_admin_overview():
    ...
    recent = (
        SecurityEvent.query.filter(SecurityEvent.event_type.like("super_admin.%"))
...
def super_admin_create_school_registration_link():
    ...
    if not country_code or len(country_code) != 2:
        return (
            jsonify(
                {"success": False, "error": "country_code must be a 2-letter code"}
            ),
            400,
        )
    ...
    registration_url = (
        f"{frontend_url}/school/register?token={token}"
        if frontend_url
        else f"/school/register?token={token}"
    )

    _audit(
        "super_admin.school_registration_link_created",
        actor.id,
        {
    ...
    if send_email_flag:
        try:
            from app.services.email_service import \
                send_school_registration_email
...
def super_admin_list_users():
    ...
    tenant_id = (
        request.args.get("tenant_id") or request.args.get("school_id") or ""
    ).strip()
    ...
    if tenant_id or membership_status:
        ...
        if tenant_id:
            try:
                ...
                membership_user_ids = membership_user_ids.filter(
        ...
        if membership_user_ids is not None and membership_status:
            membership_user_ids = membership_user_ids.filter(
                TenantMembership.status == membership_status
            )
    ...
    memberships_by_user = _membership_map_for_user_ids(
        ...
        jsonify(
            {
                "success": True,
                "users": [
                    _serialize_user(u, memberships_by_user.get(u.id, []))
        ...
        try:
            ...
            email_sent = bool(
                send_password_reset_email(
                    user.email, token, frontend_url=frontend_url, async_send=True
                )
            )
            _audit(
                "super_admin.user_reset_sent",
                actor.id,
                {"target_user_id": user.id, "email_sent": email_sent},
            )
        except Exception:
            email_sent = False
...
def super_admin_update_user(user_id: int):
    ...
    if target.id == actor.id:
        if "role" in payload and (payload.get("role") or "").strip() != "super_admin":
            return (
                jsonify({"success": False, "error": "Cannot change your own role"}),
                400,
            )
        if "status" in payload and (payload.get("status") or "").strip() in (
            "inactive",
            "deactivated",
        ):
            return (
    ...
    if "role" in payload:
        ...
        if new_role:
            if (
                new_role in ("super_admin", "super_manager")
    ...
    _audit(
        ...
        actor.id,
        {"target_user_id": target.id, "old": old, "new": _serialize_user(target)},
    )
    db.session.commit()
...
def super_admin_deactivate_user(user_id: int):
    ...
    if target.id == actor.id:
        return (
            jsonify({"success": False, "error": "Cannot deactivate your own account"}),
            400,
        )
    if target.role in ("super_admin", "super_manager"):
        return (
            jsonify({"success": False, "error": "Cannot deactivate this account"}),
            400,
        )
...
def super_admin_reactivate_user(user_id: int):
    ...
    _audit("super_admin.user_reactivated", actor.id, {"target_user_id": target.id})
    db.session.commit()
...
def super_admin_send_password_reset(user_id: int):
    ...
    if send_email:
        ...
        email_sent = send_password_reset_email(
            target.email, token, frontend_url=frontend_url, async_send=True
        )

        _audit(
            "super_admin.user_reset_sent",
            actor.id,
            {"target_user_id": target.id, "email_sent": bool(email_sent)},
        )
    else:
        _audit(
            "super_admin.user_reset_link_generated",
            actor.id,
            {"target_user_id": target.id},
        )
    db.session.commit()
    ...
    payload = {
        ...
        "email_suppressed": (
            bool(current_app.config.get("MAIL_SUPPRESS_SEND")) if send_email else False
        ),
        "link": reset_url,
    }
    if (
...
def super_admin_purge_user(user_id: int):
    ...
    if target.role == "super_admin":
        return (
            jsonify({"success": False, "error": "Cannot delete a super admin account"}),
            400,
        )
...
def super_admin_audit_logs():
    ...
    if date_from:
        try:
            query = query.filter(
                SecurityEvent.created_at >= datetime.fromisoformat(date_from)
            )
        except Exception:
            pass
    if date_to:
        try:
            query = query.filter(
                SecurityEvent.created_at <= datetime.fromisoformat(date_to)
            )
        except Exception:
            pass
    if q:
...
def super_admin_delete_orphan_tenant(tenant_id: str):
    actor = g.current_user
    confirm = (request.args.get("confirm") or "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    try:
        ...
        _audit(
            "super_admin.orphan_tenant_deleted",
            actor.id,
            {"tenant_id": result.get("tenant_id"), "deleted": result.get("deleted")},
            severity="warning",
        )
        db.session.commit()
        return jsonify({"success": True, "result": result}), 200
        ...
        db.session.rollback()
        return (
            jsonify({"success": False, "error": "Failed to delete orphan tenant"}),
            500,
        )
...
def super_admin_purge_tenant(tenant_id: str):
    ...
    try:
        ok, result = OrphanCleanupService.purge_tenant(
            tenant_id, actor_user_id=actor.id
        )
        if not ok:
            return (
                jsonify(
        ...
        _audit(
            "super_admin.tenant_purged",
            actor.id,
            {"tenant_id": tenant_id, "deleted": result.get("deleted")},
            severity="critical",
        )
        db.session.commit()
        return jsonify({"success": True, "result": result}), 200
...
def super_admin_get_orphan_user_status(user_id: int):
    try:
        ...
        db.session.rollback()
        return (
            jsonify({"success": False, "error": "Failed to fetch orphan user status"}),
            500,
        )
...
def super_admin_delete_orphan_user(user_id: int):
    ...
    try:
        ok, result = OrphanCleanupService.delete_orphan_user(
            user_id, actor_user_id=actor.id
        )
        if not ok:
            return jsonify({"success": False, "status": result}), 400
        ...
        _audit(
            "super_admin.orphan_user_deleted",
            actor.id,
            {"user_id": user_id, "deleted": result.get("deleted")},
            severity="warning",
        )
...
def super_admin_force_purge_user(user_id: int):
    ...
    if not ok:
        return (
            jsonify(
                {
                    "success": False,
                    "error": result.get("error", "Force purge failed"),
                    "detail": result,
                }
            ),
            400,
    ...
    _audit(
        "super_admin.user_force_purged",
        actor.id,
        {"user_id": user_id, "purged": True},
        severity="critical",
    )
    return jsonify({"success": True, "result": result}), 200
