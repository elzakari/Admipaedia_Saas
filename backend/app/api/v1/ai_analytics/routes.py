...
def predict_student_performance(student_id):
    ...
    try:
        ...
        result = AIAnalyticsService.predict_student_performance(
            student_id=student_id, prediction_period=prediction_period
        )
...
@ai_analytics_bp.route(
    "/recommendations/<string:user_role>/<int:user_id>", methods=["GET"]
)
@jwt_required()
@require_permission("view_analytics")
...
def batch_risk_assessment():
    ...
    try:
        ...
        if not data or "student_ids" not in data:
            return (
                jsonify({"success": False, "message": "student_ids is required"}),
                400,
            )
...
def get_school_wide_insights():
    ...
    try:
        ...
        include_predictions = (
            request.args.get("include_predictions", "true").lower() == "true"
        )
