...
def update_event(event_id):
    ...
    try:
        ...
        # Update event
        event = CalendarService.update_event(event_id=event_id, **data)
...
def get_shared_events():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": "Failed to fetch shared events"}),
            500,
        )
...
def export_events():
    ...
    try:
        ...
        response = current_app.response_class(cal.to_ical(), mimetype="text/calendar")
        response.headers["Content-Disposition"] = "attachment; filename=calendar.ics"
        return response

    except Exception as e:
        current_app.logger.error(f"Error exporting events: {str(e)}")
        return jsonify({"success": False, "message": "Failed to export events"}), 500
...
def import_events():
    ...
    try:
        ...
        return jsonify({"success": False, "message": "Failed to import events"}), 500
...
def update_academic_term(term_id):
    ...
    term = AcademicTermService.update_term(
        term_id, getattr(g, "tenant_id", None), **data
    )
    if not term:
