...
from app.middleware.security_middleware import (log_security_event, rate_limit,
                                                ...
                                                security_headers)
from app.models.security import PasswordHistory, SecurityEvent
...
class BulkUserActionSchema(Schema):
    ...
    action = fields.String(
        required=True, validate=validate.OneOf(["activate", "deactivate", "delete"])
    )
...
def update_user(user_id):
    ...
    try:
        ...
        if (
            target_role in ("super_admin", "super_manager")
            and actor_role != "super_admin"
        ):
            return jsonify({"success": False, "error": "Unauthorized"}), 403
        ...
        for field, value in data.items():
            if field == "role":
                if (
                    value in ("super_admin", "super_manager")
                    and actor_role != "super_admin"
                ):
                    return jsonify({"success": False, "error": "Unauthorized"}), 403
                # Handle role update
...
def delete_user(user_id):
    ...
    try:
        ...
        if not status.get("exists"):
            return (
                jsonify(
                    {"success": False, "error": "User not found", "status": status}
                ),
                404,
            )
        if not status.get("can_delete"):
            ...
            return (
                jsonify(
                    {"success": False, "error": "Cannot delete user", "status": status}
                ),
                code,
            )
        ...
        if not ok:
            return (
                jsonify(
                    {"success": False, "error": "Cannot delete user", "status": result}
                ),
                400,
            )
...
def bulk_user_action():
    ...
    try:
        ...
        if len(users) != len(user_ids):
            return jsonify({"success": False, "error": "Some users not found"}), 404
...
def get_user_statistics():
    ...
    try:
        ...
        # Role distribution
        role_stats = (
            db.session.query(Role.name, db.func.count(user_roles.c.user_id))
            .join(user_roles)
            .group_by(Role.name)
            .all()
        )
        ...
        recent_registrations = (
            User.query.filter(User.created_at >= thirty_days_ago).count()
...
def admin_reset_password(user_id):
    ...
    try:
        ...
        log_security_event(
            ...
            {"target_user_id": user.id, "reset_by": get_jwt_identity()},
        )
    ...
    try:
        ...
        if "password" in data or "new_password" in data:
            ...
            if not user.check_password_hash(current_password):
                return (
                    jsonify({"success": False, "error": "Invalid current password"}),
                    400,
                )
        ...
        log_security_event(
            ...
            {"user_id": user.id, "updated_fields": list(data.keys())},
        )
