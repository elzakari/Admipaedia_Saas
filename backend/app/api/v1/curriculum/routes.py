...
def _parse_curriculum_standard(value):
    ...
    try:
        ...
        raise ValueError(
            f"Invalid curriculum_standard. Allowed values: {allowed}"
        ) from exc
...
def _serialize_curriculum(curriculum, include_details=False):
    ...
    curriculum_data = {
        ...
        "educational_level": (
            {
                ...
                "age_range": (
                    ...
                    and educational_level.min_age is not None
                    and educational_level.max_age is not None
                    else None
                ),
            }
            if educational_level
        ...
        "curriculum_standard": (
            curriculum.curriculum_standard.value
            if curriculum.curriculum_standard
            else None
        ),
        ...
        "created_at": (
            curriculum.created_at.isoformat() if curriculum.created_at else None
        ),
        "updated_at": (
            curriculum.updated_at.isoformat() if curriculum.updated_at else None
        ),
    ...
    if include_details:
        ...
        curriculum_data["units"] = [
            _serialize_curriculum_unit(unit) for unit in curriculum.units
        ]
        curriculum_data["creator"] = (
            {
                "id": curriculum.creator.id,
                "name": (
                    curriculum.creator.get_full_name()
                    if hasattr(curriculum.creator, "get_full_name")
                    else curriculum.creator.username
                ),
            }
            if curriculum.creator
            else None
        )
        curriculum_data["approver"] = (
            {
                ...
                "name": (
                    curriculum.approver.get_full_name()
                    if hasattr(curriculum.approver, "get_full_name")
                    else curriculum.approver.username
                ),
            }
            if curriculum.approver
            else None
        )
...
def get_curricula():
    ...
    try:
        ...
        if educational_level_id:
            query = query.filter(
                Curriculum.educational_level_id == educational_level_id
            )
        if subject_id:
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error retrieving curricula: {str(e)}"}
            ),
            500,
        )
...
def get_curriculum(curriculum_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error retrieving curriculum: {str(e)}"}
            ),
            500,
        )
...
def create_curriculum():
    ...
    try:
        ...
        if not educational_level:
            return (
                jsonify({"success": False, "message": "Educational level not found"}),
                400,
            )
        if not subject:
        ...
        if sum(weights.values()) != 100.0:
            return (
                jsonify(
                    {"success": False, "message": "Competency weights must sum to 100%"}
                ),
                400,
            )
        ...
        curriculum = Curriculum(
            ...
            curriculum_standard=_parse_curriculum_standard(
                data.get("curriculum_standard")
            ),
            ...
            jsonify(
                {
                    ...
                    "curriculum": _serialize_curriculum(
                        curriculum, include_details=True
                    ),
                }
            ),
            201,
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error creating curriculum: {str(e)}"}
            ),
            400,
        )
    except Exception as e:
...
def update_curriculum(curriculum_id):
    ...
    try:
        ...
        if "curriculum_standard" in data:
            curriculum.curriculum_standard = _parse_curriculum_standard(
                data.get("curriculum_standard")
            )
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error updating curriculum: {str(e)}"}
            ),
            500,
        )
...
def delete_curriculum(curriculum_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": f"Error deleting curriculum: {str(e)}"}
            ),
            500,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Error retrieving curriculum units: {str(e)}",
                }
            ),
            500,
        )
...
def create_curriculum_unit(curriculum_id=None):
    ...
    try:
        ...
        if not resolved_curriculum_id:
            return (
                jsonify({"success": False, "message": "curriculum_id is required"}),
                400,
            )
        ...
        unit = CurriculumUnit(
            ...
            objectives=_coerce_text(
                data.get("objectives")
                or data.get("key_concepts")
                or data.get("learning_activities")
            ),
            resources=_coerce_text(
                data.get("resources") or data.get("resources_required")
            ),
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Error creating curriculum unit: {str(e)}",
                }
            ),
            500,
        )
...
def update_curriculum_unit(unit_id):
    ...
    try:
        ...
        if (
            "objectives" in data
            or "key_concepts" in data
            or "learning_activities" in data
        ):
            unit.objectives = _coerce_text(
                data.get("objectives")
                or data.get("key_concepts")
                or data.get("learning_activities")
            )
        if "resources" in data or "resources_required" in data:
            unit.resources = _coerce_text(
                data.get("resources") or data.get("resources_required")
            )
        if "duration_weeks" in data:
            unit.duration_weeks = int(data["duration_weeks"])
        elif "start_week" in data and "end_week" in data:
            unit.duration_weeks = max(
                int(data["end_week"]) - int(data["start_week"]) + 1, 1
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Error updating curriculum unit: {str(e)}",
                }
            ),
            500,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Error deleting curriculum unit: {str(e)}",
                }
            ),
            500,
        )
...
def get_learning_objectives(curriculum_id):
    ...
    try:
        ...
        return (
            jsonify({"success": True, "data": objectives, "total": len(objectives)}),
            200,
        )
...
def create_learning_objective(curriculum_id):
    ...
    try:
        ...
        existing_objective = LearningObjective.query.filter_by(
            curriculum_id=curriculum_id, objective_code=data["objective_code"]
        ).first()
...
def get_student_competency_mapping(student_id):
    ...
    try:
        ...
        sba_records = EnhancedSBA.query.filter_by(
            student_id=student_id, academic_year=str(current_year)
        ).all()
        ...
        if sba_records:
                ...
                competency_data["sba_summary"].append(
                    {
                        ...
                        "assessment_date": (
                            sba.assessment_date.isoformat()
                            if sba.assessment_date
                            else None
                        ),
                    }
                )
            ...
            for competency in competency_sums:
                competency_data["core_competencies"][competency] = (
                    competency_sums[competency] / total_records
                )
        ...
        if progression:
            competency_data["progression_status"] = {
                "current_level": (
                    progression.current_educational_level.level_name
                    if progression.current_educational_level
                    else None
                ),
                "next_level": (
                    progression.next_educational_level.level_name
                    if progression.next_educational_level
                    else None
                ),
                ...
                "promotion_status": (
                    progression.promotion_status.value
                    if progression.promotion_status
                    else None
                ),
                "promotion_decision_date": (
                    progression.promotion_decision_date.isoformat()
                    if progression.promotion_decision_date
                    else None
                ),
            }

            competency_data["overall_progress"] = (
                progression.overall_academic_average or 0.0
            )

        return jsonify({"success": True, "data": competency_data}), 200
...
def get_curriculum_standards():
    ...
    try:
        standards = [
            {"value": standard.value, "name": standard.name.replace("_", " ").title()}
            for standard in CurriculumStandard
        ]

        objective_types = [
            {"value": obj_type.value, "name": obj_type.name.replace("_", " ").title()}
