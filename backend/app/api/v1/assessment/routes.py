...
from app.models import (AssessmentFramework, AssessmentRubric, AssessmentTask,
                        ContinuousAssessmentRecord, DifferentiatedAssessment,
                        SchoolBasedAssessment)
from app.models.assessment_methods import (AssessmentMode, AssessmentScore,
...
def _current_teacher_profile():
    ...
    if not teacher:
        raise PermissionError(
            "An active teacher profile is required for assessment entry"
        )
    return teacher
...
def _current_student_profile():
    ...
    if not student:
        raise PermissionError(
            "An active student profile is required for assessment submission"
        )
    return student
...
def create_assessment_framework():
    ...
    try:
        ...
        educational_level_id = _parse_int(
            data.get("educational_level_id"), "educational_level_id"
        )
        ...
        framework = AssessmentFramework(
            ...
            formative_weight=_parse_float(
                data.get("formative_weight"), "formative_weight", default=30.0
            ),
            summative_weight=_parse_float(
                data.get("summative_weight"), "summative_weight", default=40.0
            ),
            school_based_weight=_parse_float(
                data.get("school_based_weight"), "school_based_weight", default=20.0
            ),
            project_weight=_parse_float(
                data.get("project_weight"), "project_weight", default=10.0
            ),
...
def create_assessment_task():
    ...
    try:
        ...
        task = AssessmentTask(
            ...
            assessment_type=_parse_enum(
                data.get("assessment_type"), AssessmentType, "assessment_type"
            ),
            assessment_mode=_parse_enum(
                data.get("assessment_mode"), AssessmentMode, "assessment_mode"
            ),
            scheduled_date=(
                _parse_date(data.get("scheduled_date"), "scheduled_date")
                if data.get("scheduled_date") not in (None, "")
                else None
            ),
            duration_minutes=(
                _parse_int(data.get("duration_minutes"), "duration_minutes")
            ...
            pass_mark=(
                _parse_int(data.get("pass_mark"), "pass_mark")
                if data.get("pass_mark") not in (None, "")
                else None
            ),
...
def submit_assessment():
    ...
    try:
        ...
        submission.submitted_at = _parse_datetime(
            data.get("submitted_at"), "submitted_at", default=datetime.utcnow()
        )
...
def score_assessment():
    ...
    try:
        ...
        percentage_score = (
            round((raw_score / max_marks) * 100, 2) if max_marks > 0 else None
        )
        ...
        score = AssessmentScore(
            ...
            rubric_id=(
                _parse_int(data.get("rubric_id"), "rubric_id")
                if data.get("rubric_id") not in (None, "")
                else None
            ),
            ...
            criterion_scores=(
                ...
                if isinstance(data.get("criterion_scores"), dict)
                else None
            ),
...
def create_school_based_assessment():
    ...
    try:
        ...
        marking_scheme = (
            data.get("marking_scheme")
            if isinstance(data.get("marking_scheme"), dict)
            else {}
        )
        class_test_scores = _parse_score_list(
            data.get("class_test_scores", marking_scheme.get("class_test_scores"))
        )
        class_exercises_score = _parse_float(
            data.get(
                "class_exercises_score", marking_scheme.get("class_exercises_score")
            ),
        ...
        class_test_average = _parse_float(
            ...
            "class_test_average",
            default=(
                sum(class_test_scores) / len(class_test_scores)
                if class_test_scores
                else 0.0
            ),
        )
        ...
        if total_sba_score in (None, 0.0) and data.get("total_marks") not in (None, ""):
            total_sba_score = _parse_float(
                data.get("total_marks"), "total_marks", default=0.0
            )
        ...
        if explicit_percentage not in (None, ""):
            sba_percentage = _parse_float(
                explicit_percentage, "sba_percentage", default=0.0
            )
        ...
        sba = SchoolBasedAssessment(
            ...
            core_competencies_score=data.get(
                "core_competencies_score", marking_scheme.get("core_competencies_score")
            ),
            subject_competencies_score=data.get(
                "subject_competencies_score",
                marking_scheme.get("subject_competencies_score"),
            ),
            teacher_id=teacher.id,
...
def record_continuous_assessment(student_id):
    ...
    try:
        ...
        derived_class_score = _parse_float(
            data.get("class_score"), "class_score", default=None
        )
        ...
        homework_score = _parse_float(
            data.get("homework_score"), "homework_score", default=None
        )
        participation_score = _parse_float(
            data.get("participation_score"), "participation_score", default=None
        )
        ...
        if raw_score is not None:
            ...
            if participation_score is None and assessment_type in {
                "participation",
                "classwork",
                "oral",
            }:
                participation_score = raw_score
            if quiz_score is None and assessment_type in {
                "quiz",
                "test",
                "class_test",
                "continuous",
        ...
        record = ContinuousAssessmentRecord(
            ...
            term=term,
            week_number=(
                _parse_int(data.get("week_number"), "week_number")
                if data.get("week_number") not in (None, "")
                else None
            ),
            assessment_date=assessment_date,
            assessment_focus=(
                str(data.get("assessment_focus") or "").strip()
                or assessment_type
                or None
            ),
            ...
            teacher_observations=data.get("teacher_observations")
