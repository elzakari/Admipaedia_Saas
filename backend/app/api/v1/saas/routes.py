...
from app.models.service_tokens import (TenantServiceToken,
                                       TenantServiceTokenEvent,
                                       TenantServiceTokenUsage)
from app.models.tenant import Tenant
from app.services.integrations.token_service import (SERVICE_TYPES,
                                                     ServiceTokenService,
                                                     current_period_ym)
...
def create_tenant():
    ...
    return (
        jsonify({"success": True, "tenant": SaaSService.serialize_tenant(tenant)}),
        201,
    )
...
def list_my_tenants():
    user_id = get_jwt_identity()
    return (
        jsonify({"success": True, "items": SaaSService.get_user_tenants(int(user_id))}),
        200,
    )
...
def create_invoice(tenant_id):
    ...
    if not issued_on:
        return (
            jsonify(
                {"success": False, "message": "issued_on is required (YYYY-MM-DD)"}
            ),
            400,
        )
    ...
    if err:
        ...
        return jsonify({"success": False, "message": err}), status
    return (
        jsonify({"success": True, "invoice": SaaSService.serialize_invoice(inv)}),
        201,
    )
...
def record_payment(tenant_id):
    ...
    if not paid_on:
        return (
            jsonify({"success": False, "message": "paid_on is required (YYYY-MM-DD)"}),
            400,
        )
    ...
    if err:
        ...
        return jsonify({"success": False, "message": err}), status
    return (
        jsonify({"success": True, "payment": SaaSService.serialize_payment(payment)}),
        201,
    )
...
def platform_update_tenant(tenant_id):
    ...
    tenant, err = SaaSService.platform_update_tenant(
        tenant_id, status=data.get("status"), plan=data.get("plan")
    )
    if err:
        return jsonify({"success": False, "message": err}), 404
    return (
...
def platform_list_service_tokens(tenant_id):
    ...
    usage_rows = TenantServiceTokenUsage.query.filter_by(
        tenant_id=tenant_uuid, year=year, month=month
    ).all()
    ...
    return (
        jsonify(
            {"success": True, "tokens": items, "period": {"year": year, "month": month}}
        ),
        200,
    )
...
@saas_bp.route(
    "/platform/tenants/<tenant_id>/service-tokens/provision", methods=["POST"]
)
@require_platform_super_admin()
def platform_provision_service_tokens(tenant_id):
    ...
    issued = ServiceTokenService.provision_for_tenant(
        ...
        db.session.add(
            SecurityEvent(
                ...
                details={
                    "tenant_id": tenant_id,
                    "issued": {
                        k: ("********" if v else None) for k, v in issued.items()
                    },
                },
                severity="info",
            )
...
@saas_bp.route(
    "/platform/tenants/<tenant_id>/service-tokens/<service_type>/rotate",
    methods=["POST"],
)
@require_platform_super_admin()
def platform_rotate_service_token(tenant_id, service_type):
    ...
    try:
        token_plain = ServiceTokenService.rotate_token(
            tenant_id, service_type, actor_user_id=actor_user_id
        )
        from app.extensions import db
...
@saas_bp.route(
    "/platform/tenants/<tenant_id>/members/<membership_id>", methods=["PATCH"]
)
@require_platform_super_admin()
def platform_patch_membership(tenant_id, membership_id):
    updates = request.get_json() or {}
    membership, err = SaaSService.platform_update_membership(
        tenant_id, membership_id, updates
    )
    if err:
...
@saas_bp.route(
    "/platform/tenants/<tenant_id>/members/<membership_id>", methods=["DELETE"]
)
@require_platform_super_admin()
...
def platform_financial_summary():
    return (
        jsonify({"success": True, "summary": SaaSService.platform_financial_summary()}),
        200,
    )
...
def complete_registration_link():
    ...
    try:
        ...
        confirm_password = str(
            data.get("confirm_password") or data.get("confirmPassword") or ""
        ).strip()
...
def complete_setup():
    ...
    if not education_system or not str(education_system).strip():
        return (
            jsonify(
                {"success": False, "message": "Education system framework is required"}
            ),
            400,
        )
    if not academic_year_name or not str(academic_year_name).strip():
        return (
            jsonify({"success": False, "message": "Academic year name is required"}),
            400,
        )
    ...
    if not term_start_date or not term_end_date:
        return (
            jsonify(
                {"success": False, "message": "Term start and end dates are required"}
            ),
            400,
        )
    ...
    try:
        try:
            EducationalSystemService.apply_template_to_tenant(
                education_system, tenant_id
            )
        except ValueError as legacy_err:
            logger.warn(
                "legacy_system_setup_skipped",
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": f"Failed to apply educational system: {str(e)}",
                }
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Invalid term start or end date format"}
            ),
            400,
        )
    ...
    if not existing_academic_year:
        ...
        academic_year = AcademicYear(
            name=year_name, start_date=y_start, end_date=y_end, is_current=True
        )
    ...
    existing_term = Term.query.filter_by(
        academic_year_id=active_year_id, name=term_name_clean
    ).first()
    ...
    try:
        ...
        db.session.rollback()
        return (
            jsonify({"success": False, "message": f"Database save failed: {str(e)}"}),
            500,
        )
...
def patch_admission_status(form_id):
    ...
    application = (
        AdmissionApplication.query.filter_by(id=form_id).with_for_update().first()
    )
    if not application:
        return (
            jsonify({"success": False, "message": "Admission application not found"}),
            404,
        )
    ...
    if not next_status:
        return (
            jsonify({"success": False, "message": "status parameter is required"}),
            400,
        )
    ...
    if next_status not in (
        ...
        return (
            jsonify({"success": False, "message": f"Invalid status: {next_status}"}),
            400,
        )
        ...
        if user.role != "parent":
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Only parents can submit applications",
                    }
                ),
                403,
            )
    ...
    if next_status == "approved":
                ...
                jsonify(
                    {
                        ...
                        "data": {
                            ...
                            "student_id": (
                                existing_student.id if existing_student else None
                            ),
                        },
                    }
                ),
        ...
        tenant_id = (
            application.target_class.tenant_id if application.target_class else None
        ) or (application.parent.tenant_id if application.parent else None)
        if not tenant_id:
            return (
                jsonify(
                    {"success": False, "message": "Could not resolve tenant context"}
                ),
                400,
            )
        ...
        if not applicant_email and application.form_data:
            ...
            if isinstance(form_dict, dict):
                applicant_email = form_dict.get("student_email") or form_dict.get(
                    "email"
                )
        ...
        first_name_val = getattr(application, "student_first_name", None) or getattr(
            application, "first_name", ""
        )
        last_name_val = getattr(application, "student_last_name", None) or getattr(
            application, "last_name", ""
        )
        dob_val = getattr(application, "student_date_of_birth", None) or getattr(
        ...
        try:
            ...
            classroom_id = (
                form_data.get("classroom_id")
                or form_data.get("class_id")
                or application.target_class_id
            )
            ...
            student, err = student_service.create_student(
            ...
            parent_email = (
                ...
                if (application.parent and application.parent.user)
                else None
            )
            if not parent_email:
                parent_email = form_data.get("father_email") or form_data.get(
                    "mother_email"
                )
        ...
        jsonify(
            {
                ...
                "email": {
                    "status": (
                        "Connected (Active)"
                        if email_status.allowance is not None
                        else "Disconnected"
                    ),
                    "remaining": (
                        "Unlimited"
                        if email_status.unlimited
                        else email_status.remaining
                    ),
                    "allowance": (
                        "Unlimited"
                        if email_status.unlimited
                        else email_status.allowance
                    ),
                ...
                "sms": {
                    "status": (
                        "Connected (Active)"
                        if sms_status.allowance is not None
                        else "Disconnected"
                    ),
                    "remaining": (
                        "Unlimited" if sms_status.unlimited else sms_status.remaining
                    ),
                    "allowance": (
                        "Unlimited" if sms_status.unlimited else sms_status.allowance
                    ),
