...
from app.schemas.teacher import TeacherListSchema, TeacherSchema
...
def get_teacher(teacher_id):
    ...
    return jsonify({"success": True, "teacher": teacher_schema.dump(teacher)}), 200
...
def create_teacher():
    ...
    try:
        ...
        try:
            teacher, error = TeacherService.create_teacher(
            ...
            if isinstance(e, IntegrityError):
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Duplicate teacher data",
                            "error": str(e),
                        }
                    ),
                    409,
        ...
        if all([minimal["user_id"], minimal["first_name"], minimal["last_name"]]):
            teacher, error = TeacherService.create_teacher(
                minimal, tenant_id=getattr(g, "tenant_id", None)
            )
            if teacher:
                tdata = teacher_schema.dump(teacher)
                tdata["name"] = f"{teacher.first_name} {teacher.last_name}"
...
def update_teacher_status(teacher_id):
    ...
    try:
        ...
        if teacher and getattr(teacher, "tenant_id", None) != getattr(
            g, "tenant_id", None
        ):
            return jsonify({"success": False, "message": "Teacher not found"}), 404
...
def get_teacher_classes(teacher_id):
    ...
    try:
        ...
        if not is_authorized:
            return (
                jsonify({"error": "Insufficient permissions to read teacher classes"}),
                403,
            )
        ...
        if status:
            ...
            if getattr(g, "tenant_id", None) is not None:
                query = query.filter(
                    ClassModel.tenant_id == getattr(g, "tenant_id", None)
                )
            ...
            paginated_classes = query.paginate(
        ...
        if not teacher_record:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Teacher record not found for the authenticated user",
                    }
                ),
                404,
            )
...
def assign_teacher_class(teacher_id):
    ...
    class_obj, error = ClassService.assign_teacher(
        class_id=int(class_id), teacher_id=teacher_id
    )
    if error:
        return jsonify({"success": False, "message": error}), 400
...
def unassign_teacher_class(teacher_id, class_id):
    ...
    success, error = ClassService.unassign_teacher(
...
from app.schemas.teacher_attendance import (TeacherAttendanceCreateSchema,
...
def get_teacher_attendance(teacher_id):
    ...
    return (
        jsonify(
            {
                "success": True,
                "attendance": teacher_attendances_schema.dump(
                    paginated_attendance.items
                ),
                "pagination": {
    ...
    try:
        ...
        if not is_authorized:
            return (
                jsonify(
                    {
                        "error": "Insufficient permissions to read teacher schedule assets"
                    }
                ),
                403,
            )
