...
def paystack_webhook():
    ok, msg = PaymentService.handle_webhook(
        gateway_name="paystack", headers=_headers_dict(), body=request.get_data() or b""
    )
...
def cinetpay_webhook():
    ok, msg = PaymentService.handle_webhook(
        gateway_name="cinetpay", headers=_headers_dict(), body=request.get_data() or b""
    )
    status = 200 if ok else 400
    return jsonify({"success": ok, "message": msg}), status
