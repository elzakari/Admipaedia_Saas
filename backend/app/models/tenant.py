...
class Tenant(db.Model):
    ...
    enabled_features = db.Column(
        JSON().with_variant(db.ARRAY(db.Text), "postgresql"), default=[]
    )
    ...
    def education_system(self):
        if isinstance(self.settings, dict):
            setting_val = self.settings.get("education_system") or self.settings.get(
                "educational_system"
            )
            if setting_val:
                return setting_val
        ...
        cfg = EducationalSystemConfig.query.filter_by(
            tenant_id=self.id, is_active=True
        ).first()
...
class PlatformAuditLog(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("tenants.id"), nullable=True
    )
    actor_id = db.Column(
        UUID(as_uuid=True), nullable=True
    )  # User ID (might be from different schemas)
    ...
    ip_address = db.Column(
        db.String(45).with_variant(INET(), "postgresql"), nullable=True
    )
...
class TenantInvitation(db.Model):
    ...
    __table_args__ = (db.Index("idx_tenant_invite_token", "token"),)
...
class PlatformPayment(db.Model):
    ...
    invoice_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("platform_invoices.id"), nullable=True
    )
