...
class CalendarEvent(db.Model):
    ...
    creator = db.relationship(
        "User", backref="created_events", foreign_keys=[created_by]
    )

    # Define relationship with shared roles (many-to-many)
    shared_with_roles = db.relationship(
        "Role",
        secondary="event_role_association",
        backref=db.backref("shared_events", lazy="dynamic"),
    )


# Association table for many-to-many relationship between events and roles
event_role_association = db.Table(
    "event_role_association",
    db.Column(
        "event_id", db.String(36), db.ForeignKey("calendar_events.id"), primary_key=True
    ),
...
class Notification(db.Model):
    ...
    scope = db.Column(
        db.String(50), nullable=True, default="all"
    )  # all, teachers, students, parents, admins
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(
        db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow
    )
    ...
    user = db.relationship(
        "User", backref=db.backref("notifications", lazy=True), foreign_keys=[user_id]
    )
