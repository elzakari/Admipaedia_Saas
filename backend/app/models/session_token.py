...
class SessionToken(db.Model):
    ...
    user = db.relationship(
        "User",
        backref=db.backref(
            "session_tokens", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
    ...
    @classmethod
    def create_token(
        cls, jti, user_id, token_type, expires_at, ip_address=None, user_agent=None
    ):
    ...
    def cleanup_expired_tokens(cls):
        ...
        expired_tokens = cls.query.filter(
            cls.expires_at < now, cls.is_revoked == False
        ).all()
    ...
    def get_user_active_sessions(cls, user_id):
        ...
        now = datetime.utcnow()
        return (
            cls.query.filter_by(user_id=user_id, is_revoked=False, token_type="access")
    ...
    def get_session_stats(cls, user_id):
        ...
        now = datetime.utcnow()
        active_tokens = cls.query.filter_by(user_id=user_id, is_revoked=False).filter(
            cls.expires_at > now
        )
