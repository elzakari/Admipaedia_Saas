...
class TenantServiceProviderOverride(db.Model, _EncryptedConfigMixin):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            ...
            "service_type",
            "provider_key",
            name="uq_tenant_provider_override",
        ),
...
class TenantServiceToken(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "tenant_id", "service_type", name="uq_tenant_service_token"
        ),
        db.Index("ix_tenant_service_tokens_tenant", "tenant_id"),
        db.Index(
            "ix_tenant_service_tokens_active", "tenant_id", "service_type", "status"
        ),
...
class TenantServiceTokenUsage(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            ...
            "year",
            "month",
            name="uq_tenant_service_token_usage_period",
        ),
...
class TenantServiceTokenEvent(db.Model):
    ...
    token_id = db.Column(
        _uuid_type(), db.ForeignKey("tenant_service_tokens.id"), nullable=True
    )
    service_type = db.Column(db.String(32), nullable=False)
