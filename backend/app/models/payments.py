...
from app.utils.secret_crypto import decrypt_value, encrypt_value
...
class PaymentGateway(db.Model):
    ...
    __table_args__ = (
        db.Index(
            "idx_payment_gateways_country_currency_active",
            "country_code",
            "currency",
            "is_active",
        ),
...
class Payment(db.Model):
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint(
            "gateway_name",
            "payment_reference",
            name="uq_billing_invoice_payments_gateway_reference",
        ),
    )
    ...
    invoice_id = db.Column(
        db.Integer, db.ForeignKey("billing_invoices.id"), nullable=False
    )
    ...
    payment_gateway_id = db.Column(
        db.Integer, db.ForeignKey("payment_gateways.id"), nullable=True
    )
    ...
    gateway = db.relationship(
        "PaymentGateway", backref=db.backref("payments", lazy=True)
    )
