...
class Analytics(db.Model):
    ...
    )  # e.g., 'student_performance', 'attendance_rate'
    metric_name = db.Column(
        db.String(200), nullable=False
    )  # e.g., 'average_grade', 'monthly_attendance'
    metric_value = db.Column(db.Float, nullable=False)
    entity_type = db.Column(
        db.String(50), nullable=True
    )  # e.g., 'student', 'class', 'school'
    ...
    def __repr__(self):
        return (
            f"<Analytics {self.metric_type}: {self.metric_name} = {self.metric_value}>"
        )
