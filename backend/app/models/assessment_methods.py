...
class AssessmentFramework(db.Model):
    ...
    educational_level_id = Column(
        Integer, ForeignKey("educational_levels.id"), nullable=False
    )
    ...
    # Relationships
    educational_level = relationship(
        "EducationalLevel", backref="assessment_frameworks"
    )
...
class AssessmentTask(db.Model):
    ...
    description = Column(Text)
    framework_id = Column(
        Integer, ForeignKey("assessment_frameworks.id"), nullable=False
    )
...
class AssessmentScore(db.Model):
    ...
    submission_id = Column(
        Integer, ForeignKey("assessment_submissions.id"), nullable=False
    )
...
class SchoolBasedAssessment(db.Model):
    ...
    subject = relationship(
        "Subject", backref="school_based_assessments"
    )  # Changed from 'sba_records'
    class_ = relationship("Class", backref="sba_records")
    teacher = relationship(
        "Teacher", foreign_keys=[teacher_id], backref="sba_assessments"
    )
    moderator = relationship(
        "Teacher", foreign_keys=[moderated_by], backref="sba_moderations"
    )
