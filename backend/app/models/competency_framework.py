...
class CompetencyIndicator(db.Model):
    ...
    min_educational_level = db.Column(
        db.Integer, nullable=False
    )  # Sequence order from educational_levels
...
class StudentCompetencyProfile(db.Model):
    ...
    areas_for_improvement = db.Column(
        db.JSON, nullable=True
    )  # Areas needing development
    recommended_activities = db.Column(
        db.JSON, nullable=True
    ...
    __table_args__ = (
        CheckConstraint(
            "overall_score >= 0 AND overall_score <= 4",
            name="overall_score_range_check",
        ),
        db.UniqueConstraint(
            "student_id", "academic_year", name="unique_student_year_profile"
        ),
    )
...
class CompetencyEvidence(db.Model):
    ...
    assessment_id = db.Column(
        db.Integer, db.ForeignKey("student_competency_assessments.id"), nullable=False
    )
    indicator_id = db.Column(
        db.Integer, db.ForeignKey("competency_indicators.id"), nullable=False
    )

    # Evidence details
    evidence_type = db.Column(
        db.String(50), nullable=False
    )  # observation, artifact, performance, etc.
    ...
    assessment = db.relationship(
        "StudentCompetencyAssessment", backref="evidence_items"
    )
...
class CompetencyLearningActivity(db.Model):
    ...
    activity_type = db.Column(
        db.String(50), nullable=False
    )  # project, discussion, simulation, etc.
    ...
    # Educational context
    suitable_educational_levels = db.Column(
