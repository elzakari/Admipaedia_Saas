...
class GradingScheme(db.Model):
    ...
    educational_level_id = db.Column(
        db.Integer, db.ForeignKey("educational_levels.id"), nullable=True
    )
    grade_level_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("grade_levels.id"), nullable=True, index=True
    )
    ...
    class_score_weight = db.Column(
        db.Float, default=40.0
    ...
    grade_boundaries = db.relationship(
        "GradeBoundary", backref="grading_scheme", cascade="all, delete-orphan"
    )

    # Constraints
    __table_args__ = (
        CheckConstraint(
            "class_score_weight + external_exam_weight = 100", name="weight_sum_check"
        ),
        CheckConstraint(
            "class_score_weight >= 0 AND class_score_weight <= 100",
            name="class_weight_range_check",
        ),
        CheckConstraint(
            "external_exam_weight >= 0 AND external_exam_weight <= 100",
            name="external_weight_range_check",
        ),
    )
...
class GradeBoundary(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    grading_scheme_id = db.Column(
        db.Integer, db.ForeignKey("grading_schemes.id"), nullable=False
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "min_score >= 0 AND min_score <= 100", name="min_score_range_check"
        ),
        CheckConstraint(
            "max_score >= 0 AND max_score <= 100", name="max_score_range_check"
        ),
...
class EnhancedGrade(db.Model):
    ...
    assessment_type_id = db.Column(
        db.Integer, nullable=False
    )  # Reference to assessment types
    ...
    is_class_score = db.Column(
        db.Boolean, default=True
    )  # True for continuous assessment
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "percentage >= 0 AND percentage <= 100", name="percentage_range_check"
        ),
    ...
    def calculate_grade(self):
        ...
        for boundary in sorted(
            self.grading_scheme.grade_boundaries, key=lambda x: x.sequence_order
        ):
...
class FinalGrade(db.Model):
    ...
    # Component scores
    class_score_average = db.Column(
        db.Float, nullable=True
    )  # Average of continuous assessments
    ...
    subject_rank = db.Column(
        db.Integer, nullable=True
    )  # Rank in subject across classes
