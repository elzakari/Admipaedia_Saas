...
class Announcement(db.Model):
    ...
    )  # all, students, parents, teachers, admins
    scope = db.Column(
        db.String(20), nullable=False, default="class_bound"
    )  # global, class_bound
    ...
    tenant = db.relationship(
        "Tenant", backref=db.backref("announcements", lazy="dynamic")
    )
    class_ = db.relationship(
        "Class", backref=db.backref("announcements", lazy="dynamic")
    )
    teacher = db.relationship(
        "Teacher", backref=db.backref("posted_announcements", lazy="dynamic")
    )
