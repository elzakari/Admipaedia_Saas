...
class ExternalExamination(db.Model):
    ...
    student_registrations = relationship(
        "ExternalExamRegistration", backref="examination", cascade="all, delete-orphan"
    )
    results = relationship(
        "ExternalExamResult", backref="examination", cascade="all, delete-orphan"
    )
...
class ExternalExamRegistration(db.Model):
    ...
    examination_id = Column(
        Integer, ForeignKey("external_examinations.id"), nullable=False
    )
    ...
    results = relationship(
        "ExternalExamResult", backref="registration", cascade="all, delete-orphan"
    )

    def __repr__(self):
        return (
            f"<ExternalExamRegistration {self.index_number}: Student {self.student_id}>"
        )
...
class ExternalExamResult(db.Model):
    ...
    registration_id = Column(
        Integer, ForeignKey("external_exam_registrations.id"), nullable=False
    )
    student_id = Column(Integer, ForeignKey("students.id"), nullable=False)
