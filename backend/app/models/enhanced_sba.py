...
class EnhancedSBA(db.Model):
    ...
    character_traits_scores = db.Column(
        db.JSON, nullable=True
    )  # Dict of trait_id: score
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "critical_thinking_score >= 1 AND critical_thinking_score <= 4"
        ),
    ...
    def calculate_total_sba(self):
        ...
        total_weight = (
            self.class_exercises_weight
            + self.homework_weight
            + self.project_work_weight
            + self.class_tests_weight
            + self.practical_work_weight
            + self.oral_assessment_weight
        )
