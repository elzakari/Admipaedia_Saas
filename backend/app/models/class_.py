...
class Class(db.Model):
    ...
    start_time = db.Column(
        db.String(20)
    )  # Store time as string in format like "08:00 AM"
    end_time = db.Column(
        db.String(20)
    )  # Store time as string in format like "03:30 PM"
...
class ClassTeacherMapping(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "class_id", "teacher_id", name="class_teacher_mappings_class_teacher_key"
        ),
    )
    ...
    class_ = db.relationship(
        "Class",
        backref=db.backref(
            "teacher_mappings", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
    teacher_user = db.relationship(
        "User",
        backref=db.backref(
            "class_mappings", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
