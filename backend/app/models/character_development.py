...
class CharacterTrait(db.Model):
    ...
    )  # e.g., "Honesty", "Respect", "Compassion"
    ...
    educational_level = relationship(
        "EducationalLevel", back_populates="character_traits"
    )
    assessments = relationship("CharacterAssessment", back_populates="trait")
    activities = relationship(
        "CharacterActivity", secondary="activity_traits", back_populates="target_traits"
    )
...
class CharacterActivity(db.Model):
    ...
    activity_type = Column(
        String(50)
    )  # "story_telling", "role_play", "community_service", etc.
...
activity_traits = db.Table(
    "activity_traits",
    Column(
        "activity_id", Integer, ForeignKey("character_activities.id"), primary_key=True
    ),
