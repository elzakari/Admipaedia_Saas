...
class LoginAttempt(db.Model):
    ...
    def get_recent_attempts(cls, identifier, minutes=30, limit=100):
        ...
        return (
            cls.query.filter(
                cls.identifier == identifier, cls.attempted_at >= time_limit
            )
...
class SecurityEvent(db.Model):
    ...
    def __repr__(self):
        return (
            f"<SecurityEvent {self.event_type} - {self.severity} at {self.created_at}>"
        )
...
class PasswordHistory(db.Model):
    ...
    user = db.relationship(
        "User", backref=db.backref("password_history", cascade="all, delete-orphan")
    )
...
class PasswordResetToken(db.Model):
    ...
    def validate_token(token: str):
        ...
        raw_row = (
            db.session.execute(query, {"token_hash": token_hash}).mappings().first()
        )
