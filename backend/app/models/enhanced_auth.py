...
class AuthenticationAttempt(db.Model):
    ...
    attempt_type = db.Column(
        db.String(20), nullable=False
    )  # 'login', 'mfa', 'password_reset'
    ...
    attempt_metadata = db.Column(
        db.JSON, nullable=True
    )  # Changed from 'metadata' to 'attempt_metadata'
...
class UserSecuritySettings(db.Model):
    ...
    user = db.relationship(
        "User", backref=db.backref("security_settings", uselist=False)
    )
