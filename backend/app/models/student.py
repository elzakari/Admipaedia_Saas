...
class Student(db.Model):
    ...
    profile_picture_locked = db.Column(
        db.Boolean, nullable=False, default=False, server_default=db.text("false")
    )
    class_id = db.Column(
        db.Integer, db.ForeignKey("classes.id"), nullable=True, index=True
    )
    parent_id = db.Column(
        db.Integer, db.ForeignKey("parents.id"), nullable=True, index=True
    )
    ...
    character_assessments = db.relationship(
        "CharacterAssessment", back_populates="student", lazy=True
    )
    ...
    def __init__(self, **kwargs):
        ...
        if "admission_number" in kwargs and kwargs["admission_number"]:
            kwargs["admission_number"] = re.sub(
                r"\s+", "-", str(kwargs["admission_number"]).strip()
            )
    ...
    )  # National ID or other official ID
...
def handle_student_billing_proration(mapper, connection, target):
    try:
        ...
        tenant_res = connection.execute(
            text(
                "SELECT country_code, currency, plan FROM tenants WHERE id = :tenant_id"
            ),
        ...
        count_res = connection.execute(
            text(
                "SELECT COUNT(*) FROM students WHERE tenant_id = :tenant_id AND status = 'active'"
            ),
            {"tenant_id": tenant_param},
        ).scalar()
        ...
        if plan_id:
            plan_res = connection.execute(
                text(
                    "SELECT id, name, slug, price_per_student, currency, billing_min_months FROM plans WHERE id = :plan_id"
                ),
                {"plan_id": plan_id},
            ).fetchone()
        elif plan_slug:
            plan_res = connection.execute(
                text(
                    "SELECT id, name, slug, price_per_student, currency, billing_min_months FROM plans WHERE slug = :plan_slug LIMIT 1"
                ),
                {"plan_slug": plan_slug},
            ).fetchone()
        else:
            plan_res = connection.execute(
                text(
                    "SELECT id, name, slug, price_per_student, currency, billing_min_months FROM plans WHERE slug = :tenant_plan LIMIT 1"
                ),
