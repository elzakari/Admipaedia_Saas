...
class StudentProgression(db.Model):
    ...
    meets_attendance_threshold = db.Column(
        db.Boolean, default=False
    )  # >= 75% attendance
    meets_competency_threshold = db.Column(
        db.Boolean, default=False
    )  # >= Level 2 in competencies
    ...
    __table_args__ = (
        CheckConstraint(
            "overall_academic_average >= 0 AND overall_academic_average <= 100"
        ),
        CheckConstraint("attendance_percentage >= 0 AND attendance_percentage <= 100"),
        CheckConstraint(
            "core_competencies_average >= 1 AND core_competencies_average <= 4"
        ),
        CheckConstraint(
            "character_development_score >= 1 AND character_development_score <= 4"
        ),
    )
    ...
    def evaluate_promotion_eligibility(self):
        ...
        if all(
            [
                self.meets_academic_threshold,
                self.meets_attendance_threshold,
                self.meets_competency_threshold,
                self.meets_age_requirement,
            ]
        ):
            self.promotion_status = PromotionStatus.PROMOTED
        elif (
            self.overall_academic_average >= 40.0 and self.attendance_percentage >= 60.0
        ):
