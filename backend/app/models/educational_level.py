...
class EducationalLevel(db.Model):
    ...
    level_code = db.Column(
        "code", db.String(10), unique=True, nullable=False
    )  # maps to migration 'code'
    level_name = db.Column(
        "name", db.String(100), nullable=False
    )  # maps to migration 'name'
    key_phase = db.Column(
        db.String(50), nullable=False
    )  # store raw string, aligns with migration
    # sequence_order removed: not present in migration
    min_age = db.Column(
        "age_range_start", db.Integer, nullable=True
    )  # maps to migration 'age_range_start'
    max_age = db.Column(
        "age_range_end", db.Integer, nullable=True
    )  # maps to migration 'age_range_end'
    curriculum_focus = db.Column(
        "description", db.Text, nullable=True
    )  # maps to migration 'description'
    ...
    character_traits = db.relationship(
        "CharacterTrait", back_populates="educational_level", lazy=True
    )
    ...
    def key_phase_description(self):
        ...
        val = (
            self.key_phase.value if hasattr(self.key_phase, "value") else self.key_phase
        )
    ...
    def get_by_key_phase(cls, key_phase):
        ...
        return (
            cls.query.filter_by(key_phase=key_phase_value, is_active=True)
            .order_by(cls.id)
            .all()
        )
    ...
    def get_next_level(cls, current_level_id):
        ...
        if not current:
            return None
        return (
            cls.query.filter(cls.id > current.id, cls.is_active == True)
            .order_by(cls.id)
            .first()
        )
...
class StudentCompetencyAssessment(db.Model):
    ...
    competency_id = db.Column(
        db.Integer, db.ForeignKey("core_competencies.id"), nullable=False
    )
    ...
    level_achieved = db.Column(
        db.Integer, nullable=False
    )  # 1=Beginning, 2=Developing, 3=Proficient, 4=Excellent
    ...
    def level_description(self):
        ...
        levels = {1: "Beginning", 2: "Developing", 3: "Proficient", 4: "Excellent"}
