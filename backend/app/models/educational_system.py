...
class EducationalSystemConfig(db.Model):
    ...
    grade_levels = db.relationship(
        "GradeLevel", backref="educational_system", lazy=True
    )
...
class GradeLevel(db.Model):
    ...
    educational_system_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("educational_system_config.id"), nullable=True
    )

    track_id = db.Column(
        UUID(as_uuid=True),
        db.ForeignKey("grade_tracks.id", ondelete="CASCADE"),
        nullable=True,
    )
    name = db.Column(db.String(255), nullable=False)
    ...
    next_level_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("grade_levels.id"), nullable=True
    )

    created_at = db.Column(db.DateTime(timezone=True), server_default=func.now())

    # Relationships
    next_level = db.relationship(
        "GradeLevel", remote_side=[id], backref="previous_level"
    )
