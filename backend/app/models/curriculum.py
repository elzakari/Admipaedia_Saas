...
class Curriculum(db.Model):
    ...
    creator = db.relationship(
        "User", foreign_keys=[created_by], backref="created_curricula"
    )
    approver = db.relationship(
        "User", foreign_keys=[approved_by], backref="approved_curricula"
    )

    # Constraints
    __table_args__ = (
        CheckConstraint(
            "critical_thinking_weight + creativity_weight + communication_weight + collaboration_weight = 100"
        ),
...
class LearningObjective(db.Model):
    ...
    curriculum_id = db.Column(
        db.Integer, db.ForeignKey("curricula.id", ondelete="CASCADE"), nullable=False
    )
    ...
    prerequisite_objectives = db.Column(
    ...
    def __repr__(self):
        return (
            f"<LearningObjective {self.objective_code}: {self.objective_text[:50]}...>"
        )


# Association table for curriculum-competency mapping
curriculum_competencies = db.Table(
    "curriculum_competencies",
    db.Column(
        "curriculum_id", db.Integer, db.ForeignKey("curricula.id"), primary_key=True
    ),
    db.Column(
        "competency_id",
        db.Integer,
        db.ForeignKey("core_competencies.id"),
        primary_key=True,
    ),
