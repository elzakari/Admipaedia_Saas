...
class GradeTrack(db.Model):
    ...
    )  # 'Maternelle', 'Primary', 'Lower Secondary', 'High School'
    numeric_level_rank = db.Column(db.Integer, nullable=False, default=1)
    created_at = db.Column(db.DateTime(timezone=True), server_default=func.now())

    # Relationships
    levels = db.relationship(
        "GradeLevel", backref="grade_track", lazy=True, cascade="all, delete-orphan"
    )
    grading_scales = db.relationship(
        "PolymorphicGradingScale",
        backref="grade_track",
        lazy=True,
