...
class User(db.Model):
    ...
    def get_full_name(self):
        ...
        if self.profile and (
            getattr(self.profile, "first_name", None)
            or getattr(self.profile, "last_name", None)
        ):
...
class LoginHistory(db.Model):
    ...
    user = db.relationship(
        "User",
        backref=db.backref(
            "login_history", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
