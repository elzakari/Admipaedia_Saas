...
class STEMDomain(db.Model):
    ...
    # Relationships
    stem_subjects = db.relationship(
        "STEMSubject", backref="stem_domain_ref", cascade="all, delete-orphan"
    )
...
class STEMSubject(db.Model):
    ...
    stem_domain_id = db.Column(
        db.Integer, db.ForeignKey("stem_domains.id"), nullable=False
    )
    educational_level_id = db.Column(
        db.Integer, db.ForeignKey("educational_levels.id"), nullable=False
    )

    # Integration and scheduling
    integration_level = db.Column(
    ...
    learning_modules = db.relationship(
        "STEMLearningModule", backref="stem_subject", cascade="all, delete-orphan"
    )
...
class STEMLearningModule(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    stem_subject_id = db.Column(
        db.Integer, db.ForeignKey("stem_subjects.id"), nullable=False
    )
    educational_level_id = db.Column(
        db.Integer, db.ForeignKey("educational_levels.id"), nullable=False
    )
    ...
    primary_approach_id = db.Column(
        db.Integer, db.ForeignKey("learning_approaches.id"), nullable=False
    )
    ...
    projects = db.relationship(
        "STEMProject", backref="learning_module", cascade="all, delete-orphan"
    )
    assessments = db.relationship(
        "STEMAssessment", backref="learning_module", cascade="all, delete-orphan"
    )

    # Constraints
    __table_args__ = (
        CheckConstraint(
            "formative_assessment_percentage + summative_assessment_percentage = 100",
            name="assessment_percentage_check",
        ),
    )
...
class STEMProject(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    learning_module_id = db.Column(
        db.Integer, db.ForeignKey("stem_learning_modules.id"), nullable=False
    )
    ...
    submissions = db.relationship(
        "STEMProjectSubmission", backref="project", cascade="all, delete-orphan"
    )
...
class STEMAssessment(db.Model):
    ...
    )  # Practical, Project, Portfolio, Presentation
    ...
    results = db.relationship(
        "STEMAssessmentResult", backref="assessment", cascade="all, delete-orphan"
    )

    # Constraints
    __table_args__ = (
        CheckConstraint(
            "scientific_method_weight + technical_skills_weight + innovation_creativity_weight + communication_weight = 100",
            name="stem_assessment_weight_check",
        ),
    )
...
class STEMProjectSubmission(db.Model):
    ...
    project_id = db.Column(
        db.Integer, db.ForeignKey("stem_projects.id"), nullable=False
    )
    student_id = db.Column(db.Integer, db.ForeignKey("students.id"), nullable=False)

    # Group project handling
    group_members = db.Column(
        db.JSON, nullable=True
    )  # List of student IDs for group projects
...
class STEMAssessmentResult(db.Model):
    ...
    assessment_id = db.Column(
        db.Integer, db.ForeignKey("stem_assessments.id"), nullable=False
    )
    student_id = db.Column(db.Integer, db.ForeignKey("students.id"), nullable=False)
    ...
    # Competency tracking
    competencies_demonstrated = db.Column(
        db.JSON, nullable=True
...
class STEMResourceCenter(db.Model):
    ...
    bookings = db.relationship(
        "STEMResourceBooking", backref="resource", cascade="all, delete-orphan"
    )
...
class STEMResourceBooking(db.Model):
    ...
    resource_id = db.Column(
        db.Integer, db.ForeignKey("stem_resource_center.id"), nullable=False
    )
    ...
    # Relationships
    booker = db.relationship(
        "User", foreign_keys=[booked_by], backref="stem_resource_bookings"
    )
    approver = db.relationship(
        "User", foreign_keys=[approved_by], backref="approved_stem_bookings"
    )
