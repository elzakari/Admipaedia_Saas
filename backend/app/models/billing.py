...
class SchoolPlanSubscription(db.Model):
    ...
    plan_id = db.Column(
        db.Integer, db.ForeignKey("plans.id"), nullable=False, index=True
    )
    ...
    features_snapshot = db.Column(
        db.JSON().with_variant(JSONB(), "postgresql"), nullable=True
    )
    limits_snapshot = db.Column(
        db.JSON().with_variant(JSONB(), "postgresql"), nullable=True
    )
    created_at = db.Column(db.DateTime(timezone=True), server_default=func.now())
    updated_at = db.Column(
...
class SchoolFeatureOverride(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "school_id", "feature_key", name="uq_school_feature_overrides_school_key"
        ),
...
class SchoolLimitOverride(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "school_id", "limit_key", name="uq_school_limit_overrides_school_key"
        ),
...
class StudentTermRegistration(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            ...
            "academic_term_id",
            "student_id",
            name="uq_student_term_registration",
        ),
        db.Index(
    ...
    academic_term_id = db.Column(
        db.Integer, db.ForeignKey("academic_terms.id"), nullable=False, index=True
    )
    registration_status = db.Column(
        db.String(20), nullable=False, server_default="registered"
    )
...
class BillingInvoice(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "tenant_id", "academic_term_id", name="uq_billing_invoice_tenant_term"
        ),
    ...
    branch = db.relationship(
        "Branch", backref=db.backref("billing_invoices", lazy=True)
    )
...
class PlanPricingTier(db.Model):
    ...
    __table_args__ = (
        db.Index(
            "idx_plan_pricing_tiers_plan_country_currency_active",
            "plan_id",
            "country_code",
            "currency",
            "is_active",
        ...
        db.Index(
            "idx_plan_pricing_tiers_plan_range",
            "plan_id",
            "min_students",
            "max_students",
        ),
    ...
    plan = db.relationship(
        "Plan",
        backref=db.backref("pricing_tiers", lazy=True, cascade="all, delete-orphan"),
    )
...
class SubscriptionChangeRequest(db.Model):
    ...
    effective_academic_term_id = db.Column(
    ...
    requested_plan = db.relationship(
        "Plan", backref=db.backref("change_requests", lazy=True)
    )
...
class PendingInvoiceAdjustment(db.Model):
    ...
    invoice_id = db.Column(
        db.Integer, db.ForeignKey("billing_invoices.id"), nullable=True, index=True
    )
