...
role_permissions = db.Table(
    ...
    db.Column(
        "permission_id",
        db.Integer,
        db.ForeignKey("rbac_permissions.id"),
        primary_key=True,
    ),
...
user_role_assignments = db.Table(
...
role_hierarchy = db.Table(
    "role_hierarchy",
    db.Column(
        "parent_role_id", db.Integer, db.ForeignKey("rbac_roles.id"), primary_key=True
    ),
    db.Column(
        "child_role_id", db.Integer, db.ForeignKey("rbac_roles.id"), primary_key=True
    ),
...
class RBACPermission(db.Model):
    ...
    scope = db.Column(
        db.String(50), default="global"
    )  # global, department, class, self
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "permission_type": (
                self.permission_type.value if self.permission_type else None
            ),
            "scope": self.scope,
...
class RBACRole(db.Model):
    ...
    def get_all_permissions(self, include_inherited: bool = True) -> Set[str]:
        ...
        if include_inherited:
            for parent_role in self.parent_roles:
                permissions.update(
                    parent_role.get_all_permissions(include_inherited=True)
                )
    ...
    def to_dict(self, include_permissions: bool = False) -> Dict[str, Any]:
        ...
        if include_permissions:
            result["permissions"] = [
                p.to_dict() for p in self.permissions if p.is_active
            ]
...
class UserRoleAssignment(db.Model):
    ...
    role_id = db.Column(
        db.Integer, db.ForeignKey("rbac_roles.id"), nullable=False, index=True
    )
    ...
    user = db.relationship(
        "User", foreign_keys=[user_id], backref="detailed_role_assignments"
    )
...
class PermissionGrant(db.Model):
    ...
    permission_id = db.Column(
        db.Integer, db.ForeignKey("rbac_permissions.id"), nullable=False, index=True
    )
...
class AccessControlList(db.Model):
    ...
    # Permission
    permission_id = db.Column(
        db.Integer, db.ForeignKey("rbac_permissions.id"), nullable=False
    )

    # Access control
    access_type = db.Column(
        db.String(10), default="allow", nullable=False
    )  # 'allow', 'deny'
    priority = db.Column(
    ...
    def create_role(self, role_name: str, **kwargs) -> RBACRole:
        ...
        role = RBACRole(
            ...
            **{
                ...
                if k in ["level", "color", "icon", "priority"]
            },
        )
