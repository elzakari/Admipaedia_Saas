...
from app.models.rbac import RBACPermission, RBACRole
    ...
    with app.app_context():
        try:
            ...
            if existing_permissions > 0 or existing_roles > 0:
                logger.warning(
                    f"RBAC system already initialized. Found {existing_permissions} permissions and {existing_roles} roles."
                )
                response = input(
                    "Do you want to reinitialize? This will clear existing RBAC data. (y/N): "
                )
                if response.lower() != "y":
            ...
            if admin_user:
                logger.info(
                    f"Assigning super_admin role to user: {admin_user.username}"
                )
                RBACService.assign_role_to_user(admin_user.id, "super_admin")
                logger.info("Super admin role assigned successfully.")
            else:
                logger.warning(
                    "No admin user found. Please create an admin user and assign the super_admin role manually."
                )
...
def print_rbac_summary():
    ...
    for role in roles:
        ...
        print(
            f"  • {role.display_name} ({role.name}) - Level {role.hierarchy_level} - {perm_count} permissions"
        )
