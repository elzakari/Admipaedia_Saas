...
def check_resource_access(
    ...
    acl_entries = (
        ...
        .order_by(AccessControlList.priority.desc())
        .all()
    )

    user_roles = [
        assignment.role.id
        for assignment in UserRoleAssignment.query.filter_by(
            user_id=user.id, is_active=True
        ).all()
        if assignment.is_valid()
    ]
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if resource_type and resource_id_param:
                ...
                if resource_id and not check_resource_access(
                    user, resource_type, str(resource_id), permission_name
                ):
                    logger.warning(
...
def require_role(role_names: Union[str, List[str]], require_all: bool = False):
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if (
                user_legacy_role
                in ("admin", "superadmin", "super_admin", "super_manager")
                or user_legacy_role in role_names
            ):
                g.current_user = user
            ...
            if require_all:
                ...
                if missing_roles:
                    ...
                    return (
                        jsonify(
                            {
                                "error": f"Missing required roles: {', '.join(missing_roles)}"
                            }
                        ),
                        403,
                    )
...
def require_any_permission(permission_names: List[str]):
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not user:
                logger.warning(
                    "any_permission_check_no_user", permissions=permission_names
                )
                return jsonify({"error": "Authentication required"}), 401
...
def require_all_permissions(permission_names: List[str]):
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not user:
                logger.warning(
                    "all_permissions_check_no_user", permissions=permission_names
                )
                return jsonify({"error": "Authentication required"}), 401
