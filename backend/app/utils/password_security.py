...
class PasswordSecurity:
    ...
    def validate_password_strength(
        cls, password: str, username: str = None, email: str = None
    ) -> Tuple[bool, List[str]]:
        """
    ...
    def _is_similar(cls, password: str, reference: str, threshold: float = 0.7) -> bool:
        ...
        similarity = len(common_chars) / max(
            len(set(password_lower)), len(set(reference_lower))
        )
...
class AccountSecurity:
    ...
    def record_failed_login(
        ...
        attempt = LoginAttempt(
            ...
            or (request.headers.get("User-Agent") if request else None),
            success=False,
        ...
        if isinstance(LoginAttempt, Mock) or hasattr(
            LoginAttempt, "_mock_return_value"
        ):
    ...
    def record_successful_login(
        cls, identifier: str, ip_address: str = None, user_agent: str = None
    ):
    ...
    def is_account_locked(cls, identifier: str) -> Tuple[bool, int]:
        ...
        cutoff_time = datetime.utcnow() - timedelta(
            minutes=PasswordSecurity.LOCKOUT_DURATION
        )
        ...
        if isinstance(LoginAttempt, Mock) or hasattr(
            ...
            recent_failed = (
                ...
                .limit(PasswordSecurity.MAX_FAILED_ATTEMPTS)
                .all()
            )
        ...
        if len(recent_failed) >= PasswordSecurity.MAX_FAILED_ATTEMPTS:
            ...
            if not successful_since:
                unlock_time = last_failed + timedelta(
                    minutes=PasswordSecurity.LOCKOUT_DURATION
                )
                if datetime.utcnow() < unlock_time:
                    seconds_left = int(
                        (unlock_time - datetime.utcnow()).total_seconds()
                    )
    ...
    def get_lockout_info(cls, identifier: str) -> Dict:
        ...
        return {
            ...
            "remaining_attempts": max(
                0, PasswordSecurity.MAX_FAILED_ATTEMPTS - failed_count
            ),
        }
