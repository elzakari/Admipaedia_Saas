...
from app.services.entitlements.service import (EntitlementError,
                                               EntitlementService)
...
def has_any_feature_access(
    ...
    *,
    ...
    if isinstance(features, dict) and features:
        return any(
            bool(features.get(feature_key)) for feature_key in normalized_feature_keys
        )
    ...
    if normalized_enabled_features:
        return any(
            feature_key in normalized_enabled_features
    ...
    if tenant and fallback_plan_slugs:
        ...
        if normalized_plan in {
            str(plan_slug).strip().lower()
            for plan_slug in fallback_plan_slugs
            if str(plan_slug).strip()
        }:
            return True
