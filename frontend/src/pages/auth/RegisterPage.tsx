        ...
        <div className="w-full max-w-md">
          <div className="bg-white/90 backdrop-blur rounded-2xl shadow-2xl ring-1 ring-black/5 p-6 sm:p-8">
            ...
            <div className="text-center mb-6">
              <h1 className="text-3xl font-display font-bold text-slate-900 tracking-tight">{t('auth.create_account_title', 'Create your account')}</h1>
              <p className="mt-2 text-sm text-slate-600">
                <span>{t('auth.already_have_account', 'Already have an account?')} </span>
                <Link to="/login" className="font-semibold text-indigo-700 hover:text-indigo-800">
                  {t('auth.sign_in', 'Sign in')}
                </Link>
              </p>
              <p className="mt-2 text-sm text-slate-600">
                <span>{t('auth.parent_question', 'Are you a parent?')} </span>
                <Link to="/parent-register" className="font-semibold text-indigo-700 hover:text-indigo-800">
                  {t('auth.register_as_parent', 'Register as a parent')}
                </Link>
              </p>
            </div>
