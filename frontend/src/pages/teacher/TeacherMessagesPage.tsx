...
import { teacherThreads } from './teacherMockData';
import { MessageSquare, Send } from 'lucide-react';

const TeacherMessagesPage: React.FC = () => {
  const [activeId, setActiveId] = useState(teacherThreads[0]?.id ?? '');
  const active = useMemo(() => teacherThreads.find((t) => t.id === activeId) ?? null, [activeId]);
