    ...
    <div className="p-6 space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Read-only school calendar</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="text-sm text-slate-600">Loading…</div>
          ) : !list.length ? (
            <div className="text-sm text-slate-600">No events this month.</div>
          ) : (
            <div className="space-y-2">
              {list.map((e: any) => (
                <div key={e.id} className="rounded-lg border border-slate-200 p-3">
                  ...
                  <div className="text-xs text-slate-600">{e.date ? new Date(e.date).toLocaleString() : ''}{e.location ? ` • ${e.location}` : ''}</div>
