...
const studentPrivacyPrefsStorageNamespace = 'sms.student.privacy_prefs.v1';
...
const StudentPrivacySettings: React.FC = () => {
  const { toast } = useToast();

  const initial = useMemo(() => {
    try {
      const raw = localStorage.getItem(studentPrivacyPrefsStorageNamespace);
  ...
  const save = async () => {
    setIsSaving(true);
    try {
      localStorage.setItem(studentPrivacyPrefsStorageNamespace, JSON.stringify(prefs));
  ...
  const reset = () => {
    setPrefs(defaults);
    localStorage.setItem(studentPrivacyPrefsStorageNamespace, JSON.stringify(defaults));
